...
- `Change image` and the context-menu item `Choose image from your computer...` are the same
  ...
  the gallery and goes straight to the picker. This suite attaches to an ordinary running Bloom,
  not one started with `--e2e`, so the `e2e/nextFileToChoose` hook that the BloomE2E suite arms
  is **not registered** here (posting to it raises Bloom's missing-endpoint problem dialog). The
  way to get past a picker in this suite is `winformsUia.ps1` in the `bloom-automation` skill:
  it fills the picker's "File name:" box and presses Open, or Cancel, over UI Automation, with no
  pointer or keyboard input. Use `armFileChooser` (`src/BloomE2E/helpers/talkingBook.ts`) only
  when the Bloom under test was launched with `--e2e`.
