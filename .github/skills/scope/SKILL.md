...
Do exactly this, in order: 0) Preflight: if `scope-mcp` is available, call `list_pages`.
...
1. If you know which component they mean (because it was the last one discussed in this chat), run `yarn scope` from that component folder.
2. Else, if you are currently working inside a specific component folder, run `yarn scope` from there.
3. Else ask exactly ONE question: “Which component folder (must be under `src/BloomBrowserUI/react_components/`)?”

Then do NOT open any more browsers. `yarn scope` already opens a Chromium tab that tools can attach to via remote debugging.

Immediately after running `yarn scope`, do exactly ONE “sanity check” pass and then STOP:
...
3. Select the harness tab and take ONE snapshot to confirm it is actually reachable.
    - If the snapshot shows an unreachable/connection-refused page, re-run `yarn scope` once (it starts/reuses Vite) and then re-run steps (2) and (3), then STOP.
...
Never call `open_simple_browser` as part of this workflow. Never open any browsers. `yarn scope` does that.

Hard rules (to prevent over-thinking):

-   Do NOT run extra terminal diagnostics after `yarn scope` (no `python`, no `curl` to `/json/list`, no port-scanning, no grepping logs).
...
`cd src/BloomBrowserUI/react_components/color-picking && yarn scope`

To use a real Bloom backend (Bloom running on localhost:8089):

`cd src/BloomBrowserUI/react_components/color-picking && yarn scope --backend`
