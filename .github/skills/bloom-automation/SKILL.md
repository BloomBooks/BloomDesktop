...
- **"Bloom had a problem" report dialogs.** Bloom surfaces errors (including non-fatal ones, especially in Debug builds) as a modal "Bloom had a problem" dialog. It is hosted in its OWN WinForms window with its own WebView2, so it appears as a SEPARATE CDP page target — not inside the shell document or the `page` iframe — and in dev it is even served from the Vite port rather than the Bloom http port. Detect it by the `.problem-dialog` root (from `problemDialog/*.tsx`) present in ANY page target. Never just leave one sitting on screen, and never move past it silently.
  ...
  - **The dialog can be invisible to CDP, and then the helper reports "no problem dialog was showing" while one is on screen.** Every WebView2 Bloom makes asks for the same remote-debugging port; only the first window binds it, so a dialog window opened later exposes no target at all, and `/json/list` shows just the main page. Two other things do show it, and neither needs CDP: the log at `%TEMP%\SIL\Bloom\Log.txt` (grep `NonFatalProblem:` and `ProblemReportApi is about to report`, which carries the full stack and the detail message), and the list of VISIBLE top-level windows of the Bloom process, where a second, untitled `WindowsForms10.Window` beside the titled main window is the dialog's host. Closing still works: `POST /bloom/api/common/closeReactDialog` with `Content-Length: 0` (curl needs `-H "Content-Length: 0"` on a body-less POST, or Bloom answers HTTP 411), once per queued report, then re-check the window list.
  - **Check for the dialog as a routine, not only when a step fails.** The developer often watches the live Bloom while an agent drives it, and a report dialog the agent never noticed sits on top of the screen looking abandoned. So: run the detect-or-dismiss check (the helper, or a cheap "any target with `.problem-dialog`" probe) after every action that changes Bloom's state, again before you take any screenshot you intend to show anyone, and once more before you end a live session. A dialog you find is FINDINGS, not an obstacle: gather its stack first, close it second, and account for it in your report.
...
- **After relinking/rebuilding a linked front-end dependency**, restart the whole `go.sh` session rather than relying on hot reload; otherwise the page runs the stale copy. A launcher `/restart` is *not* enough: it restarts Bloom.exe but the Vite dev server started by the first `go.sh` keeps running and keeps serving the module it transformed at startup (`curl http://localhost:<vitePort>/@id/<dep>` and grep for your change to confirm). Do not kill the Vite process on its own either, because that takes the launcher down with it and the control API disappears. Stop the session (`POST /shutdown`) and start `./go.sh` again; the new session picks new Vite and control ports, so re-read `output/bloom-launcher.json`. **Do not trust a rebuild that worked once.** On BL-16777 the first `pnpm run build` of the linked `bloom-table` plus a launcher `POST /restart` did reach the running Bloom, and the second build of the same session did not: Vite had cached the transform of `/@fs/D:/bloom-table/dist/bloom-table.mjs?t=<its own timestamp>`, and it never invalidated again, because rollup replaces the file rather than writing it in place and the watcher does not follow it. The symptom is not an error message: Bloom sits on the startup spinner for ever, and the reason only appears if you import the entry yourself in the page (`await import("http://localhost:<vitePort>/app/App.entry.tsx?probe=1")`), which answers `does not provide an export named '<your new export>'`. Two checks, in this order: `curl http://localhost:<vitePort>/@fs/<absolute path to the dependency's built module>` and grep for your change; then the same URL with `?v=probe` added, which forces a fresh transform. New content only with the query means Vite is serving a stale module and the whole session has to restart.
- **Reach a submenu item sideways, never diagonally.** A menu built of MUI nested items opens
  ...
  **If even the sideways move closes the submenu**, stop driving the mouse by coordinates:
  hover the parent with `locator.hover()`, then click the submenu item with
  `locator.click({force: true})`. Playwright jumps straight to the element rather than
  interpolating a path, so nothing in between gets a mouseover. Verified on the canvas
  context toolbar's "..." menu ("Cell Type" → "Text"), where the coordinate route closed the
  submenu between reading the item's rectangle and clicking it, and the keyboard route did
  not work at all: ArrowRight on the parent row moved focus to the body, so these submenus
