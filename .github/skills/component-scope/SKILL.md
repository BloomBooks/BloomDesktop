...
description: Launch / Show a component in a shared MCP-controllable browser and collaborate in the same tab. We call this the "scope".
metadata:
...
1) If you know which component they mean (because it was the last one discussed in this chat), run its component-local `show-mcp.sh`.
2) Else, if you are currently working inside a specific component folder and it contains `show-mcp.sh`, run that.
3) Else ask exactly ONE question: “Which component folder (must be under `src/BloomBrowserUI/react_components/`)?”

Then do NOT open any more browsers. The show script already opens a Chromium tab that MCP can control.

Immediately after running `show-mcp.sh`, do exactly ONE “sanity check” pass and then STOP:
1) Verify the launch succeeded by confirming the script printed something like:
  - “opened an scope on port …”
2) Verify scope mcp can see the page (minimal chrome-devtools-9222 check):
  - call `list_pages` on the scope-mpc
  - confirm there is a tab whose URL looks like the Vite harness (e.g. `http://127.0.0.1:####/?modulePath=...`)
3) If the MCP tools show `about:blank`, then you opened two browsers instead of one. Close the "about:blank" one.

Only after the user asks to interact with the component (click/type/inspect), you may do additional MCP calls like:
- `select_page`
- `take_snapshot`
