...
# 2) Screen out the two C# files that we know may have calls using SIL.IO.File safely
...
#    is complicated slightly by a set of localization IDs which use File.)
...
git status --porcelain=v1 --untracked=no --ignored=no --no-renames | grep '^[AM]' | cut -c4- | grep '\.cs$' | \
	grep -v 'src/BloomExe/RobustFileIO.cs$' | grep -v 'src/BloomExe/ToPalaso/RobustIO.cs$' | \
	grep -v 'src/BloomTests/' | \
	xargs grep -H '\([^A-Za-z0-9_]File\.[A-Z]\|new\s\+FileStream\)' | grep -v 'PublishTab\.Android\.File\.'
status=$?
# if grep finds instances of File.<method>, then stop the commit.
