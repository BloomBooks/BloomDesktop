...
Entity-relationship diagram of the Supabase Postgres `tc` schema. Reflects the migrations in
`supabase/migrations/` through `20260716000001`. Renders on GitHub and in any mermaid-aware
viewer. See `CONTRACTS.md` for the RPC/edge-function surface that reads and writes these tables
(clients never write them directly — all mutations go through RLS-gated RPCs / edge functions).
...
migration changes the schema: a new / removed / renamed table or column, or a changed foreign key.

1. Make the schema change in a migration under `supabase/migrations/` as usual.
2. Reconcile the `erDiagram` block above with the new reality. To see the current definitions
   quickly, from the repo root:

   ```bash
   git grep -nE "CREATE TABLE"   -- supabase/migrations   # every table + where it's defined
   git grep -nE "ADD COLUMN"     -- supabase/migrations   # columns added by later migrations
   git grep -nE "REFERENCES tc\." -- supabase/migrations  # foreign keys
   ```

   (Or run `supabase db reset` and inspect the live schema, e.g. in Studio.)
3. Update the "through migration `YYYYMMDDNNNNNN`" note near the top of this file to the latest
   migration you accounted for.
4. Preview before committing: paste the fenced ```mermaid block into <https://mermaid.live>, or
