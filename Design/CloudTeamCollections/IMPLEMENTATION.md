...
- 20 Jul 2026 · DB → declarative schema · direct commits · Replaced the 16 forward-only
  `supabase/migrations/*.sql` with a declarative source of truth in `supabase/schemas/`
  (01_schema, 02_functions, 03_tables, 04_security), wired via `[db.migrations].schema_paths`.
  History was intentionally discarded (only a local DB existed). The single
  `20260720000001_init_tc_schema.sql` that actually runs is a *generated* lossless concatenation
  of the schema files (`build/regen-init-migration.sh`) — NOT `supabase db diff`, which was found
  to silently drop every `COMMENT ON` and `GRANT EXECUTE ON FUNCTION` (both diff engines). Verified:
  a fresh `db reset` reproduces the pre-refactor schema **byte-for-byte** (pg_dump diff empty) and
  all 102 pgTAP pass. This SUPERSEDES the 7 Jul "new migration files only" convention below; see
  CONTRACTS.md → "Database: declarative schema" for the current pre/post-launch workflow.
