...
public static class CollapsibleSections
{
    ...
    ///
    /// This exact shape has been confirmed to render on the real tracker as a collapsed block with the
    /// angle brackets intact; only `&lt;summary&gt;` is untested. The risk in changing it is that an
    /// unsupported tag shows as literal text on every card, which is worse than not collapsing at all.
    /// </summary>
