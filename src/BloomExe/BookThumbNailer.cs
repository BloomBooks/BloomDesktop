		...
		internal static bool CreateThumbnailOfCoverImage(Book.Book book, HtmlThumbNailer.ThumbnailOptions options, Action<Image> callback = null)
		{
			...
			try
			{
				...
				using (var coverImage = PalasoImage.FromFile(imageSrc))
				{
					...
					var dashedBorder = options.BorderStyle == HtmlThumbNailer.ThumbnailOptions.BorderStyles.Dashed;
					coverImage.Image = options.CenterImageUsingTransparentPadding ?
						ImageUtils.CenterImageIfNecessary(new Size(size, size), coverImage.Image, dashedBorder) :
						ImageUtils.ResizeImageIfNecessary(new Size(size, size), coverImage.Image, dashedBorder);
