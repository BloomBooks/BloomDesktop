...
namespace Bloom.Api
{
	...
	class ReadersApi : IDisposable
	{
		...
		public void RegisterWithApiHandler(BloomApiHandler apiHandler)
		{
			...
			{
				var bookFontName = _bookSelection.CurrentSelection.BookData.GetLanguage(LanguageSlot.Language1).FontName;
		...
		private string GetSampleTextsList(string settingsFilePath)
		{
			...
			var langFileName = String.Format(DecodableReaderToolSettings.kSynphonyLanguageDataFileNameFormat,
				CurrentBook.BookData.GetLanguage(LanguageSlot.Language1).Iso639Code);
		...
		private static string GetReaderSettings(BookData bookData)
		{
			...
			if (jsonSettings.Length > 0)
			{
				...
				fixedSettings.writingSystemTag = bookData.GetLanguage(LanguageSlot.Language1).Iso639Code;
			...
			var settingsString = "{\"writingSystemTag\": \"" + bookData.GetLanguage(LanguageSlot.Language1).Iso639Code+"\", "
			                     +"\"letters\":\"a b c d e f g h i j k l m n o p q r s t u v w x y z\","
				+ $"\"lang\":\"{bookData.GetLanguage(LanguageSlot.Language1).Iso639Code}\","
			                     + "\"moreWords\":\"\","
		...
		private void MakeLetterAndWordList(string jsonSettings, string allWords)
		{
			...
			sb.AppendLineFormat(str, CurrentBook.BookData.GetLanguage(LanguageSlot.Language1).Name);
		...
		private void SaveSynphonyLanguageData(string jsonString)
		{
			...
			if (jsonString.Contains("\"LangName\":\"\""))
				jsonString = jsonString.Replace("\"LangName\":\"\"", "\"LangName\":\"" + CurrentBook.GetLanguage(LanguageSlot.Language1).Name + "\"");

			if (jsonString.Contains("\"LangID\":\"\""))
				jsonString = jsonString.Replace("\"LangID\":\"\"", "\"LangID\":\"" + CurrentBook.GetLanguage(LanguageSlot.Language1).Iso639Code + "\"");

			var fileName = String.Format(DecodableReaderToolSettings.kSynphonyLanguageDataFileNameFormat, CurrentBook.GetLanguage(LanguageSlot.Language1).Iso639Code);
