	...
	public class AddOrChangePageApi
	{
			...
			bool allowDataLoss)
		{
			...
			foreach (var page in book.GetPages())
			{
				...
				// Even if the pages have the same original template, if one has been edited to have a different number
				// of editable blocks of some kind, it's too dangerous to migrate it; it's not 'similar' enough.
				// For example, quite different pages could have all been created from our 'custom' template. See BL-11147.
