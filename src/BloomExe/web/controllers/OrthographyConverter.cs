    ...
    public class OrthographyConverter
    {
        ...
        protected void InitializeMappings(string filename)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    Debug.Assert(
                        !mappings.ContainsKey(fields[0]),
                        $"Invalid Orthography Conversion settings ({filename}). Duplicate key: \"{fields[0]}\"."
                    );
                ...
                else
                {
                    Debug.Assert(
                        false,
                        $"OrthographyConverter: Parsed line with invalid format (2+ fields expected): \"{line}\""
                    );
                }
            }
