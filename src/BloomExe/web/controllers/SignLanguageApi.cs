	...
	public class SignLanguageApi
	{
			...
			string sourceBookFolder)
		{
			...
			if (!string.IsNullOrEmpty(FfmpegProgram) && !string.IsNullOrEmpty(timings) &&
			    VideoIsMarkedForTrimming(sourceBookFolder, videoUrl, timings))
			{
		...
		private static bool VideoIsMarkedForTrimming(string sourceBookFolder, string videoUrl, string rawTimings)
		{
			var timings = new[] { 0.0m, 0.0m };
			ConvertRawTimingsToDecimalArray(rawTimings, timings);
			if (timings[0] > 0) return true;
			...
			// if our trim setting is less than 1/10 second from the end, don't bother trimming the video.
			return duration - timings[1] > 0.1m;
		}
