...
namespace Bloom.web.controllers
{
	...
	public class SignLanguageApi
	{
		...
		private static void ParseDuration(string output, Dictionary<string, object> statistics)
		{
			...
			statistics.Add("vidDuration",
				duration.Substring(0, 3) == "00:" ? duration.Substring(3, 5) : duration.Substring(0, 8));
		}

		private static void ParseFileSize(long sizeInBytes, Dictionary<string, object> statistics)
		{
			var sizeInMb = ConvertBytesToMegabyteString(sizeInBytes);
			if (sizeInMb != string.Empty)
			{
				statistics.Add("vidFileSize", sizeInMb + " MB");
			}
		}
		...
		private static void ParseFrameSize(string output, Dictionary<string, object> statistics)
		{
			...
			// put out www x hhh
			statistics.Add("vidFrameSize", $"{width} x {height}");
		}

		private static void ParseFramesPerSecond(string output, Dictionary<string, object> statistics)
		{
			...
			if (!match.Success)
				return;
			var fps = match.Value;
			statistics.Add("vidFramesPerSecond", fps.Substring(2).ToUpper(CultureInfo.CurrentUICulture));
		}

		private static void ParseFileFormat(string output, Dictionary<string, object> statistics)
		{
			var re = new Regex("[V|v]ideo:\\w[A-Za-z0-9]*\\w");
			var match = re.Match(output);
			if (!match.Success)
				return;
			statistics.Add("vidFileFormat", match.Value.Substring(7).ToUpper(CultureInfo.CurrentUICulture));
		}
