	...
	public class ImageApi
	{
		...
		private static string CreateMultiplePageReference(List<string> pages)
		{
			...
			for (var i = 1; i < pages.Count; i++)
			{
				...
				if (NewPageShouldBeGroupedWithPreviousPage(previousPage, thisPage))
				{
					if (!workingOnDash)
					{
						workingOnDash = true;
		...
		private static bool NewPageShouldBeGroupedWithPreviousPage(int prevPage, string newPage)
		{
			int newPageNum;
			if (!Int32.TryParse(newPage, out newPageNum))
			{
				return false;
			}
			return newPageNum == prevPage + 1;
		}
