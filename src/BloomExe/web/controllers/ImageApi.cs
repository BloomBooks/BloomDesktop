...
namespace Bloom.web.controllers
{
	public class ImageApi
	{
		...
		/// <returns></returns>
		public static IEnumerable<string> CollectFormattedCredits(Dictionary<string, List<string>> credits)
		{
		...
		private static string CreateMultiplePageReference(List<string> pages)
		{
			...
			sb.Append(pages[0]);
			int lastPage;
			IsInt(pages[0], out lastPage);
			for (var i = 1; i < pages.Count; i++)
			{
				var thisPage = pages[i];
				if (IsIncrementOf(lastPage, thisPage))
				{
					if (!workingOnDash)
					{
						workingOnDash = true;
				...
				else
				{
					if (workingOnDash)
					{
						sb.Append(lastPage.ToString());
						workingOnDash = false;
					}
					sb.Append(commaSpace + thisPage);
				}
				IsInt(thisPage, out lastPage);
			}
			if (workingOnDash)
			{
				sb.Append(lastPage.ToString());
		...
		private static bool IsInt(string pageLabel, out int result)
		{
			result = -1;
			try
			{
				result = Convert.ToInt32(pageLabel);
				return true;
			}
		...
		private static bool IsIncrementOf(int lastPage, string newPage)
		{
			int newPageNum;
			if (!IsInt(newPage, out newPageNum))
			{
				return false;
			}
			return newPageNum == lastPage + 1;
		}
