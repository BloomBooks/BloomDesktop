...
namespace Bloom.web.controllers
{
	...
	public class CopyrightAndLicenseApi
	{
		...
		private void HandleBookCopyrightAndLicense(ApiRequest request)
		{
			switch (request.HttpMethod)
			{
				case HttpMethods.Get:
					...
					var intellectualPropertyData = GetJsonFromMetadata(Model.CurrentBook.GetLicenseMetadata(), forBook: true);
					request.ReplyWithJson(intellectualPropertyData);
					break;
				case HttpMethods.Post:
					try
					{
						Model.ChangeBookLicenseMetaData(GetMetadataFromJson(request, forBook: true));
					}
					catch (Exception ex)
					{
		...
		private static string GetCcDerivativeRulesAsString(CreativeCommonsLicense.DerivativeRules rules)
		{
			switch (rules)
			{
