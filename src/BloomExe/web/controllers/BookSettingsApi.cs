...
namespace Bloom.Api
{
    ...
    public class BookSettingsApi
    {
        ...
        private void HandleStylesAndFonts(ApiRequest request)
        {
            ...
            foreach (var style in stylesInBook)
            {
        ...
        private static HashSet<string> GetStylesUsedInBook(HtmlDom dom, string bookFolderPath)
        {
            var stylesInBook = new HashSet<string>();
            foreach (var div in dom.SafeSelectNodes("//div[contains(@class,'bloom-page')]//div[contains(@class, '-style')]"))
            {
                ...
                if (!stylesInBook.Contains(style))
                {
                    //Debug.WriteLine($"DEBUG: {style} first seen on page id={pageDiv.GetAttribute("id")} xmatter={pageDiv.GetAttribute("data-xmatter-page")} number={pageDiv.GetAttribute("data-page-number")}");
                    stylesInBook.Add(style);
                }
            }
            return stylesInBook;
        }
