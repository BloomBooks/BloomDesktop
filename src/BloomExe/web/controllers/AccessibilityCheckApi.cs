...
namespace Bloom.web.controllers
{
	...
	public class AccessibilityCheckApi
	{
		...
		{	
			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "bookName", request =>
			{
				request.ReplyWithText(request.CurrentBook.TitleBestForUserDisplay);
			}, false);

			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "showAccessibilityChecker", request =>
			{
				AccessibilityCheckWindow.StaticShow(()=>_webSocketServer.SendEvent(kWebSocketContext, kWindowActivated));
				request.PostSucceeded();
			}, true);

			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "descriptionsForAllImages", request =>
			{
			...
			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "audioForAllImageDescriptions", request =>
			{
			...
			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "audioForAllText", request =>
			{
			...
			server.ApiHandler.RegisterBooleanEndpointHandler(kApiUrlPart + "noEssentialInfoByColor",
			...
			// Just a checkbox that the user ticks to say "yes, I checked this"
			// At this point, we don't have a way to clear that when the book changes.
			server.ApiHandler.RegisterBooleanEndpointHandler(kApiUrlPart + "noTextIncludedInAnyImages",
			...
			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "aceByDaisyReportUrl", request => { MakeAceByDaisyReport(request); },
				false, false
				);
			...
			server.ApiHandler.RegisterBooleanEndpointHandler(kApiUrlPart + "cataracts",
			...
			server.ApiHandler.RegisterBooleanEndpointHandler(kApiUrlPart + "colorBlindness",
				request => _simulateColorBlindness,
				(request, b) => { _simulateColorBlindness = b; },
				false);
			server.ApiHandler.RegisterEnumEndpointHandler(kApiUrlPart + "kindOfColorBlindness",
