...
namespace Bloom.web.controllers
{
	internal class ProblemReportApi : IDisposable
	{
		...
		private static BookSelection _myBookSelection = null;
		private static /*readonly*/ BookSelection BookSelection
		{
			get => _myBookSelection;

			// This setter basically simulates readonly for a static variable. It only allows the variable to be assigned to a non-null value once.
			// (The actual readonly keyword only allows you to set it from a static constructor, not a normal constructor)
			set
			{
				if (_myBookSelection == null)
					_myBookSelection = value;
				else
					throw new ApplicationException("BookSelection reference was modified after it was first instantiated.");
			}
		}
		...
		private string CollectionFolder => Path.GetDirectoryName(BookSelection.CurrentSelection.StoragePageFolder);
		...
		public void RegisterWithApiHandler(BloomApiHandler apiHandler)
		{
			...
			apiHandler.RegisterEndpointHandler("problemReport/screenshot",
				(ApiRequest request) =>
				{
					...
					_takingScreenshotLock.WaitOne(millisecondsTimeout: 5000);
		...
		private string CreateBookZipFile(string basename, string userDesc)
		{
			try
			{
				...
				_bookZipFile.AddDirectory(BookSelection.CurrentSelection.StoragePageFolder);
			...
			string detailedMessage = "", string levelOfProblem = "user", string shortUserLevelMessage = "")
		{
			...
			var book = BookSelection.CurrentSelection;
			var bestBookName = BookSelection.CurrentSelection?.TitleBestForUserDisplay;
		...
		// This lock uses a ManualResetEvent instead of a Monitor, because a Monitor only works for one thread.
		...
		private static ManualResetEvent _takingScreenshotLock = new ManualResetEvent(false);

		private static void TryGetScreenshot(Control controlForScreenshotting)
		{
			_takingScreenshotLock.Reset();	// Acquire lock (Blocks other threads)
				...
				SafeInvoke.Invoke("Screen Shot", controlForScreenshotting, false, true, () =>
					{
						...
						{
							_takingScreenshotLock.Set();	// Release lock (Unblock others)
						}
					}
				);
			}
		...
		{
			var email = string.IsNullOrWhiteSpace(userEmail) ? userEmail : _reportInfo?.UserEmail;
