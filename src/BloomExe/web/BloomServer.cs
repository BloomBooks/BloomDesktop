	...
	public class BloomServer : IBloomServer, IDisposable
	{
		...
		private bool ProcessImageFileRequest(IRequestInfo info)
		{
			...
			else if (!RobustFile.Exists(imageFile))
			{
				...
				// But things in the book folder should be processed (except when marked with OriginalImageMarker, handled above).
				// An earlier version of this code did not process book-preview images except for the cover image.
				// But I think this was a mistake: the main point of compressing images is to save memory for large
				// documents like our preview. One or two large images in a single page being edited should not hurt much,
				// and images for PDF generation get marked with OriginalImageMarker, so the only remaining place where
				// image compression can help much is in our preview!
				// Note that, if we change our minds about processing everything that's for a preview, we must at least
				// process anything that is for the cover image, so as to get background transparency.
