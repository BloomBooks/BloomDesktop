...
namespace Bloom.web
{
	...
	public class BloomServer : ServerBase
	{
		...
		static readonly Dictionary<string, string> _urlToSimulatedPageContent = new Dictionary<string, string>(); // see comment on MakeSimulatedPageFileInBookFolder
		...
		/// </summary>
		internal BloomServer() : this( new RuntimeImageProcessor(new BookRenamedEvent()))
		{ }

		public BloomServer(RuntimeImageProcessor cache)
		{
			_cache = cache;
		...
		/// </summary>
		public BloomServer(RuntimeImageProcessor cache, BloomFileLocator fileLocator)
			: this(cache)
		{
		...
		private readonly object I18NLock = new object();
		...
		/// <summary>
		/// This method just serves up images. Its job is to take the original image
		...
		/// </summary>
		private bool ProcessImageRequest(IRequestInfo info)
		{
			if (!_useCache)
				return false;
