...
namespace Bloom.Api
{
	...
	public class BloomWebSocketServer : IBloomWebSocketServer, IDisposable
	{
					...
					try
					{
						if (socket != null && socket.IsAvailable)
						{
							...
							catch (ConnectionNotAvailableException)
							{
								...
								ReportConnectionError(socket, eventBundle);
							}
						}
						else
						{
		...
		private void ReportConnectionError(IWebSocketConnection socket, dynamic eventBundle)
		{
			...
			NonFatalProblem.Report(ModalIf.Alpha, PassiveIf.Alpha, "web socket is not available when trying to send",
				$"{socketDetail} bundle clientContext: {eventBundle.clientContext}, eventId: {eventBundle.eventId}, eventBundle: {eventBundle}",
