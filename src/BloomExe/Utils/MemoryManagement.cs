...
namespace Bloom.Utils
{
    ...
    /// </summary>
    /// <remarks>
    /// This class was copied from libpalaso and then modified.  It does not seem to be used much
    /// (if at all) outside of Bloom.  Changes in libpalaso have become much harder over time, and
    /// some aspects of the code, such as the "safelimit" variable calculated in CheckMemory() would
    /// seem to be program specific.
    /// </remarks>
    public static class MemoryManagement
    {
        ...
        )
        {
            ...
            using (var proc = Process.GetCurrentProcess())
            {
                ...
                bldr.AppendFormat(
                    " Memory Use ({0}-bit process): private {1:N0}K, virtual {2:N0}K,",
                    Environment.Is64BitProcess ? 64 : 32,
                    memorySize64 / 1024,
                    virtualMemory64 / 1024
                );
                bldr.AppendFormat(" physical {0:N0}K,", workingSet64 / 1024);
                bldr.AppendFormat(" managed heap {0:N0}K,", heapMem / 1024);
                bldr.AppendLine();
                bldr.AppendFormat(
                    "        peak virtual {0:N0}K, peak physical {1:N0}K;",
                    peakVirtualMemory64 / 1024,
                    peakWorkingSet64 / 1024
                );
                bldr.AppendFormat(
                    " system virtual {0}K, system physical (RAM) {1:N0}K",
                    (memInfo.TotalVirtualMemory / 1024).ToString("N0"),
                    memInfo.TotalPhysicalMemory / 1024
                );
                bldr.AppendLine();
            ...
            // Set the limit where we get concerned enough to notify the user about how much memory
            // has been consumed.  (The message is phrased in terms of running low, but that's harder
            // to evaluate than running high.)
            var safelimit = Environment.Is64BitProcess
                ? 4000000000L
                : // may not be low of memory, but that's way too much memory to handle...
                (
                    Environment.Is64BitOperatingSystem
                        ? 2000000000L
                        : // Bloom is "Large Address Aware", so allow about half of the address space
                        1000000000L
                );
            // We also don't want Bloom to totally overwhelm the available physical memory.
            safelimit = Math.Min(safelimit, (long)memInfo.TotalPhysicalMemory);
            bool danger;
            ...
            {
                s_warningShown = true;
                // These strings are in the Palaso.xlf file owned by Bloom.  Lookup in L10NSharp.LocalizationManager
                // does not depend on the source file.  It depends only on the id string.  Since Crowdin likes to
                // remove approvals at the slightest change, I'm leaving the strings where they are.
                var warning = LocalizationManager.GetString(
                    "MemoryWarning",
                ...
                MessageBox.Show(
                    null,
                    string.Format(warning, Application.ProductName),
                    caption,
        ...
        public static MemoryInformation GetMemoryInformation()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    returnVal.TotalVirtualMemory =
                        availableMemory > twoGB ? twoGB : availableMemory;
                }
            }
            return returnVal;
        }
