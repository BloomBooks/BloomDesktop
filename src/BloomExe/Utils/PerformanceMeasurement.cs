	...
	*/
	public class PerformanceMeasurement :IDisposable
	{
		...
		public IDisposable Measure(string action, string details ="")
		{
			if (!CurrentlyMeasuring)
				return null;
			var previousSize = 0L;
			var prev = _previousMeasurement;
			if (prev != null)
				previousSize = prev.FinalSize;
	...
	public class Measurement
	{
		...
		public long FinalSize
		{
			...
			{
				if (_end != null)
					return _end.privateBytesKb;
				else if (_start != null)
					return _start.privateBytesKb;
				else
					return 0L;
			}
		}

		public Measurement(string action, string details, long previousSize)
		{
			_action = action;
			_details = details;
			_previousSizeKb = previousSize;
		...
		public override string ToString()
		{
			...
			bldr.AppendFormat("Measurement: details=\"{0}\"; start={1}KB ({2}); end={3}KB ({4})", _details, _start.privateBytesKb, _start.when,
				_end?.privateBytesKb, _end?.when);
