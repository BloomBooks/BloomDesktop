    ...
    static class Program
    {
        ...
        static int Main(string[] args1)
        {
            ...
            try
            {
                ...
                if (Settings.Default.NeedUpgrade)
                {
                    ...
                    // A folder named with --user-settings-folder holds exactly the settings its
        ...
        internal static string[] ParseStartupPortArguments(string[] args, out string errorMessage)
        {
            ...
            for (var i = 0; i < args.Length; i++)
            {
                if (
                    ...
                    || TryHandleUserSettingsFolderArgument(args, ref i, out errorMessage)
                )
                {
        ...
        static void HandleProjectWindowClosed(object sender, EventArgs e)
        {
            ...
            if (((Shell)sender).UserWantsToOpenADifferentProject)
            {
                ...
                // (AI suggests that using BeginInvoke might be more reliable, but we're closing
                // all the controls we could invoke on. There shouldn't be anything competing
                // for idle time.)
