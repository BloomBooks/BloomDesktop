...
/*					if (!GrabTokenForThisProject(projectPath))
		...
		private static string GetDesiredUiLanguage(string installedStringFileFolder)
		{
			...
			{
				var uiLangs = LocalizationManager.GetUILanguages(true).ToArray();
				if (uiLangs.Length == 0)
				{
					// LocalizationManager.GetUILanguages(true) doesn't work before calling LocalizationManager.Create(),
					// so create a LocalizationManager that we know will succeed without bothering the user.
					var lmEn = LocalizationManager.Create("en", "Bloom", "Bloom", Application.ProductVersion,
						installedStringFileFolder, "SIL/Bloom", Resources.BloomIcon, "issues@bloomlibrary.org",
						//the parameters that follow are namespace beginnings:
						"Bloom");
					uiLangs = LocalizationManager.GetUILanguages(true).ToArray();
				}
				...
				foreach  (var lang in uiLangs)
				{
					if ((lang as CultureInfo).IetfLanguageTag == UserInterfaceCulture.IetfLanguageTag)
