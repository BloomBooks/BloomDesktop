...
namespace Bloom.CollectionCreating
{
        ...
        : SIL.Windows.Forms.Miscellaneous.FormForUsingPortableClipboard
    {
        ...
        )
        {
            ...
            _collectionInfo.Language1.IsRightToLeft = IsRtl;
            _fontDetails.RightToLeft = IsRtl;
        ...
        private void SetLocalizedStrings()
        {
            Text = LocalizationManager.GetString(
                "NewCollectionWizard.NewCollectionWindowTitle",
                "Create New Bloom Collection"
            );
            _welcomePage.Text = LocalizationManager.GetString(
                "NewCollectionWizard.WelcomePage",
                "Welcome To Bloom!"
            );
            ...
            _languageFontPage.Text = LocalizationManager.GetString(
                "NewCollectionWizard.FontAndScriptPage",
                "Font and Script"
            );
            _vernacularLanguagePage.Text = LocalizationManager.GetString(
                "NewCollectionWizard.ChooseLanguagePage",
                "Choose the main language for this collection."
            );
            _finishPage.Text = LocalizationManager.GetString(
                "NewCollectionWizard.FinishPage",
                "Ready To Create New Collection"
            );
            _wizardControl.NextButtonText = LocalizationManager.GetString(
                "Common.Next",
                "&Next",
                "Used for the Next button in wizards, like that used for making a New Collection"
            );
            _wizardControl.FinishButtonText = LocalizationManager.GetString(
                "Common.Finish",
                "&Finish",
            ...
            _wizardControl.CancelButtonText = LocalizationManager.GetString(
                "Common.CancelButton",
                "&Cancel"
            );

            var one = LocalizationManager.GetString(
                "NewCollectionWizard.WelcomePage.WelcomeLine1",
                "You are almost ready to start making books."
            );
            var two = LocalizationManager.GetString(
                "NewCollectionWizard.WelcomePage.WelcomeLine2.V2",
                "In order to keep things simple and organized, Bloom keeps all the books you make in one or more 'Collections'. The first thing we need to do is make one for you."
            );
            var three = LocalizationManager.GetString(
                "NewCollectionWizard.WelcomePage.WelcomeLine3",
        ...
        private bool DefaultCollectionPathWouldHaveProblems
        {
            get
            {
                try
                {
                    return Path.GetFileName(_collectionInfo.PathToSettingsFile)
        ...
        private void _finishPage_Initialize(object sender, EventArgs e)
        {
            var format = LocalizationManager.GetString(
                "NewCollectionWizard.FinishPageContent",
                "OK, that's all we need to get started with your new '{0}' collection.\r\nClick on the 'Finish' button.",
                "{0} is the name of the new collection"
            );
            _finalMessage.Text = String.Format(
                format,
                Path.GetFileNameWithoutExtension(_collectionInfo.PathToSettingsFile)
            );
        }
    }
