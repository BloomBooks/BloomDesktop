    ...
    public class UpdateVersionTable
    {
        ...
        public UpdateTableLookupResult LookupURLOfUpdate(bool forceReload = false)
        {
            ...
            try
            {
                ...
                foreach (
                    var line in TextContentsOfTable.Split(
                        new[] { '\n' },
                ...
                )
                {
                    ...
                    {
                        Logger.WriteEvent(
                            "***Error: UpdateVersionTable could not parse line {0} of this updateTableContent:",
                            line
                        );
                        LogTableContents();
                ...
                parsingErrorMsg = string.Format(
                    "{0} contains no record for this version of Bloom",
                    GetUrlOfTable()
                );
            }
            ...
            catch (ArgumentException e)
            {
                ...
                // Put a message in the log and don't upgrade (and return a message that will get into a 'toast')
                parsingErrorMsg =
                    "Could not parse a version number in the UpdateVersionTable" + e.Message;
                Logger.WriteEvent(parsingErrorMsg);
            }
        ...
        /// <returns></returns>
        internal bool CanGetVersionTableFromWeb(
            IBloomWebClient client,
            out UpdateTableLookupResult errorResult
        )
        {
            errorResult = null;
            try
            {
                ...
                Logger.WriteEvent(
                    "UpdateVersionTable looking for UpdateVersionTable URL: " + GetUrlOfTable()
                );
        ...
        private void LogTableContents()
        {
            ...
            Logger.WriteEvent(
                "***UpdateVersionTable contents are " + Environment.NewLine + safeContents
            );
        }
