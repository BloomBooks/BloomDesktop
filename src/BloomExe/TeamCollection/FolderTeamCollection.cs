...
namespace Bloom.TeamCollection
{
    ...
    public class FolderTeamCollection : TeamCollection
    {
        ...
        )
        {
            ...
            else
            {
                ...
                if (RobustFile.Exists(bookPath))
                {
                    ...
                    // Even though it's not shared, I'd prefer not to pollute the shared folder
                    // with a temp folder.
                    ...
                    pathToWrite = AvailablePath(
                        bookFolderName,
                        GetTempFolder(bookDirectoryPath),
                        ".tmp"
                    );
                }
            }
        ...
        string GetTempFolder(string sharedBookFolder)
        {
        ...
        {
            // Ensure the path is properly formatted for PowerShell and encapsulated in single quotes
            string powerShellPath = $"'{fullPath}'";
            string command =
                $"Set-Content -Path {powerShellPath} -Stream com.dropbox.ignored -Value 1";

            // Use Process to run the PowerShell command
