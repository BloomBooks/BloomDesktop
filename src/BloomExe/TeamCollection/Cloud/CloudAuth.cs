...
namespace Bloom.TeamCollection.Cloud
{
    ...
    public class CloudSession
    {
        ...
        /// means "this provider doesn't track the concept" (the local provider's accounts are all
        /// auto-confirmed, so it always sets true -- see LocalCloudAuthProvider.ToSession).
        /// </summary>
    ...
    /// file): reports enough that the client can decide whether to show a plain local-mode
    /// email/password form (AuthMode == "local") instead of a real sign-in browser flow, plus the
    ...
    /// implementations: <see cref="LocalCloudAuthProvider"/> (local GoTrue) and
    ...
    public interface ICloudAuthProvider
    {
        ...
        /// this, so the local password-based provider and any test doubles predating this method
    ...
    /// is process-lifetime only (used by the local provider and by tests); the persistent,
    ...
    public class CloudAuth : IDisposable
    {
        ...
        public static CloudAuth CreateInitialized(CloudEnvironment environment)
        {
            ...
            // every sign-in/refresh re-saves it. Harmless for the auto-sign-in env-override path,
            // which re-signs from BLOOM_CLOUDTC_USER before ever consulting the store (see
            // InitializeAtStartup).
        ...
        public static ICloudAuthProvider CreateProvider(CloudEnvironment environment)
        {
            switch (environment.AuthMode)
            {
                ...
                case CloudAuthMode.Local:
                default:
                    return new LocalCloudAuthProvider(environment);
            }
        }
        ...
        /// <summary>
        /// Explicit sign-in (e.g. the user submitted the local-mode email/password form). Throws
        ...
        {
            if (!string.IsNullOrEmpty(environment.AutoSignInEmail))
            {
                try
                {
                    SignIn(
                        environment.AutoSignInEmail,
                        environment.AutoSignInPassword ?? string.Empty
                    );
                }
                catch (Exception e)
                {
        ...
        public CloudLoginState GetLoginState(CloudEnvironment environment) =>
            new CloudLoginState
            {
                AuthMode = environment.AuthMode == CloudAuthMode.Local ? "local" : "cloud",
                SignedIn = IsSignedIn,
                Email = CurrentEmail,
