...
{
    /// <summary>
    /// Which backend a Cloud Team Collection authenticates against. "Local" talks to the local
    /// GoTrue instance bundled with the local Supabase stack (accepts any email/password);
    /// "Cloud" is the real BloomLibrary/Firebase sign-in (Option A, decided 8 Jul 2026 -- see
    /// Design/CloudTeamCollections.md and GOING-LIVE.md Phase 3). ("Local" here means the
    /// on-this-machine emulation, never the hosted dev/sandbox test cloud, which uses "Cloud".)
    /// This is the BACKEND sense of "local"; it is unrelated to a collection's local working copy
    /// — the this-machine's-copy-on-disk sense used by e.g. CloudCachedBook.LocalVersionSeq.
    /// The string value ("local"/"cloud") is also what travels over the wire in
    /// CloudLoginState/sharing/loginState, and must keep matching BloomBrowserUI's
    /// SharingLoginMode type in sharingApi.ts.
    /// </summary>
    ...
    /// point at the local stack. Nothing else in Bloom should read these environment
    /// variables directly; switching local &lt;-&gt; sandbox &lt;-&gt; production is a matter of
    /// setting environment variables, not changing code.
    /// </summary>
    public class CloudEnvironment
    {
        // Compiled defaults match the "Local value" column of server/dev/README.md's environment
        // variable table, so a plain checkout of Bloom talks to the local stack with no
        ...
        private const CloudAuthMode DefaultAuthMode = CloudAuthMode.Local;
        ...
        /// the local stack (MinIO) sets BLOOM_CLOUDTC_S3_ENDPOINT explicitly.
        /// </summary>
        ...
        /// </summary>
        public string AutoSignInEmail { get; }

        /// <summary>The password to pair with <see cref="AutoSignInEmail"/>, from BLOOM_CLOUDTC_PASSWORD.</summary>
        public string AutoSignInPassword { get; }
        ...
        public CloudEnvironment(Func<string, string> getEnvironmentVariable)
        {
            ...
            // Real AWS never sets this override; only the local stack (MinIO) does.
            S3ForcePathStyle = !string.IsNullOrEmpty(S3Endpoint);

            var authModeRaw = Get(
                "BLOOM_CLOUDTC_AUTH_MODE",
                DefaultAuthMode == CloudAuthMode.Local ? "local" : "cloud"
            );
            AuthMode = string.Equals(authModeRaw, "cloud", StringComparison.OrdinalIgnoreCase)
                ? CloudAuthMode.Cloud
                : CloudAuthMode.Local;

            AutoSignInEmail = Get("BLOOM_CLOUDTC_USER", null);
            AutoSignInPassword = Get("BLOOM_CLOUDTC_PASSWORD", null);
