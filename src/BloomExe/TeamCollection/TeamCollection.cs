...
namespace Bloom.TeamCollection
{
	...
	public abstract class TeamCollection: IDisposable
	{
		...
		public void SyncLocalAndRepoCollectionFiles(bool atStartup = true)
		{
			...
			var savedLocalModTime = LocalCollectionFilesSyncTime();
			if (repoModTime > savedLocalModTime)
			{
		...
		internal bool LocalCollectionFilesUpdated()
		{
			...
			var savedModTime = LocalCollectionFilesSyncTime();
		...
		internal DateTime LocalCollectionFilesSyncTime()
		{
			var path = GetCollectionFileSyncLocation();
			if (!File.Exists(path))
		...
		public static List<string> RootLevelCollectionFilesIn(string folder, string collectionNameOrNull = null)
		{
			...
			if (File.Exists(Path.Combine(folder, "customCollectionStyles.css")))
				files.Add("customCollectionStyles.css");
			foreach (var path in Directory.EnumerateFiles(folder, "ReaderTools*.json"))
			{
				files.Add(Path.GetFileName(path));
			}
			if (File.Exists(Path.Combine(folder, "configuration.txt")))
				files.Add("configuration.txt");
			return files;
		}
		...
		/// <param name="internalFolderForAllowedWords">Used only for testing, real callers pass null</param>
