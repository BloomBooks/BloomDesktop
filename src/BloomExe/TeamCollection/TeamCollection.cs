...
namespace Bloom.TeamCollection
{
	...
	public abstract class TeamCollection: IDisposable
	{
		...
		private const string kWebSocketContext = "teamCollectionMerge"; // must match what is in TODO
		...
		public void SynchronizeSharedAndLocal()
		{
			...
			// Can add this if we want a bigger title in the content of the dialog (changes in ProgressDialog, too).
			...
			using (var dlg = new BrowserDialog(url))
			{
				...
				// one in HTML.)
				dlg.Text = "  ";
				//dlg.Text = "Team Collection Activity";
				// None...no border anywhere
				// FixedSingle...full title bar with menu, maximize, minimize
				// dlg.FormBorderStyle = System.Windows.Forms.FormBorderStyle.FixedSingle;
				// We need to do the main work in a background thread so the UI thread can run the dialog.
