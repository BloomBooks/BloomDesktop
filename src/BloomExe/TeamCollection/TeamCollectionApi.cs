	...
	public class TeamCollectionApi
	{
		...
		public void HandleRepoFolderPath(ApiRequest request)
		{
			try
			{
				...
				request.ReplyWithText(_tcManager.CurrentCollectionEvenIfDisabled?.RepoDescription ?? "");
			}
			catch (Exception e)
			{
		...
		public void HandleIsTeamCollectionEnabled(ApiRequest request)
		{
			try
			{
				...
				request.ReplyWithBoolean(_tcManager.CurrentCollectionEvenIfDisabled != null &&
		...
		public void HandleCurrentBookStatus(ApiRequest request)
		{
			try
			{
				...
				var whoHasBookLocked = _tcManager.CurrentCollectionEvenIfDisabled?.WhoHasBookLocked(BookFolderName);
				// It's debatable whether to use CurrentCollectionEvenIfDisabled everywhere. For now, I've only changed
				...
				request.ReplyWithJson(JsonConvert.SerializeObject(
					new
					{
						...
						where = _tcManager.CurrentCollectionEvenIfDisabled?.WhatComputerHasBookLocked(BookFolderName),
						...
						disconnected = _tcManager.CurrentCollectionEvenIfDisabled?.IsDisconnected
					}));
			}
			catch (Exception e)
			{
		...
		public bool CanEditBook()
		{
			...
			if (_tcManager.CurrentCollectionEvenIfDisabled == null)
			{
				return true; // no team collection, no problem.
			}

			return _tcManager.CurrentCollectionEvenIfDisabled.IsCheckedOutHereBy(_tcManager.CurrentCollectionEvenIfDisabled.GetStatus(folderName));
		}
	}
}
