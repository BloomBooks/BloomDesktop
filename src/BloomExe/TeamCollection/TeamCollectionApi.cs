...
namespace Bloom.TeamCollection
{
	...
	public class TeamCollectionApi
	{
		...
		internal TeamCollectionMessage MakeLockFailedMessageFromException(Exception e, string bookFolder)
		{
			...
			var param1 = e.Message;
			if (e is FolderTeamCollection.CannotLockException cannotLockException)
			{
				if (cannotLockException.SyncAgent != "Unknown")
				{
					msgId = "TeamCollection.BookFileLockedDropbox";
					param1 = cannotLockException.SyncAgent;
					msgEnglish =
						"Bloom was not able to check out \"{0}\" because some other program is busy with it. This may just be {1} synchronizing the file. Please try again later. If the problem continues, restart your computer.";
				}
				else
				{
					msgId = "TeamCollection.BookFileLockedUnknown";
					msgEnglish =
						"Bloom was not able to check out \"{0}\" because some other program is busy with it. This may just be something synchronizing the file. Please try again later. If the problem continues, restart your computer.";
				}
			}

			var msg = new TeamCollectionMessage(MessageAndMilestoneType.Error, msgId, msgEnglish,
				Path.GetFileName(bookFolder), param1);
			return msg;
		}
