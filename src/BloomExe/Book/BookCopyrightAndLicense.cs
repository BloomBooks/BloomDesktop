	...
	public class BookCopyrightAndLicense
	{
		...
		private static void UpdateBookLicenseIcon(Metadata metadata, string bookFolderPath)
		{
			...
			if (RobustFile.Exists(imagePath) && BloomFileLocator.IsInstalledFileOrDirectory(imagePath))
				return;
			FileAttributes originalFileAttributes = FileAttributes.Normal;
			try
			{
				...
				{
					originalFileAttributes= RobustFile.GetAttributes(imagePath);
					var originalReadOnly = (originalFileAttributes & FileAttributes.ReadOnly);
					if ((originalFileAttributes & FileAttributes.ReadOnly) == FileAttributes.ReadOnly)
			...
			catch(Exception error)
			{
				...
				if (RobustFile.Exists(imagePath))
				{
					...
					bldr.AppendLine($"You may find help for this problem at https://community.software.sil.org/c/bloom/5. [TODO: create page and finish link]");
					bldr.AppendLine($"The following information may be helpful.");
					bldr.AppendLine($"{imagePath} original ReadOnly attribute was {(originalFileAttributes & FileAttributes.ReadOnly) == FileAttributes.ReadOnly}");
		...
		private static string GetOriginalCopyrightSentence(IEnumerable<string> languagePriorityIds,
