	...
	public class BookStorage : IBookStorage
	{
		...
		public void CleanupUnusedAudioFiles()
		{
			...
			if (Directory.Exists(audioFolderPath))
			{
				...
				audioFilesToDeleteIfNotUsed = new HashSet<string>(Directory.EnumerateFiles(audioFolderPath).Where(AudioProcessor.IsAudioFile)
			...
			foreach (var fileName in usedAudioFileNames)
				audioFilesToDeleteIfNotUsed.Remove(fileName);  //This call just returns false if not found, which is fine.
		...
		public IEnumerable<string> GetBackgroundMusicFileNamesReferencedInBook()
		{
			return GetAudioSourceIdentifiers(HtmlDom.SelectChildBackgroundMusicElements(Dom.RawDom.DocumentElement))
				.Where(AudioProcessor.IsBackgroundMusicAudioFile)
