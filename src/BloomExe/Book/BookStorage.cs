...
namespace Bloom.Book
{
	...
	{
		public delegate BookStorage Factory(string folderPath, bool fixBookCompletely = false);//autofac uses this
		...
		public BookStorage(string folderPath, bool fixBookCompletely, IChangeableFileLocator baseFileLocator,
						   BookRenamedEvent bookRenamedEvent, CollectionSettings collectionSettings)
		{
			...
			ExpensiveInitialization(fixBookCompletely);
		}
...
//				    }
		...
		/// </summary>
		/// <param name="fixBookCompletely">flag whether to do everything possible in preparation for editing or uploading</param>
		private void ExpensiveInitialization(bool fixBookCompletely = false)
		{
			...
			// ALternatively, if we want to fully fix the book and we have a backup file, then we'll use that to proceed.
			...
			if (!RobustFile.Exists(pathToExistingHtml) && (!fixBookCompletely || !RobustFile.Exists(backupPath)))
			{
			...
			else
			{
				...
				catch (Exception error)
				{
					...
					if (fixBookCompletely && TryGetValidXmlDomFromHtmlFile(backupPath, out xmlDomFromHtmlFile))
					{
				...
				if (RobustFile.Exists(brandingPath))
				{
					...
					catch (System.UnauthorizedAccessException error)
					{
						...
						Logger.WriteEvent("*** ERROR cannot delete old branding.css file in ExpensiveInitialization(" + fixBookCompletely + ")");
						Logger.WriteEvent("*** ERROR: " + error.Message.Replace("{", "{{").Replace("}", "}}"));
						return;
					}
				}
				...
				if (fixBookCompletely && !string.IsNullOrEmpty(InitialLoadErrors))
				{
				...
				// probably not needed at runtime if !fixBookCompletely, but one unit test relies on it having been done, and is very fast, so ok.
				Dom.UpdatePageDivs();
