...
namespace Bloom.Book
{
	...
	public class BookStorage : IBookStorage
	{
		...
		public void CleanupUnusedVideoFiles()
		{
			...
			var videoExtensions = new HashSet<string>(new[] { ".mp4" });  // .mov, .avi, .orig...?

			if (Directory.Exists(videoFolderPath))
			{
				foreach (var path in Directory.EnumerateFiles(videoFolderPath).Where(
					s => videoExtensions.Contains(Path.GetExtension(s).ToLowerInvariant())))
				{
			...
			foreach (var relativeFilePath in usedVideoPaths) // relativeFilePath includes "video/"
			{
				...
				{
					if (videoExtensions.Contains(Path.GetExtension(relativeFilePath).ToLowerInvariant()))
					{
				...
				{
					var tempfileName = Path.ChangeExtension(relativeFilePath, ext);
