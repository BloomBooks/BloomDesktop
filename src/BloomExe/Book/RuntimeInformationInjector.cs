...
//		}
		...
		private static void AddLabelTranslations(XmlElement root, Dictionary<string, string> dictionary)
		{
			...
			foreach (XmlElement label in labels)
			{
				...
				// Copied from AddTranslationToDictionaryUsingEnglishAsKey. I'm not sure whether we need it here.
				var translation = _collectDynamicStrings
					? LocalizationManager.GetDynamicString("Bloom", key, label.InnerText)
					: LocalizationManager.GetString(key, label.InnerText);
