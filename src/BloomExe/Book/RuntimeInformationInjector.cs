...
//		}
		...
		public static void AddUISettingsToDom(HtmlDom pageDom, BookData bookData, IFileLocator fileLocator)
		{
			...
			d.Add("languageForNewTextBoxes", bookData.GetLanguage(LanguageSlot.Language1).Iso639Code);
			...
			if (!String.IsNullOrEmpty(language2.Iso639Code))
			{
				d.Add("currentCollectionLanguage2", language2.Iso639Code);
			}
			var language3 = bookData.GetLanguage(LanguageSlot.Language3);
			if (!String.IsNullOrEmpty(language3.Iso639Code))
			{
				d.Add("currentCollectionLanguage3", language3.Iso639Code);
			}
