...
namespace Bloom.Book
{
	...
	public class BookInfo
	{
		...
		/// - meta.json
		/// - bookName.bloombookorder (if present)
		...
		public static void RepairDuplicateInstanceIds(string pathToDirectory)
		{
			...
			var setOfIds = new Dictionary<string, SortedList<DateTime, string>>();
			var currentFolder = pathToDirectory;

			GatherInstanceIdsRecursively(currentFolder, setOfIds);
			...
			var problemIds = setOfIds.Keys.Where(key => setOfIds[key].Count > 1);
			foreach (var problemId in problemIds)
			{
				var listToFix = setOfIds[problemId];
				var countToFix = listToFix.Keys.Count - 1;
				Logger.WriteEvent($"***Fixing {countToFix} duplicate ids for: {problemId}");

				var keysExceptFirst = listToFix.Keys.Skip(1);
				foreach (var dateTimeKey in keysExceptFirst)
				{
					InstallFreshInstanceGuid(listToFix[dateTimeKey]);
				}
			}
		}

		private static void GatherInstanceIdsRecursively(string currentFolder, Dictionary<string, SortedList<DateTime, string>> setOfIds)
		{
			...
			if (!File.Exists(metaJsonPath))
			{
				...
				{
					GatherInstanceIdsRecursively(subDirectory, setOfIds);
				}
				return;
			}
			...
			var id = bi.Id;
			SafelyAddToIdSet(id, metaFileLastWriteTime, currentFolder, setOfIds);
		}

		private static void SafelyAddToIdSet(string bookId, DateTime lastWriteTime, string bookFolder,
			IDictionary<string, SortedList<DateTime, string>> setOfIds)
		{
			if (setOfIds.ContainsKey(bookId))
			{
				var list = setOfIds[bookId];
				...
				var smallTime = new TimeSpan(100);
				while (true)
				{
					if (list.ContainsKey(lastWriteTime))
					{
						// not good
						lastWriteTime += smallTime;
					}
					else break;
				}
				list.Add(lastWriteTime, bookFolder);
			}
			else
			{
				var list = new SortedList<DateTime, string> { { lastWriteTime, bookFolder } };
				setOfIds.Add(bookId, list);
			}
		}
	}
