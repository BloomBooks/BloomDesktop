...
public class AppearanceSettings
{
    ...
    public string AppearanceUIOptions
    {
        get
        {
            ...
            x["firstPossiblyLegacyCss"] = GetOffendingFileNameIfItReallyIs();
    ...
    /// this method.
    /// </summary>
    /// <returns>base filename of offending CSS file, or null if it either doesn't exist is isn't a problem</returns>
    private string GetOffendingFileNameIfItReallyIs()
    {
        ...
        if (offendingFile == "customBookStyles2.css")
            return offendingFile;   // not something that got migrated
        ...
        {
            // We have a file that looks like it should have been migrated, so we don't want to complain about it.
            // Do a few more checks to make sure we don't want to complain.
            // REVIEW: are these checks all needed? or all valid?
            var folder = Path.GetDirectoryName(FirstPossiblyOffendingCssFile);
            var css2Path = Path.Combine(folder, "customBookStyles2.css");
            if (RobustFile.Exists(css2Path) && CssThemeName != "default")
            {
                // we seem to have a migration, so assume this file is not a problem.
                // (We want to keep it around for backwards compatibility with earlier versions of Bloom.)
                return null;
            }
        }
