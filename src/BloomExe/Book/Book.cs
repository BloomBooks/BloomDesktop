    ...
    public class Book
    {
        ...
        public void BringXmatterHtmlUpToDate(HtmlDom bookDOM)
        {
            ...
            var customLayoutIds = bookDOM
                ...
                .ToHashSet();
            var hasCustomLayoutIds = bookDOM
                .SafeSelectNodes(
                    "//div[contains(@class, 'bloom-page') and @data-custom-layout-id and contains(concat(' ', normalize-space(@class), ' '), ' bloom-hasCustomLayout ')]"
                )
                .Cast<SafeXmlElement>()
                .Select(page => page.GetAttribute("data-custom-layout-id"))
                .Where(id => !string.IsNullOrEmpty(id))
            ...
            )
            {
                ...
                if (hasCustomLayoutIds.Contains(customLayoutId))
                    page.AddClass("bloom-hasCustomLayout");
            }
