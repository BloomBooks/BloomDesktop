...
namespace Bloom.Book
{
    public class Book
    {
        ...
        )
        {
            ...
            if (ShouldForceFullSaveForXmatterBackgroundAudio(pageFromEditedDom))
        ...
        private static bool ShouldForceFullSaveForXmatterBackgroundAudio(
            SafeXmlElement pageFromEditedDom
        )
        {
            // xmatter pages can be regenerated from bloomDataDiv, so keep audio attrs there in sync.
            if (string.IsNullOrEmpty(pageFromEditedDom.GetAttribute("data-xmatter-page")))
                return false;

            return !string.IsNullOrEmpty(pageFromEditedDom.GetAttribute(HtmlDom.musicAttrName))
                || !string.IsNullOrEmpty(pageFromEditedDom.GetAttribute(HtmlDom.musicVolumeName));
        }
