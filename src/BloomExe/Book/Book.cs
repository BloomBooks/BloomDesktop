...
//                BookStorage.HideAllTextAreasThatShouldNotShow(dom, iso639CodeToLeaveVisible, Page.GetPageSelectorXPath(dom));
		...
		/// <summary>
		/// At one point in v4.1, Question pages were able to be recorded with the Talking Book tool,
		/// which embedded tons of span elements which messed up BR display. New question pages have classes
		...
		private void RepairQuestionsPages(HtmlDom bookDOM)
		{
			...
			const string classesToAdd = "bloom-userCannotModifyStyles bloom-noAudio";
			...
			foreach (XmlElement quizContentsElement in questionNodes)
			{
				...
				{
					HtmlDom.AddClassIfMissing(quizContentsElement, classesToAdd);
					HtmlDom.StripUnusedTagsPreservingText(bookDOM.RawDom, quizContentsElement, new []{ "div", "p", "br" });
				}
			}
		}
