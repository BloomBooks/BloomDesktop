...
//                BookStorage.HideAllTextAreasThatShouldNotShow(dom, iso639CodeToLeaveVisible, Page.GetPageSelectorXPath(dom));
		...
		internal string GetBestPHashImageSource()
		{
			...
			var deprecatedFirstContentImage = OurHtmlDom.SelectSingleNode(firstContentImageContainerPath);
			if (deprecatedFirstContentImage != null)
			{
				var src = HtmlDom.GetImageElementUrl(deprecatedFirstContentImage).UrlEncoded;
				return src;
			}
			...
			var deprecatedCoverImg = OurHtmlDom.SelectSingleNode(coverImageContainerPath);
			if (deprecatedCoverImg != null)
			{
				var src = HtmlDom.GetImageElementUrl(deprecatedCoverImg).UrlEncoded;
				return src;
			}
