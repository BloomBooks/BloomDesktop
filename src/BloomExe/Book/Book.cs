...
namespace Bloom.Book
{
	public class Book
	{
		...
		{
			var display = title.GetExactAlternative(bookData.GetLanguage(LanguageSlot.Language1).Iso639Code);
			if (string.IsNullOrEmpty(display))
			{
				...
				else
				{
					...
					//already checked for this, previously. orderedPreferences.Add(<Language1.Iso639Code>);
					var isoCode2 = bookData.GetLanguage(LanguageSlot.Language2).Iso639Code;
					if (!String.IsNullOrEmpty(isoCode2))
						orderedPreferences.Add(isoCode2);
					var isoCode3 = bookData.GetLanguage(LanguageSlot.Language3).Iso639Code;
					if (!String.IsNullOrEmpty(isoCode3))
						orderedPreferences.Add(isoCode3);
...
//			dom.AddJavascriptFile("lib/jquery.qtipSecondary.js".ToLocalhost());
		...
		{
			TranslationGroupManager.UpdateContentLanguageClasses(dom.RawDom, _bookData, GetLanguage(LanguageSlot.Language1).Iso639Code,
				_bookData.MultilingualContentLanguage2, _bookData.MultilingualContentLanguage3);
			BookInfo.IsRtl = GetLanguage(LanguageSlot.Language1).IsRightToLeft;
...
//                BookStorage.HideAllTextAreasThatShouldNotShow(dom, iso639CodeToLeaveVisible, Page.GetPageSelectorXPath(dom));
		...
		public virtual HtmlDom GetPreviewHtmlFileForWholeBook()
		{
			...
			var primaryLanguage = GetLanguage(LanguageSlot.Language1).Iso639Code;
			if (IsShellOrTemplate)
			{
				...
				if (previewDom.SelectSingleNode($"//*[@lang='{primaryLanguage}' and contains(@class, 'bloom-editable') and text()!='']") == null)
					primaryLanguage = GetLanguage(LanguageSlot.Language2).Iso639Code;
			}
		...
		public void BringBookUpToDate(IProgress progress, bool forCopyOfUpToDateBook = false)
		{
			...
			{
				SHRP_TeachersGuideExtension.UpdateBook(OurHtmlDom, GetLanguage(LanguageSlot.Language1).Iso639Code);
			}
		...
		{
			bookDom.Body.SetAttribute("data-L1", this.GetLanguage(LanguageSlot.Language1).Iso639Code);
			bookDom.Body.SetAttribute("data-L2", this.GetLanguage(LanguageSlot.Language2).Iso639Code);
			bookDom.Body.SetAttribute("data-L3", this.GetLanguage(LanguageSlot.Language3).Iso639Code );
		}
		...
		private void BringBookUpToDateUnprotected(HtmlDom bookDOM, IProgress progress)
		{
			...
			bookDOM.UpdatePageNumberAndSideClassOfPages(CollectionSettings.CharactersForDigitsForPageNumbers, GetLanguage(LanguageSlot.Language1).IsRightToLeft);

			UpdateTextsNewlyChangedToRequiresParagraph(bookDOM);

			UpdateCharacterStyleMarkup(bookDOM);

			bookDOM.SetImageAltAttrsFromDescriptions(GetLanguage(LanguageSlot.Language1).Iso639Code);
		...
		private void CreateOrUpdateDefaultLangStyles()
		{
			...
			if (doesAlreadyExist)
			{
				...
				cssLangs.Add(GetLanguage(LanguageSlot.Language1).Iso639Code);
				cssLangs.Add(GetLanguage(LanguageSlot.Language2).Iso639Code);
				if (!String.IsNullOrEmpty(GetLanguage(LanguageSlot.Language3).Iso639Code))
					cssLangs.Add(GetLanguage(LanguageSlot.Language3).Iso639Code);
		...
		public void BringXmatterHtmlUpToDate(HtmlDom bookDOM)
		{
			...
			helper.InjectXMatter(_bookData.GetWritingSystemCodes(), layout, BookInfo.UseDeviceXMatter, GetLanguage(LanguageSlot.Language2).Iso639Code);
...
//at version 0.9.71, we introduced this book lineage for real. At that point almost all books were from Basic book,
		...
		public bool HasSourceTranslations
		{
			get
			{
				...
				var x = OurHtmlDom.SafeSelectNodes($"//*[contains(@class,'bloom-translationGroup')]//textarea[@lang and @lang!='{GetLanguage(LanguageSlot.Language1).Iso639Code}']");
				return x.Count > 0;
			}
...
//        }
		...
		public IEnumerable<string> ActiveLanguages
		{
			get
			{
				var result = new HashSet<string>(new [] {GetLanguage(LanguageSlot.Language1).Iso639Code});
		...
		{
			var languagesOfBook = GetLanguage(LanguageSlot.Language1).Name;

			if (MultilingualContentLanguage2 != null)
			{
				languagesOfBook += ", " + ((MultilingualContentLanguage2 == GetLanguage(LanguageSlot.Language2).Iso639Code) ?
					GetLanguage(LanguageSlot.Language2).Name :
					GetLanguage(LanguageSlot.Language3).Name);
			}
			if (MultilingualContentLanguage3 != null)
			{
				languagesOfBook += ", " + GetLanguage(LanguageSlot.Language3).Name;
			}
		...
		public Dictionary<string, bool> AllPublishableLanguages(bool includeLangsOccurringOnlyInXmatter = false)
		{
			...
			{
				result.Clear();
				result[GetLanguage(LanguageSlot.Language1).Iso639Code] = true;
			}
			return result;
		}
		...
		private bool IsLanguageWanted(XmlElement parent, string lang)
		{
			...
			foreach (var defaultLang in dataDefaultLanguages)
			{
				...
				case "V":
				case "L1":
					if (lang == GetLanguage(LanguageSlot.Language1).Iso639Code)
						return true;
					break;
				case "N1":
				case "L2":
					if (lang == GetLanguage(LanguageSlot.Language2).Iso639Code)
						return true;
					break;
				case "N2":
				case "L3":
					if (lang == GetLanguage(LanguageSlot.Language3).Iso639Code)
						return true;
					break;
				}
			}
...
//            var colorValue = String.Format("#{0:X2}{1:X2}{2:X2}", coverColor.R, coverColor.G, coverColor.B);
		...
		public void DeletePage(IPage page)
		{
			...
			Storage.Dom.UpdatePageNumberAndSideClassOfPages(CollectionSettings.CharactersForDigitsForPageNumbers, GetLanguage(LanguageSlot.Language1).IsRightToLeft);
		...
		private void OrderOrNumberOfPagesChanged()
		{
			OurHtmlDom.UpdatePageNumberAndSideClassOfPages(CollectionSettings.CharactersForDigitsForPageNumbers,
				GetLanguage(LanguageSlot.Language1).IsRightToLeft);
		}
		...
		public void SavePage(HtmlDom editedPageDom, bool needToDoFullSave = true)
		{
			...
			try
			{
				...
				HtmlDom.SetImageAltAttrsFromDescriptions(pageFromStorage, GetLanguage(LanguageSlot.Language1).Iso639Code);
...
//        }
		...
		{
			var language1Iso639Code = GetLanguage(LanguageSlot.Language1).Iso639Code;
		...
		{
			var languageName = GetLanguage(LanguageSlot.Language1).GetNameInLanguage(inLanguage);
