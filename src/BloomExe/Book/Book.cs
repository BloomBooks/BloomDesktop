    ...
    public class Book
    {
        ...
        public void BringXmatterHtmlUpToDate(HtmlDom bookDOM)
        {
            ...
            frontCover =
                RawDom.SelectSingleNode("/html/body/div[contains(@class, 'outsideFrontCover')]")
                as SafeXmlElement;
            var frontCoverHasCustomMarginBox =
                frontCover
                    ?.SafeSelectNodes(
                        ".//div[contains(concat(' ', normalize-space(@class), ' '), ' bloom-customMarginBox ')]"
                    )
                    .Any() ?? false;
            // I think I've changed the pug so that all our xmatters now have a custom margin box element,
            // but just in case there are some that don't, we don't want to hide the ordinary margin box
            // if there somehow isn't a custom one to show instead.
            if (frontCoverWasCustom && frontCoverHasCustomMarginBox)
            {
