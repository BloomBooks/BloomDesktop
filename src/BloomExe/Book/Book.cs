		...
		public string ComputePHashOfFirstContentImage()
		{
			...
			for (int num = 1; num <= max; ++num)
			{
				...
				if (firstContentImageElement != null)
				{
					...
					// Save the hash with the page number where the image was found.  The specific page presumably has some bearing
					// on whether the book is the same as another book that has the same image.
					if (hash != null)
			...
			if (coverImg != null)
			{
				...
				var hash = ComputePHashOfImageFile(src);
				// Save the hash with a flag that the image is from the front cover.
				if (hash != null)
