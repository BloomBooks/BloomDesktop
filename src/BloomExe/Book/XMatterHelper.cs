...
namespace Bloom.Book
{
	...
	public class XMatterHelper
	{
		...
		/// <param name="useDeviceOverride">If true, use a pack-specific device xmatter, or "Device" if none found. E.g. ABC => ABC-Device</param>
		public XMatterHelper(HtmlDom bookDom, string xmatterNameFromCollectionSettings, IFileLocator fileLocator, bool useDeviceOverride = false)
		{
			...
			if (!String.IsNullOrWhiteSpace(bookSpecificXMatterPack))
			{
				...
				if (useDeviceOverride)
					_nameOfXMatterPack = GetDeviceXMatterOverride(_nameOfXMatterPack, fileLocator);
		...
		public static string GetDeviceXMatterOverride(string xmatterName, IFileLocator fileLocator)
		{
