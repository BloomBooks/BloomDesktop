...
namespace Bloom.Book
{
	...
	public class XMatterHelper
	{
		...
		{
			_bookDom = bookDom;
			var specifiedXMatterPack = GetXMatterFromBook(bookDom);
			string directoryPath = null;
			if (!String.IsNullOrEmpty(specifiedXMatterPack))
			{
				_nameOfXMatterPack = specifiedXMatterPack;
				var errorTemplate = LocalizationManager.GetString("Errors.SpecifiedXMatterNotFound",
					"This Book called for Front/Back Matter pack named '{0}', but Bloom couldn't find that on this computer.  {1} is being used instead, and the book will be modified accordingly if you edit it.");
				directoryPath = GetXMatterDirectory(fileLocator, String.Format(errorTemplate, specifiedXMatterPack, nameOfDefaultXMatterPack), false);
		...
		{
			var xmatter = bookDom.GetMetaValue("xmatter", null);
			if (xmatter == null)
				return null;
			if (String.IsNullOrWhiteSpace(xmatter))
				return null;
			return xmatter.Trim();
		}
