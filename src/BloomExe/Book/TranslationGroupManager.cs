...
namespace Bloom.Book
{
	...
	public static class TranslationGroupManager
	{
			...
			BookData bookData) // use to get the collection's current N1 and N2 in xmatter or other template pages that specify default languages
		{
			...
			{
				// Note there are (perhaps unfortunately) two different labelling systems, but they have a more-or-less 1-to-1 correspondence:
				...
				// Review: do we really want V (AND L1) to mean "the first selected language"? It's important for 'Title'
				// but it might be better to make a special case for "V, N1"
