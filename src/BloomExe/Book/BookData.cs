...
namespace Bloom.Book
{
	...
	*/
	...
	public class BookData
	{
		...
		/// If the book is configured as bilingual, currently this is always the second
		/// of the two languages the user selected. Otherwise, it is the collection's L2.
		/// It should never be null, but might be the same as Language1 (or 2, or in
		/// principle even L3, though I don't think that can actually happen yet).
		/// </summary>
		...
		private void CacheLangData()
		{
			...
			// We don't have any Lang2 derivatives at this point because the derivatives come
			// from history before we introduced MetadataLanguage1. So changes to Language2
			// result in changes to things that are now derived from ML1.
		...
		private void SetUpDisplayOfTopicInBook(DataSet data)
		{
			...
			var tagsInPriorityOrder = GetPrioritizedLocalizationTags();
		...
		public void Set(string key, XmlString value, bool isCollectionValue)
		{
			...
			{
				_cachedIsoLang2 = value.Unencoded;
				UpdateML1Derivatives();
			}
...
//        }
		...
		/// <summary>
		/// Get a list of codes to try in order to see if a string is localized into any of them
		/// </summary>
		/// <returns></returns>
		public List<string> GetPrioritizedLocalizationTags()
		{
			var result = GetBasicBookLanguageCodes(true);
			// Try any collection settings we don't already have
			AddLang(result, CollectionSettings.Language1Iso639Code);
			AddLang(result, CollectionSettings.Language2Iso639Code);
			AddLang(result, CollectionSettings.Language3Iso639Code);
			return result;
		}
