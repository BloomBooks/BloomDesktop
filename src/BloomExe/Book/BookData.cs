	...
	*/
	...
	public class BookData
	{
		...
		private void UpdateVariablesAndDataDiv(XmlNode elementToReadFrom, BookInfo info = null)
		{
			...
			incomingData.UpdateGenericLanguageString("contentLanguage1", GetLanguage(LanguageSlot.Language1).Iso639Code, false);
				...
				if (int.TryParse(stageString, out stage) && CollectionSettings != null)
				{
					...
					if (File.Exists(settingsPath))
					{
						try
						{
							...
							incomingData.UpdateLanguageString("decodableStageLetters", letters, GetLanguage(LanguageSlot.Language1).Iso639Code, false);
						}
		...
		private void SetupDisplayOfLanguagesOfBook(DataSet data)
		{
			...
			{
				element.SetAttribute("lang", GetLanguage(LanguageSlot.Language2).Iso639Code);
		...
		private void SetUpDisplayOfTopicInBook(DataSet data)
		{
			...
			var langOfTopicToShowOnCover = GetLanguage(LanguageSlot.Language1).Iso639Code;
			if (LocalizationManager.GetIsStringAvailableForLangId(stringId, GetLanguage(LanguageSlot.Language1).Iso639Code))
			{
				langOfTopicToShowOnCover = GetLanguage(LanguageSlot.Language1).Iso639Code;
			}
			else if (LocalizationManager.GetIsStringAvailableForLangId(stringId, GetLanguage(LanguageSlot.Language2).Iso639Code))
			{
				langOfTopicToShowOnCover = GetLanguage(LanguageSlot.Language2).Iso639Code;
			}
			else if (LocalizationManager.GetIsStringAvailableForLangId(stringId, GetLanguage(LanguageSlot.Language3).Iso639Code))
			{
				langOfTopicToShowOnCover = GetLanguage(LanguageSlot.Language3).Iso639Code;
			}
			else
			{
		...
		private DataSet GatherDataItemsFromCollectionSettings(CollectionSettings collectionSettings)
		{
			...
			// REVIEW BL-9449: still collect language settings from CollectionSettings originally?
...
//            else
		...
		private string GetLanguageNameWithScriptVariants(string completeIsoCode, string collectionSettingsLanguageName, bool nameIsCustom)
		{
			...
			return nameIsCustom ?
				collectionSettingsLanguageName + " (" + CollectionSettings.GetLanguageName(baseIsoCode, GetLanguage(LanguageSlot.Language2).Iso639Code) + ")"
			...
			try
			{
				...
				foreach (XmlElement node in nodesOfInterest)
				{
					...
					if (lang == "") //the above doesn't stop a "" from getting through
						lang = "*";
					if (lang == "{V}")
						lang = GetLanguage(LanguageSlot.Language1).Iso639Code;
					if (lang == "{N1}")
						lang = GetLanguage(LanguageSlot.Language2).Iso639Code;
					if (lang == "{N2}")
						lang = GetLanguage(LanguageSlot.Language3).Iso639Code;
		...
		private string PossiblyCopyFromAnotherLanguage(XmlElement element, string languageCodeOfTargetField, DataSet data, string key)
		{
			...
			if ((languageCodeOfTargetField == GetLanguage(LanguageSlot.Language2).Iso639Code || //is it a national language?
				 languageCodeOfTargetField == GetLanguage(LanguageSlot.Language3).Iso639Code) ||
		...
		private void UpdateTitle(BookInfo info = null)
		{
			...
			{
				string innerHtml = title?.TextAlternatives.GetExactAlternative(GetLanguage(LanguageSlot.Language1).Iso639Code);
		...
		private string[] WritingSystemIdsToTry
		{
			get
			{
				return new string[]
				{
					GetLanguage(LanguageSlot.Language1).Iso639Code??"",
					GetLanguage(LanguageSlot.Language2).Iso639Code??"",
					GetLanguage(LanguageSlot.Language3).Iso639Code ?? "",
					"en", "fr", "th", "pt", "*"
				};
			}
		}
		...
		{
			if (language2Code == GetLanguage(LanguageSlot.Language1).Iso639Code) //can't have the vernacular twice
				language2Code = null;
			if (language3Code == GetLanguage(LanguageSlot.Language1).Iso639Code)
				language3Code = null;
...
//        }
		...
		{
			List<string> langCodes = new List<string> { GetLanguage(LanguageSlot.Language2).Iso639Code, GetLanguage(LanguageSlot.Language3).Iso639Code, "en" };
			if (includeLang1)
				langCodes.Insert(0, GetLanguage(LanguageSlot.Language1).Iso639Code);
		...
		internal LanguageDescriptor[] MakeLanguageUploadData(string[] isoCodes)
		{
			...
			for (int i = 0; i < isoCodes.Length; i++)
			{
				var code = isoCodes[i];
				string name;
				if (code == GetLanguage(LanguageSlot.Language1).Iso639Code)
					name = GetLanguage(LanguageSlot.Language1).Name;
				else if (code == GetLanguage(LanguageSlot.Language2).Iso639Code)
					name = GetLanguage(LanguageSlot.Language2).Name;
				else if (code == GetLanguage(LanguageSlot.Language3).Iso639Code)
					name = GetLanguage(LanguageSlot.Language3).Name;
				else
