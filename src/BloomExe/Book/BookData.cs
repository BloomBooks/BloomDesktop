...
//            else
		...
		private static bool StringAlternativeHasNoText(string s)
		{
			if (string.IsNullOrEmpty(s))
			{
				return true;
			}
			// review: I want to do xmlNode.InnerText, but I have only a string that might not even be xml
			// is there a better way?
			return s.Replace("<p>", string.Empty).Replace("</p>", string.Empty).Replace("<p />", string.Empty) == string.Empty;
		}
