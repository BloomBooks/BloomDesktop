	...
	public class HtmlDom
	{
		...
		public string GetColorsUsedInBookBubbleElements()
		{
			...
			foreach (var node in textOverPictureElements)
			{
				...
				var opacityValue = GetOpacityFromDataBubble(dataBubbleNoTicks);
		...
		private static XmlNodeList GetAllDivsWithClass(XmlElement containerElement, string className)
		{
		...
		private static IEnumerable<XmlElement> GetTextOverPictureElements(XmlElement bookBodyElement)
		{
		...
		*/
		...
		public static void ProcessPageAfterEditing(XmlElement destinationPageDiv, XmlElement edittedPageDiv)
		{
			...
			foreach (
				var node in
					// Review: Assuming we can guarantee they are all divs?
					GetAllDivsWithClass(destinationPageDiv, "origami-layout-mode")
						.Cast<XmlNode>()
						.ToArray())
			{
