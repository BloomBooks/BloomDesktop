	...
	public class HtmlDom
	{
		...
		public string ValidateBook(string descriptionOfBookForErrorLog, bool mustHavePages)
		{
			...
			if (mustHavePages)
			{
				...
				// The below method verifies that each page is still at the right level of the file, which actually
				...
				foreach (XmlNode pageNode in pages)
				{
					...
					if (string.IsNullOrEmpty(id) && Program.RunningUnitTests)
						continue;
					var result = RawDom.SelectSingleNodeHonoringDefaultNS("/html/body/div[@id='" + id + "']");
					if (result == null)
						builder.AppendLine("Page could not be found: " + id);
				}
			}
