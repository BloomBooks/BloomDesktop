...
namespace Bloom.Book
{
	...
	public class HtmlDom
	{
		...
		*/
		...
		/// These will only be correct if the dom is the full book; froma dom made of one page,
		...
		public void UpdatePageNumberAndSideClassOfPages(string charactersForDigits, bool languageIsRightToLeft)
		{
			...
			foreach (var pageDiv in GetPageElements())
			{
				...
				// we could keep going throught the back cover so long as the stylesheet
		...
		/// <summary>
		/// Figure out the page number (which is not just the position; frontmatter can control when numbering starts).
		/// Add it to the html in the desired script.
		/// </summary>
		public static void AddPageNumber(HtmlDom bookDom, HtmlDom pageDom, string pageId)
		{
			var existingElement = pageDom.RawDom.SelectSingleNode("//style[@id='page-number']") as XmlElement;
			var element = pageDom.RawDom.CreateElement("style");
			element.SetAttribute("id", "page-number");
			var num = bookDom.GetPageNumberOfPage(bookDom.FindPageById(pageId));
			element.InnerText = ".bloom-page{counter-reset: pageNumber " + num + "}";
