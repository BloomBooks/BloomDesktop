		...
		private static void ConvertImagesToBackground(XmlDocument dom)
		{
			foreach (var imgContainer in dom.SafeSelectNodes("//div[contains(@class, 'bloom-imageContainer')]").Cast<XmlElement>().ToArray())
			{
				...
				if (img == null || img.Attributes["src"] == null)
					continue;
				// The filename shouldn't have double quotes because that's not valid (at least in Windows).
				// It might have single quotes, which need escaping because we use them to delimit the url value in the background-image.
