...
namespace Bloom.Book
{
	public class BookCompressor
	{
		...
		internal static readonly string[] IncludedFileExtensionsLowerCase = { ".svg", ".png", ".jpg", ".tif", ".tiff", ".bmp", ".otf", ".ttf", ".woff", ".htm", ".css", ".json", ".txt", ".mp3", ".ogg", ".wav", ".mp4" };
		...
		private static bool IsValidBookFolder(string folderName, bool excludeAudio, int depthFromCollection)
		{
			...
			// We only check depth 2, because 1 is not a subfolder and farther down, if we are going there,
			// we want everything.
