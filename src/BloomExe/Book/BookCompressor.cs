...
namespace Bloom.Book
{
	public class BookCompressor
	{
		...
		private static void CompressDirectory(string directoryToCompress, ZipOutputStream zipStream, int dirNameOffset, string dirNamePrefix,
			bool forReaderTools, bool excludeAudio, bool reduceImages, bool omitMetaJson = false, string pathToFileForSha = null, bool isTopLevel=true)
		{
			...
			// Don't get distracted by HTML files in any folder other than the top-level book folder.
			// These HTML files in other locations aren't generated by Bloom. They may not have the format Bloom expects,
			// causing needless parsing errors to be throw if we attempt to read them using Bloom code.
			var bookFile = isTopLevel ? BookStorage.FindBookHtmlInFolder(directoryToCompress) : null;
			...
			foreach (var folder in folders)
			{
				...
				CompressDirectory(folder, zipStream, dirNameOffset, dirNamePrefix, forReaderTools, excludeAudio, reduceImages, isTopLevel: false);
			}
		}
