...
namespace Bloom.Book
{
	public class BookCompressor
	{
			...
			int depthFromCollection, bool forReaderTools, bool excludeAudio, bool reduceImages, bool omitMetaJson = false, string pathToFileForSha = null)
		{
			...
			{
				if (FileIsNotWhitelisted(filePath, depthFromCollection))
		...
		// This method should return 'true' only if the .bloomd doesn't need this file.
		private static bool FileIsNotWhitelisted(string filePath, int depthFromCollection)
		{
			...
			if (depthFromCollection > 2 && IsInActivitiesFolder(filePath, depthFromCollection))
				return false; // We accept any file in 'activities' and its subfolders
