...
//		}
		...
		private void SetBookTitle(BookStorage storage, BookData bookData, bool usingTemplate)
		{
			...
			if (storage.BookInfo.IsSuitableForMakingShells)
			{
				...
				// include it.
				// REVIEW BL-9449: Should Language1 for a new book still come from the collection?
		...
		private void InjectXMatter(string initialPath, BookStorage storage, Layout sizeAndOrientation)
		{
			...
			if (!TestingSoSkipAddingXMatter)
			{
				...
				// REVIEW BL-9449: Should Language1 etc. for a new book come from the collection?
