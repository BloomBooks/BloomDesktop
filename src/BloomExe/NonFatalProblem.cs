	...
	public class NonFatalProblem
	{
			...
			Exception exception = null, bool showSendReport = true)
		{
			...
			try
			{
				...
				{
					// targetting ProgressDialog doesn't work so well for toasts, so don't
					modalThreshold = (ModalIf)ModalIf.Parse(typeof(ModalIf), passiveThreshold.ToString());
				}
				...
				if (Matches(modalThreshold).Any(s => channel.Contains(s)))
				{
					try
					{
						...
						{
							// We don't want even our MessageBox targetting a ProgressDialog.
							if (formForSynchronizing is ProgressDialog)
							{
