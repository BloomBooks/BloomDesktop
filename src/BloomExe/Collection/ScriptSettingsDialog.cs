...
namespace Bloom.Collection
{
    public partial class ScriptSettingsDialog : Form
    {
        ...
        private void SetLineHeightList()
        {
            ...
            var defaultText = LocalizationManager.GetDynamicString(
                "Bloom",
                "ScriptSettingsDialog.DefaultLineSpacing",
        ...
        public decimal LanguageLineSpacing
        {
            get
            {
                if (!_lineSpacingCombo.Enabled)
                    return 0;
                return _lineSpacingCombo.SelectedIndex < 1
                    ? 0
                    : Convert.ToDecimal(_lineSpacingCombo.Text);
            }
            set
            {
                if (value < 1)
        ...
        public int UIFontSize
        {
            get
            {
                return _uiFontSizeCombo.SelectedIndex < 1
                    ? 0
                    : Convert.ToInt16(_uiFontSizeCombo.Text);
            }
            set
            {
        ...
        private void LoadFontSizeCombo()
        {
            var defaultText = LocalizationManager.GetDynamicString(
                "Bloom",
                "ScriptSettingsDialog.DefaultSize",
                "Default size"
            );
