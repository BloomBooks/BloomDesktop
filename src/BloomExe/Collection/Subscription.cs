...
public class Subscription
{
    ...
    /// <param name="descriptor">content of &lt;BrandingProjectName&gt; from file, or "Default" if empty</param>
    /// <param name="editingABlorgBook">true iff we're editing a book downloaded specifically for edit/update</param>
    /// <returns>the subscription object based on the inputs</returns>
    /// <remarks>
    /// When downloading books for editing/updating, Bloom Library provides a code of "Local-Community-***-***"
    /// for the branding of "Local-Community" and does not provide a branding.
    /// </remarks>
    ...
    )
    {
        ...
        {
            // Migrate the local collection to the actual code and descriptor.
            code = "Legacy-LC-005809-2533"; // expires on 1 July 2025
            descriptor = "Legacy-LC";
        }
        ...
        if (editingABlorgBook)
        {
            ...
            if (code == null && descriptor == "Default")
                sub.Tier = SubscriptionTier.None;
            // After migration, the code for "Local-Community" is "Legacy-LC-5809-2533", but
            // the branding project name is still "Local-Community".
            else if (descriptor.EndsWith("-LC") || descriptor == "Local-Community")
