...
namespace Bloom.Collection
{
    ...
    public static class MinimumBloomVersionCheck
    {
        ...
        {
            // Two sources can disagree: the file on this computer, and whatever a Team Collection
            // repository has told us since. Take whichever demands more, rather than letting either
            // one win outright. The repository is usually the newer word, but its copy lives in a
            // zip that can be mid-sync and momentarily missing the element -- and this is a
            // protection, so the failure we can live with is keeping someone out a little longer
            // than necessary, not letting them in when we should not have.
            var declaredVersion = MoreDemandingOf(
