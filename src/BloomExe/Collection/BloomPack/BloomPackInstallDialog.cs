    ...
    public partial class BloomPackInstallDialog : Form
    {
        ...
        public BloomPackInstallDialog(string path)
        {
            ...
            var msg = L10NSharp.LocalizationManager.GetString(
                "BloomPackInstallDialog.Opening",
                "Opening {0}..."
            );
        ...
        private void BeginInstall()
        {
            ...
            {
                string msg = L10NSharp.LocalizationManager.GetString(
                    "BloomPackInstallDialog.DoesNotExist",
                    "{0} does not exist"
                );
            ...
            if (Directory.Exists(destinationFolder))
            {
                ...
                string title = L10NSharp.LocalizationManager.GetString(
                    "BloomPackInstallDialog.BloomPackInstaller",
                    "Bloom Pack Installer",
                    "Displayed as the message box title"
                );
                string msg = L10NSharp.LocalizationManager.GetString(
                    "BloomPackInstallDialog.Replace",
                ...
                {
                    _message.Text = L10NSharp.LocalizationManager.GetString(
                        "BloomPackInstallDialog.NotInstalled",
                        "The Bloom collection will not be installed."
                    );
                    _okButton.Text = L10NSharp.LocalizationManager.GetString(
                        "Common.CancelButton",
                        "&Cancel"
                    );
                    return;
                ...
                {
                    string text = L10NSharp.LocalizationManager.GetString(
                        "BloomPackInstallDialog.UnableToReplace",
            ...
            _okButton.Enabled = false;
            _message.Text = L10NSharp.LocalizationManager.GetString(
                "BloomPackInstallDialog.Extracting",
                "Extracting...",
                "Shown while Bloom Packs are being installed"
            );
        ...
        {
            string msg = L10NSharp.LocalizationManager.GetString(
                "BloomPackInstallDialog.SingleCollectionFolder",
        ...
        {
            string msg = L10NSharp.LocalizationManager.GetString(
                "BloomPackInstallDialog.CorruptBloomPack",
        ...
        )
        {
            ...
            if (_rootDestFolder == ProjectContext.XMatterAppDataFolder)
                return;
            var newlyAddedFolderOfThePack = Path.Combine(
            ...
            foreach (var dir in Directory.GetDirectories(newlyAddedFolderOfThePack, "*-XMatter"))
            {
                ...
                catch (Exception error)
                {
                    throw new ApplicationException(
                        "Could not delete the existing xmatter pack in order to update it",
                        error
                    );
                }
                try
                ...
                {
                    throw new ApplicationException(
                        "Could not move an xmatter pack from collections to xmatter",
                        error
                    );
                }
            }
        ...
        )
        {
            ...
            {
                _message.Text = L10NSharp.LocalizationManager.GetString(
                    "BloomPackInstallDialog.ErrorInstallingBloomPack",
                    "Bloom was not able to install that Bloom Pack"
                );
                if (
                    e.Error is ArgumentException
                    && e.Error.StackTrace.Contains("CheckIllegalCharacters")
                    && !_message.IsDisposed
                )
                {
                    _message.Text +=
                        ...
                        + L10NSharp.LocalizationManager.GetString(
                            "BloomPackInstallDialog.BadCharsInFileName",
            ...
            var allDone = L10NSharp.LocalizationManager.GetString(
                "BloomPackInstallDialog.BloomPackInstalled",
            ...
            if (Program.GetRunningBloomProcessCount() > 1)
            {
                if (!_message.IsDisposed)
                {
                    _message.Text +=
                        ...
                        + Environment.NewLine
                        + L10NSharp.LocalizationManager.GetString(
                            "BloomPackInstallDialog.MustRestartToSee",
