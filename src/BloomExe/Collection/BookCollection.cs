	...
	public class BookCollection
	{
		...
		private void MakeCollectionCSSIfMissing()
		{
			...
			if(SafeFile.Exists(path))
				return;
			SafeFile.Copy(BloomFileLocator.GetBrowserFile("bookLayout", "collection styles override template.css"), path);
		}
		...
		private void LoadBooks()
		{
			...
			foreach (var folder in orderedBookFolders)
			{
				...
				if (Path.GetFileName(folder.FullName).ToLowerInvariant().Contains("xmatter"))
					continue;
				if(SafeFile.Exists(Path.Combine(folder.FullName, ".bloom-ignore")))
					continue;
		...
		private void AddBookInfo(string folderPath)
		{
			...
			catch (Exception e)
			{
				...
				try
				{
					Logger.WriteEvent(jsonPath +" Contents: " +System.Environment.NewLine+ SafeFile.ReadAllText(jsonPath));
				}
