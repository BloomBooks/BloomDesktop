	...
	public class SpreadsheetExporter
	{
		...
		public InternalSpreadsheet Export(HtmlDom dom)
		{
			...
			foreach (var page in pages)
			{
				...
				if (pageNumber == "")
					continue;
				var groups = SortTranslationGroups(TranslationGroupManager.GetTranslationGroups(page));
				int groupIndex = 1;
		...
		// Sort the list of translation groups. Ones that have tabindex are
		...
		{
			if (int.TryParse(x.Attributes["tabindex"]?.Value?? "x", out int val))
				return val;
			return int.MaxValue;
		}
	}
