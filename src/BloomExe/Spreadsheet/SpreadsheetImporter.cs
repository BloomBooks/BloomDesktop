...
namespace Bloom.Spreadsheet
{
	...
	public class SpreadsheetImporter
	{
		...
		private void UpdateDataDivFromRow(ContentRow currentRow, string dataBookLabel)
		{
			...
			else //This is not an image node
			{
				...
				foreach (string lang in _sheet.Languages)
				{
					...
					{
						foreach (XmlNode n in langMatchingNodes)
						{
				...
				{
					RemoveOtherLanguagesInDataDiv(matchingNodes);
				}
		...
		private void RemoveOtherLanguagesInDataDiv(XmlNodeList matchingNodes)
		{
			foreach (XmlElement n in matchingNodes.Cast<XmlNode>())
			{
				var lang = n.GetAttribute("lang");
				if (! _sheet.Languages.Contains(lang) && lang.Length > 0)
				{
					_dataDivElement.RemoveChild(n);
				}
			}
