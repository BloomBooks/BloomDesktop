...
namespace Bloom.Spreadsheet
{
	...
	public class SpreadsheetImporter
	{
		...
		public List<string> Import()
		{
			...
			_inputRows = _sheet.GetContentRows().ToList();
			...
			{
				var pageNumber = NumberOfPage(_currentPage);
				if (_currentRowIndex >= _inputRows.Count)
				{
					...
					while (_currentPageIndex < _pages.Count && pageNumber == "")
					{
						...
						_currentPageIndex++;
						if (GroupsOnPage(page).Count == 0)
							continue;
						pageNumber = NumberOfPage(page);
				...
				if (rowLabel != pageNumber)
				{
					...
					var indexOfRightPage = IndexOfNextPageWithNumber(rowLabel);
					if (indexOfRightPage > 0)
					{
						...
						_currentPageIndex = indexOfRightPage - 1;
		...
		private static string NumberOfPage(XmlElement page)
		{
		...
		private int IndexOfNextPageWithNumber(string number)
		{
			...
			{
				if (NumberOfPage(_pages[i]) == number)
					return i;
			}
		...
		private bool GetNextGroup()
		{
			...
			while (_groupOnPageIndex >= _groupsOnPage.Count)
			{
				...
				if (NumberOfPage(_currentPage) == "")
					_groupsOnPage = new List<XmlElement>(0); // skip xmatter or similar page
				else
					_groupsOnPage = GroupsOnPage(_currentPage);
				_groupOnPageIndex = 0;
			}
		...
		private List<XmlElement> GroupsOnPage(XmlElement page)
		{
			return SpreadsheetExporter.SortTranslationGroups(page
		...
		private void PutRowInGroup(ContentRow row, XmlElement group)
		{
			...
			foreach (var lang in sheetLanguages)
			{
				...
				var content = row.Cell(colIndex).Content;
				var editable = EditableChildInLang(group, lang);
				if (editable == null)
				{
					if (string.IsNullOrEmpty(content))
						continue; // Review: or make an empty one?
					var temp = EditableChildInLang(group, "z"); // standard template element
					if (temp == null)
						temp = EditableChildInLang(group, null); // use any available template
					if (temp == null)
						return; // todo: if temp is still null, make some sort of default? At least report problem.
			...
			if (RemoveOtherLanguages)
			{
					...
					.Cast<XmlElement>().ToList())
				{
					...
					if (lang == "z")
						continue;
					if (!sheetLanguages.Contains(lang))
		...
		private bool NoMarkup(string content)
		{
			// Anything that is bloom marked-up content is bound to have wedges.
			if (content.IndexOf("<") < 0)
				return true;
			return false;
		}

		private XmlElement EditableChildInLang(XmlElement parent, string lang)
		{
