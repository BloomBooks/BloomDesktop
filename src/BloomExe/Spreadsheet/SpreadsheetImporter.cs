	...
	public class SpreadsheetImporter
	{
		...
		public List<string> Import(InternalSpreadsheet sheet, IWebSocketProgress progress = null)
		{
			...
			if (_collectionSettings != null)
			{
				...
				// These methods are both part of BringBookUpToDate. If we run SS Importer from the CLI,
				// (without CollectionSettings), they will happen when we eventually open the book, but the
				// user gets a better experience if the "with CollectionSettings version" does them right away.
				// We considered doing at least the xmatter update unconditionally, but that breaks a bunch
				// of tests (90!). Testing is the other situation that doesn't use CollectionSettings.
				...
				if (_book != null) // Should only happen in tests
				{
					_book.BringXmatterHtmlUpToDate(_destinationDom);
					_book.Save();
				}
			}
