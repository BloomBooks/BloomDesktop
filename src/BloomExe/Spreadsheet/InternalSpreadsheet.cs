...
namespace Bloom.Spreadsheet
{
	...
	public class InternalSpreadsheet
	{
		...
		public int GetColumnForLang(string langCode)
		{
		...
		public int SetColumnForLang(string langCode, string langFriendlyName)
		{
			string columnLabel = "[" + langCode + "]";
			return SetColumnForTag(columnLabel, langFriendlyName);
		}

		/// <summary>
		/// Adds a column. If the column label already exists, no new column is added, but the column name will be updated if necessary
		/// </summary>
		/// <param name="columnLabel"></param>
		/// <param name="columnFriendlyName"></param>
		/// <returns>The index of the column (0-based)</returns>
		public int SetColumnForTag(string columnLabel, string columnFriendlyName)
		{
			for (var i = 0; i < Header.ColumnCount; i++)
			{
				if (Header.ColumnIdRow.GetCell(i).Content.Equals(columnLabel))
				{
					Header.ColumnNameRow.SetCell(i, columnFriendlyName);
					return i;
				}
			}
