...
namespace Bloom.Spreadsheet
{
	...
	public class SpreadsheetIO
	{
		...
		public static string ReplaceExcelEscapedChars(string escapedString)
		{
			...
			while (match.Success)
			{
				string x = match.Value.Substring(2, 4);
		...
		public static string BuildXmlString(ExcelRange cell)
		{
			...
			foreach (var paragraph in markedUpString.Split(whitespaceSplitters, StringSplitOptions.None))
			{
				paragraphedStringBuilder.Append("<p>");
				paragraphedStringBuilder.Append(paragraph);
				paragraphedStringBuilder.Append("</p>");
			}
