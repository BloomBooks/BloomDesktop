		...
		public static void WriteSpreadsheet(InternalSpreadsheet spreadsheet, string outputPath, bool retainMarkup)
		{
			using (var package = new ExcelPackage())
			{
				...
				catch (IOException ex) when ((ex.HResult & 0x0000FFFF) == 32)//ERROR_SHARING_VIOLATION
				{
					...
					NonFatalProblem.Report(ModalIf.All, PassiveIf.None, "Bloom could not write to the spreadsheet because another program has it locked. Do you have it open in another program?\r\n",
						moreDetails: ex.Message, showSendReport: false);
				}
				catch (Exception ex)
				{
