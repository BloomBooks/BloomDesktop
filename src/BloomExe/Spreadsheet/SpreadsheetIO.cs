...
namespace Bloom.Spreadsheet
{
    ...
    public class SpreadsheetIO
    {
        ...
        public static void WriteSpreadsheet(
            InternalSpreadsheet spreadsheet,
            string outputPath,
            bool retainMarkup,
        ...
        )
        {
            using (var package = new ExcelPackage())
            {
                ...
                var imageSourceColumn = spreadsheet.GetColumnForTag(
                    InternalSpreadsheet.ImageSourceColumnLabel
                );
                var imageThumbnailColumn = spreadsheet.GetColumnForTag(
                ...
                foreach (var row in spreadsheet.AllRows())
                {
                    ...
                    for (var c = 0; c < row.Count; c++)
                    {
                        ...
                        if (!retainMarkup && IsWysiwygFormattedPair(row, c))
                        {
                            ...
                            if (markedUpText.HasFormatting)
                            {
                                ...
                                foreach (MarkedUpTextRun run in markedUpText.Runs)
                                {
                                    if (!run.Text.Equals(""))
                                    {
                                        ...
                                        {
                                            text.VerticalAlign =
                                                ExcelVerticalAlignmentFont.Superscript;
                                        }
                ...
                int embedImage(string imageSrcPath, int rowNum, int colNum)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        progress?.MessageWithoutLocalizing(
                            errorText + ": " + imageSrcPath + ": " + ex.Message,
                ...
                foreach (var iColumn in spreadsheet.HiddenColumns)
                {
                    ...
                    SetBackgroundColorOfColumn(
                        worksheet,
                        iColumn1Based,
                        InternalSpreadsheet.HiddenColor
                    );
                }
        ...
        private static HashSet<string> nonWysiwygColumns = new HashSet<string>(
            new[]
            {
                "[*]",
        ...
        public static string ReplaceExcelEscapedCharsAndEscapeXmlOnes(
            string escapedString,
            bool wantXmlEscaping = true
        )
        {
        ...
        public static string BuildXmlString(ExcelRange cell)
        {
            ...
            string[] paragraphs = markedUpString.Split(
                whitespaceSplitters,
        ...
        private static void AddRunToXmlString(
            ExcelRichText formattingText,
            ExcelFont cellFormatting,
            string text,
        ...
        )
        {
            ...
            if (
                formattingText.VerticalAlign == ExcelVerticalAlignmentFont.Superscript
            ...
            if (color != null)
            {
                ...
                addTags(
                    "span",
                    endTags,
                    attributesString: string.Format("style=\"color:{0};\"", colorHtmlString)
                );
            }
        ...
        private static void SetBackgroundColorOfColumn(
            ExcelWorksheet worksheet,
            int iColumn,
            Color color
        )
