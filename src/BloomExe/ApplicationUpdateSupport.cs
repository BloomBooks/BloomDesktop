...
namespace Bloom
{
    ...
    static class ApplicationUpdateSupport
    {
            ...
            catch (Exception ex)
            {
                ...
                if (ignoreDeltaUpdates == false)
                {
                    ...
                    )
                    {
                        ...
                        // and the updates include a version beyond 6.2, then we should try to transition to
                        // a velopack installation as the Squirrel update appears to have crashed while
                        // installing the velopack release.  So we set a flag to do that on the retry.  Since
        ...
        )
        {
            ...
            // If Update.exe is present, then it's fair to assume this is an installed build, not a local
            // developer debug build.  In that case, if we're not in a "current" directory, we need run
            // Update.exe to clean up the mess.  (And I don't see how we could possibly be in a "current"
            // directory since that's a feature of velopack which is introduced in Bloom 6.3.)  Squirrel
            // seems to install a velopack Update.exe before it fails to apply the rest of the update.
            ...
            if (
                ...
                // I don't see how bloomDir could possibly be "current" here, but just in case...
            ...
            // I don't see how we can get here, but just in case I think it's better to report failure
            // since an exception installing the updates happened earlier to lead to this point.
            return new UpdateResult()
            {
                NewInstallDirectory = null,
                Outcome = UpdateOutcome.InstallFailed
