...
namespace Bloom.MiscUI
{
    public class BloomIntegrityChecker
    {
        ...
        public static bool CheckIntegrity()
        {
            ...
            var files = new[] { "Bloom.chm", "PdfDroplet.dll", "BloomPdfMaker.exe" };
            ...
            foreach (var fileName in files)
            {
                if (FileLocationUtilities.GetFileDistributedWithApplication(true, fileName) == null)
                {
                    ...
                    if (
                        !RobustFile.Exists(
                            Path.Combine(
                                FileLocationUtilities.DirectoryOfTheApplicationExecutable,
                                fileName
                            )
                        )
                    )
            ...
            {
                if (
                    FileLocationUtilities.GetDirectoryDistributedWithApplication(true, directory)
                    == null
                )
                {
                    errors.AppendFormat(
                        "<p>Missing Directory: {0}</p>{1}",
                        directory,
                        Environment.NewLine
                    );
                }
            ...
            const string nonHtmlMessage =
                "Bloom cannot find some of its own files, and cannot continue. After you submit this report, we will contact you and help you work this out. In the meantime, you can run the Bloom installer again.";
            var messagePath = BloomFileLocator.GetBestLocalizableFileDistributedWithApplication(
                false,
                "help",
                "IntegrityFailureAdvice-en.htm"
            );
            string message;
            ...
            {
                var installFolder = Environment
                    .GetFolderPath(Environment.SpecialFolder.LocalApplicationData)
                    .CombineForPath(Application.ProductName);
                message = RobustFile
                    .ReadAllText(messagePath)
                    .Replace("{{installFolder}}", installFolder); //new
            ...
            message = message.Replace(
                "</head>",
                "<style>body {font-family:Segoe UI, Arial, san-serif; font-size:10pt}</style></head>"
            );
