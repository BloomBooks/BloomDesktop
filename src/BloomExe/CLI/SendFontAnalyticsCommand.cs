...
namespace Bloom.CLI
{
    ...
    public class SendFontAnalyticsCommand
    {
                ...
                using (var applicationContainer = new ApplicationContainer())
                {
                    ...
                    )
                    {
                        ...
                        if (BloomPubMaker.BloomPubFontsAndLangsUsed != null)
                        {
                            ...
                            ReportFontAnalytics(
                                _book.ID,
                                "harvester sendFontAnalytics",
                                options.Testing,
                                options.SkipEpubAnalytics,
                                options.SkipPdfAnalytics
                            );
        ...
        /// </remarks>
        public static void ReportFontAnalytics(
            string bookId,
            string details,
            bool forTesting,
            bool skipEpub,
            ...
            foreach (var fontName in BloomPubMaker.BloomPubFontsAndLangsUsed.Keys)
            {
                foreach (var lang in BloomPubMaker.BloomPubFontsAndLangsUsed[fontName])
                {
                    ...
                    FontAnalytics.Report(
                        ...
                        "2.0",
                        bookId,
                        FontAnalytics.FontEventType.PublishWeb,
                        lang,
                        testOnly,
                        fontName,
                        details
        ...
        private static void CollectFontAnalytics(SendFontAnalyticsParameters options)
        {
            using (var stagingFolder = new TemporaryFolder("FontAnalytics"))
            {
                ...
                var settings = BloomPubPublishSettings.GetPublishSettingsForBook(
                    _projectContext.BookServer,
                    _book.BookInfo
                );
                // This method will gather up the desired font analytics as a side-effect.
                BloomPubMaker.PrepareBookForBloomReader(
                    settings,
                    bookFolderPath: options.BookPath,
                    bookServer: _projectContext.BookServer,
                    temp: stagingFolder,
                    progress: new Bloom.web.NullWebSocketProgress(),
                    isTemplateBook: isTemplateBook
                );
            }
        }
    }
    ...
    public class SendFontAnalyticsParameters
    {
        [Value(
            ...
            MetaName = "bookPath",
            Required = true,
            HelpText = "Path to the folder of the book to get font analytics from."
        )]
        ...
        [Option(
            "collectionPath",
            Required = true,
            HelpText = "Input path of the Bloom collection file for this book"
        )]
        ...
        [Option(
            ...
            Required = false,
            Default = false,
            HelpText = "Flag that no font analytics are wanted for an Epub"
        )]
