...
namespace Bloom.CLI
{
	...
	class SpreadsheetImportCommand
	{
		public static int Handle(SpreadsheetImportParameters options)
		{
			try
			{
				...
				var importer = new SpreadsheetImporter(dom,sheet,Path.GetDirectoryName(options.InputPath), folderPath);
	...
	[Verb("spreadsheetImport", HelpText = "Import a book from an Excel spreadsheet (xslx). See https://docs.google.com/document/d/1Fg95M3Q-8bUHMAGrsvDnnuKKXKQGGBTaVa7EaizqWwg/edit?usp=sharing")]
	public class SpreadsheetImportParameters
	{
		[Value(0, MetaName = "path", HelpText = "Path to the book (htm file) to import into.", Required = true)]
