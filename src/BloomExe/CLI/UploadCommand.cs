...
[Verb(
    "upload",
    HelpText = "Upload collections of books to bloomlibrary.org. Cannot be used to upload only a single book. Given a folder that is a collection, this will upload the it. Given a folder that is not a collection, it will search for descendant folders that contain collections.\r\nExample: bloom upload \"c:\\foo\\all my collections\". (Do not use a trailing slash). "
...
public class UploadParameters
{
    [Value(
        0,
        MetaName = "path",
        HelpText = "Specify the path to a folder containing books to upload at some level within.",
        Required = true
    )]
    ...
    [Option(
        'e',
        "excludeMusicAudio",
        HelpText = "Exclude music (background) audio files from upload.  (The default is to upload music files.)",
        Required = false
    )]
    ...
    [Option(
        'u',
        "user",
        HelpText = "Specify the email account for the upload. Must match the currently logged in email from Bloom:Publish:Upload (share on the web) screen.",
        Required = true
    )]
    ...
    [Option(
        'T',
        "preserveThumbnails",
        HelpText = "Preserve any existing thumbnail images: don't try to recreate them.",
        Required = false
    )]
    ...
    [Option(
        'd',
        "destination",
        HelpText = "'dev' will upload to dev.bloomlibrary.org (you will need to use an account from there). 'production' will upload to bloomlibrary.org",
        Required = true
    )]
    ...
    [Option(
        "dry-run",
        Default = false,
        HelpText = "If present, performs a dry-run (will just print out what would happen).",
        Required = false
    )]
    ...
    [Option(
        'F',
        "force",
        HelpText = "Force the upload even if existing .lastUploadInfo content indicates that the book has already been uploaded.",
        Required = false
    )]
