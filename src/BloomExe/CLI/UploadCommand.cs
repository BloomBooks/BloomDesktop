		...
		public static int Handle(UploadParameters options)
		{
			...
			switch (options.Dest)
			{
				...
				case UploadDestination.Production:
					break;
				default:
					Console.WriteLine($"Error: if present, upload -d <dest> must be one of {UploadDestination.DryRun}, {UploadDestination.Development}, or {UploadDestination.Production}");
					return 1;
			}
			...
			try
			{
				using (var applicationContainer = new ApplicationContainer())
				{
					...
					BookTransfer.Destination = options.Dest;	// must be set before calling BloomParseClient constructor
					...
					{
						case UploadDestination.DryRun:
							Console.WriteLine($"\nThe following actions would happen if you set dest to either '{UploadDestination.Development}' or '{UploadDestination.Production}'");
							break;
						case UploadDestination.Development:
...
[Verb("upload", HelpText = "Upload a book or folder of books to bloomlibrary.org.  A folder that contains exactly one .htm file is interpreted as a book and uploaded." +
	...
	" it uses whatever collection was last open, or the current default.\n"+
...
public class UploadParameters
{
	...
	[Option('d', "dest", HelpText = "If present, this must be one of dry-run, dev, or production. 'dry-run' (the default) will just print out what would happen. 'dev' will upload to dev.bloomlibrary.org (you will need to use an account from there). 'production' will upload to bloomlibrary.org", Required = false)]
