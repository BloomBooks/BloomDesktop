...
namespace Bloom.CLI
{
    ...
    class CreateArtifactsCommand
    {
                ...
                using (var applicationContainer = new ApplicationContainer())
                {
                    ...
                    if (DesktopAnalytics.Analytics.AllowTracking)
                    {
                        throw new ApplicationException(
                            "Allow tracking is enabled but we don't want the Harvester to actually send analytics."
                        );
                    }
        ...
        private static CreateArtifactsExitCode CreateArtifacts(CreateArtifactsParameters parameters)
        {
            ...
            {
                exitCode |= CreateBloomDigitalArtifacts(
                    parameters.BookPath,
                    parameters.Creator,
            ...
            if (
                exitCode == CreateArtifactsExitCode.Success
                && BloomPubMaker.BloomPubFontsAndLangsUsed != null
                && !parameters.NoAnalytics
            )
            {
        ...
        /// </summary>
        private static CreateArtifactsExitCode CreateBloomDigitalArtifacts(
            string bookPath,
            string creator,
            string zippedBloomPubOutputPath,
            string unzippedBloomDigitalOutputPath
        )
        {
#if DEBUG
            ...
            using (var tempBloomPub = TempFile.CreateAndGetPathButDontMakeTheFile())
            {
                ...
                using (var folderForUnzipped = new TemporaryFolder("BloomCreateArtifacts_Unzipped"))
                {
                    ...
                    {
                        SIL.IO.RobustIO.DeleteDirectory(
                            unzippedBloomDigitalOutputPath,
                            recursive: true
                        ); // In case the folder isn't already empty

                        // Ensure directory exists, just in case.
                        Directory.CreateDirectory(
                            Path.GetDirectoryName(unzippedBloomDigitalOutputPath)
                        );
    ...
    [Verb(
        "createArtifacts",
        HelpText = "Create artifacts for a book such as .bloompub, unzipped bloom digital, ePub, etc."
    )]
    public class CreateArtifactsParameters
    {
        ...
        [Option(
            "collectionPath",
            HelpText = "Input path in which to find Bloom collection file for this book",
            Required = true
        )]
        public string CollectionPath { get; set; }

        [Option(
            "bloomSourceOutputPath",
            HelpText = "Output destination path in which to place a .bloomSource file",
            Required = false
        )]
        ...
        [Option(
            "bloomdOutputPath",
            HelpText = "Output destination path in which to place bloompub file [obsolete]",
            Required = false
        )]
        public string BloomDOutputPath
        {
            get { return BloomPubOutputPath; }
            set { BloomPubOutputPath = value; }
        }

        [Option(
            "bloompubOutputPath",
            HelpText = "Output destination path in which to place bloompub file",
            Required = false
        )]
        public string BloomPubOutputPath { get; set; }

        [Option(
            "bloomDigitalOutputPath",
            HelpText = "Output destination path in which to place bloomdigital folder",
            Required = false
        )]
        ...
        [Option(
            "epubOutputPath",
            HelpText = "Output destination path in which to place epub file",
            Required = false
        )]
        public string EpubOutputPath { get; set; }

        [Option(
            "thumbnailOutputInfoPath",
            HelpText = "Output destination path for a text file which contains path information for generated thumbnail files",
            Required = false
        )]
        ...
        [Option(
            ...
            Required = false,
            Default = false,
            HelpText = "Analytics are being sent for testing, not production"
        )]
        ...
        [Option(
            ...
            Required = false,
            Default = false,
            HelpText = "Do not send any analytics"
        )]
        ...
        [Option(
            ...
            Required = false,
            Default = false,
            HelpText = "Do not send analytics for the Epub"
        )]
        ...
        [Option(
            ...
            Required = false,
            Default = false,
            HelpText = "Do not send analytics for the PDF"
        )]
