...
namespace Bloom.MiscUI
{
    ...
    public class StartupScreenManager
    {
        ...
        /// <returns></returns>
        public static IStartupAction AddStartupAction(
            Action task,
            bool shouldHideSplashScreen = false,
            bool lowPriority = false,
            Func<bool> needsToRun = null,
            string waitForMilestone = null,
            int maxTickWaitForMilestone = 0
        )
        {
            var startupAction = new StartupAction()
            {
                Priority = lowPriority ? StartupActionPriority.low : StartupActionPriority.high,
                ShouldHideSplashScreen = shouldHideSplashScreen,
                Task = task,
        ...
        {
            if (
                _startupActions.FirstOrDefault(a => a.Enabled) == null
        ...
        private static void DoStartupAction(object sender, EventArgs e)
        {
            ...
            )
            {
                ...
                {
                    Logger.WriteEvent(
                        "Failed to get to startup milestone "
                            + _current.WaitForMilestone
                            + " after five seconds"
                    );
        ...
        private static void CloseFastSplashScreen()
        {
            ...
            else if (SIL.PlatformUtilities.Platform.IsWindows)
            {
                ...
                using (
                    var closeSplashEvent = new EventWaitHandle(
                        false,
                        EventResetMode.ManualReset,
                        "CloseSquirrelSplashScreenEvent"
                    )
                )
                {
        ...
        {
            // We just want to do the following steps if _splashForm is there but disposed, as this will cause errors.
