...
namespace Bloom.MiscUI
{
	...
	public class StartupScreenManager
	{
		...
		public static IStartupAction AddStartupAction(Action task, bool shouldHideSplashScreen = false, bool lowPriority = false,
			Func<bool> needsToRun = null, string waitForMilestone=null, int tickDelayForMilestone = 0)
		{
			var startupAction = new StartupAction()
				{ Priority = lowPriority ? StartupActionPriority.low : StartupActionPriority.high,
					...
					WaitForMilestone = waitForMilestone, TickDelayForMilestone = tickDelayForMilestone };
		...
		private static void DoStartupAction(object sender, EventArgs e)
		{
			...
			if (_current.WaitForMilestone != null && !_milestones.Contains(_current.WaitForMilestone))
			{
				...
				// on the milestone.
				if (_current.TickDelayForMilestone > 0)
				{
					_current.TickDelayForMilestone--;
		...
		private class StartupAction : IStartupAction
		{
			...
			public int TickDelayForMilestone;
