...
namespace Bloom
{
	...
	{
		private const string Restoresvghere = "****RestoreSvgHere****";
		...
		{
			// Tidy chokes on embedded svgs so just take them out.
			content = new Regex("<svg .*?</svg>", RegexOptions.Singleline).Replace(content, "");
		...
		public static string RemoveSvgs(string input, List<string> svgs)
		{
			...
			for (var matchSvg = regexSvg.Match(result); matchSvg.Success; matchSvg = regexSvg.Match(result))
			{
				...
				result = result.Substring(0, matchSvg.Index) + Restoresvghere +
		...
		public static string RestoreSvgs(string input, List<string> svgs)
		{
			...
			foreach (var svg in svgs)
			{
				...
				int index = result.IndexOf(Restoresvghere);
				result = result.Substring(0, index) + svg + result.Substring(index + Restoresvghere.Length);
