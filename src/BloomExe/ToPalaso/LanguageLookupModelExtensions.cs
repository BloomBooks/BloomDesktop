	...
	public static class LanguageLookupModelExtensions
	{
		...
		public static string GetLocalizedLanguageName(this LanguageLookupModel isoModel, string code, string uiCode)
		{
			...
			try
			{
				...
				if (!langName.StartsWith("Unknown Language"))
				{
			...
			if (generalUiCode == "en" || String.IsNullOrWhiteSpace(langName))
				langName = GetBestEnglishName(isoModel, code);
			_mapIsoCodesToLanguageName.Add(key, langName);
			return langName;
		}
		...
		/// </summary>
		/// <remarks>
		/// This might be easier to implement reliably with ICU for a larger set of languages, but we can't
		/// use that approach for the reasons discribed in the previous method..
		/// </remarks>
