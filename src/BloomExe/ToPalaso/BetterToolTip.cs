...
namespace Bloom.ToPalaso
{
    ...
    public class BetterToolTip : ToolTip, ILocalizableComponent
    {
        ...
        private Dictionary<Control, string> _toolTipWhenDisabled =
        ...
        > _betterTooltipTransparentOverlay =
            new Dictionary<Control, BetterTooltipTransparentOverlay>();
        private Dictionary<Control, PaintEventHandler> _paintEventHandlers =
            new Dictionary<Control, PaintEventHandler>();
        private Dictionary<Control, EventHandler> _controlEnabledChangedHandlers =
            new Dictionary<Control, EventHandler>();
        private Dictionary<Control, Size> _sizeOfToolTipWhenDisabledDictionary =
...
#if DEBUG
                Debug.Fail(
                    "Debug Only: If you just changed the UI language, this is a BL-938 Reproduction"
                );
        ...
        public void SetToolTipWhenDisabled(Control control, string value)
        {
            ...
            if (!String.IsNullOrEmpty(value))
            {
                ...
                _controlEnabledChangedHandlers.Add(
                    control,
                    new EventHandler(control_EnabledChanged)
                );
        ...
        [Category("Misc")]
        [Description(
            "Determines the ToolTip shown when the mouse hovers over the disabled control."
        )]
        [Localizable(true)]
        ...
        public IEnumerable<LocalizingInfo> GetAllLocalizingInfoObjects(L10NSharpExtender extender)
        {
            ...
            foreach (var ctrl in _allControlsHavingToolTips)
            {
                ...
                if (!string.IsNullOrEmpty(normalTip))
                {
                    ...
                    {
                        Text = normalTip,
                        Category = LocalizationCategory.LocalizableComponent
                    };
                    result.Add(liNormal);
                }
        ...
        [Category("Misc")]
        [Description(
            "Determines the size of the ToolTip when the control is disabled."
