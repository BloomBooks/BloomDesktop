...
namespace Bloom.ToPalaso
{
    class SafeInvoke
    {
        ...
        public static void Invoke(
            string nameForErrorReporting,
            Control control,
            bool forceSynchronous,
            bool throwIfAnythingGoesWrong,
            Action action
        )
        {
            ...
            try
            {
                if (control.IsDisposed)
                {
                    throw new ObjectDisposedException(
                        "Control is already disposed. (" + nameForErrorReporting + ")"
                    );
                }
                if (control.InvokeRequired)
                {
                    var delgate = (Action)
                        ...
                        {
                            Invoke(
                                nameForErrorReporting,
                                control,
                                forceSynchronous,
                                throwIfAnythingGoesWrong,
                                action
                            );
                        };
                    if (forceSynchronous)
                ...
                else
                {
                    ...
                    if (!control.IsHandleCreated)
                    {
                        ...
                        throw new ApplicationException(
                            "SafeInvoke.Invoke apparently called before control created ("
                                + nameForErrorReporting
                                + ")"
                        );
            ...
            catch (Exception error)
            {
                ...
                {
                    Debug.Fail(
                        "This error would be swallowed in release version: " + error.Message
                    );
        ...
        /// </summary>
        public static void InvokeIfPossible(
            string nameForErrorReporting,
            Control control,
            bool forceSynchronous,
            Action action
