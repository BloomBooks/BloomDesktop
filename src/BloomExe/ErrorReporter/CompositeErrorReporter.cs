...
namespace Bloom.ErrorReporter
{
	...
	class CompositeErrorReporter : IBloomErrorReporter
	{
		...
		public void NotifyUserOfProblem(string message, Exception exception, IRepeatNoticePolicy policy,
			AllowSendReport allowSendReport,
			string extraButtonLabel, Action<string, Exception> onExtraButtonClicked)
		{
			foreach (var reporter in Reporters)
			{
				reporter.NotifyUserOfProblem(message, exception, policy, allowSendReport, extraButtonLabel, onExtraButtonClicked);
			}
		}
