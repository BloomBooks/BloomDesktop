	...
	public class ErrorReportUtils
	{
		...
		private static void CheckForFakeTestErrors(string title)
		{
			...
			else if (title == "Error NotifyUser Report Retry")
			{
				...
				BloomErrorReport.NotifyUserOfProblemCustom(fakeProblemMessage, fakeException, new ShowAlwaysPolicy(), AllowSendReport.Default, secondaryButtonLabel, ErrorReportUtils.TestAction);
			}
