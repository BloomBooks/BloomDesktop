	...
	public class ErrorReportUtils
	{
		...
		private static void CheckForFakeTestErrors(string title)
		{
			...
			else if (title == "Error NotifyUser NoReport 3")
			{
				...
				BloomErrorReport.NotifyUserOfProblemCustom(fakeProblemMessage);
			}
			...
			else if (title == "Error NotifyUser Report Retry")
			{
				...
				BloomErrorReport.NotifyUserOfProblemCustom(fakeProblemMessage, fakeException, new ShowAlwaysPolicy(), extraButtonLabel: secondaryButtonLabel, onExtraButtonPressed: ErrorReportUtils.TestAction);
			}
