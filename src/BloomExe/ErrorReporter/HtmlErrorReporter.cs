...
namespace Bloom.ErrorReporter
{
	...
	public class HtmlErrorReporter: IErrorReporter, IBloomErrorReporter
	{
		...
		public void NotifyUserOfProblem(IRepeatNoticePolicy policy, Exception exception, string message)
			=> NotifyUserOfProblem(message, exception, policy);
		...
		public void NotifyUserOfProblem(string message, Exception exception = null, IRepeatNoticePolicy policy = null,
		...
		{
			bool shouldShowReportButton = allowSendReport.IsSendReportAllowed(new AllowSendReportContext(exception));
