...
namespace Bloom.ErrorReporter
{
	internal class BloomErrorReport : SIL.Reporting.ErrorReport
	{
		...
		/// 
		/// <param name="extraButtonLabel">Optional: The label of the extra button (this is separate from the report button). "" means disabled. null means default, which is disabled.</param>
			...
			string extraButtonLabel = null, Action<string, Exception> onExtraButtonPressed = null)
		{
			...
			NotifyUserOfProblemWrapper(message, exception, () =>
			{
				...
				{
					// Normal situation
					((IBloomErrorReporter)errorReporter).NotifyUserOfProblem(message, exception, policy,
