...
namespace Bloom.ErrorReporter
{
	internal class BloomErrorReport : SIL.Reporting.ErrorReport
	{
		...
		public static void NotifyUserOfProblem(string message, NotifyUserOfProblemSettings settings, IRepeatNoticePolicy policy = null)
		...
		/// <param name="settings">Optional: The customization settings controlling which/how extra buttons show up</param>
		/// <param name="policy">Optional: The policy to use to decide whether to show the notification. Defaults to ShowAlwaysPolicy</param>
		public static void NotifyUserOfProblem(string message, Exception exception, NotifyUserOfProblemSettings settings, IRepeatNoticePolicy policy = null)
		{
			if (policy == null)
			...
			NotifyUserOfProblemWrapper(message, exception, () =>
			{
				...
				{
					// Normal situation
					((IBloomErrorReporter)errorReporter).NotifyUserOfProblem(message, exception, settings, policy);
				}
				else
				{
