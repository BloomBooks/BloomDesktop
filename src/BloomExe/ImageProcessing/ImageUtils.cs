...
namespace Bloom.ImageProcessing
{
	static class ImageUtils
	{
		...
		public static Image ResizeImageIfNecessary(Size maxSize, Image image, bool dashedBorder)
		{
			return DrawResizedImage(maxSize, image, false, dashedBorder);
		}

		public static Image CenterImageIfNecessary(Size size, Image image, bool dashedBorder)
		{
			return DrawResizedImage(size, image, true, dashedBorder);
		}
		...
		/// </remarks>
		private static Image DrawResizedImage(Size maxSize, Image image, bool centerImage, bool dashedBorder)
		{
			...
			using (var graphic = Graphics.FromImage(newImage))
			{
				...
				// (the second answer).
				// Review: I'm just refactoring what's already here, but I don't see how x and y can ever be anything but zero.
