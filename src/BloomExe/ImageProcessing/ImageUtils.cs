			...
			if ((imageInfo.Image.PixelFormat & PixelFormat.Indexed) == PixelFormat.Indexed)
			{
				...
				if (palette != null && palette.Entries != null)
				{
					...
					if (palette.Entries.Length == 2)
					{
						...
						//Console.WriteLine("DEBUG ShouldMakeBackgroundTransparent(\"{0}\"): indexed image with {1} colors, white {2}found",
						//	imageInfo.FileName, palette.Entries.Length, whiteFound ? "" : "not ");
						return whiteFound;
					}
					else
					{
						//Console.WriteLine("DEBUG ShouldMakeBackgroundTransparent(\"{0}\"): indexed image with {1} colors",
			...
			if (imageInfo.Image is Bitmap bitmapImage)
			{
				...
				for (int j = 0, y = yDelta / 2; y < bitmapImage.Height; y += yDelta, ++j)
				{
					...
					for (int i = 0, x = xDelta / 2; x < bitmapImage.Width; x += xDelta, ++i)
					{
						...
						if (color.A < 255)
						{
							//Console.WriteLine("DEBUG ShouldMakeBackgroundTransparent(\"{0}\"): {1} pixels examined before a transparent pixel found",
						...
						else if (color != color1 && color != color2)
						{
							...
							//var colorString = $"[{color1.R},{color1.G},{color1.B}], [{color2.R},{color2.G},{color2.B}], [{color.R},{color.G},{color.B}]";
							//Console.WriteLine("DEBUG ShouldMakeBackgroundTransparent(\"{0}\"): {1} pixels examined before 3 colors found {2}",
							//	imageInfo.FileName, pixelsExamined, colorString);
				...
				{
					++colorCount;
					//colors = $"; [{color1.R},{color1.G},{color1.B}]";
				...
				{
					++colorCount;
					//colors = colors + $", [{color2.R},{color2.G},{color2.B}]";
					if (IsNearWhite(color2))
						whiteFound = true;
				}
				//foreach (var color in grays)
				//{
				//	if (color != color1 && color != color2)
				//		colors = colors + $", [{color.R},{color.G},{color.B}]";
				//}
				//Console.WriteLine("DEBUG ShouldMakeBackgroundTransparent(\"{0}\"): {1} pixels examined, {2} colors found, white {3}found{4}",
				//	imageInfo.FileName, pixelsExamined, colorCount, whiteFound ? "" : "not ", colors);
