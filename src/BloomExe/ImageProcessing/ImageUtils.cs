	...
	class ImageUtils
	{
		...
		public static void SaveAsTopQualityJpeg(Image image, string destinationPath)
		{
			...
			using (var safetyImage = new Bitmap(image))
			{
				...
				string tempPath = Path.Combine(Path.GetDirectoryName(destinationPath), Path.GetFileName(temp.Path));
				using(var parameters = new EncoderParameters(1))
				{
					//0 = max compression, 100 = least
					parameters.Param[0] = new EncoderParameter(encoder, 100L);
					SIL.IO.RobustIO.SaveImage(safetyImage, tempPath, jpgEncoder, parameters);
				}
				SIL.IO.FileUtils.ReplaceFileWithUserInteractionIfNeeded(tempPath, destinationPath, null);
			}
		}
