...
namespace Bloom.Publish.Rab
{
    ...
    public class RabProjectService
    {
        ...
        private void MoveBuiltApkToCollectionOutput(RabWorkspacePaths paths, string stagedApkPath)
        {
            ...
            try
            {
                var stagedDirectory = Path.GetDirectoryName(stagedApkPath);
                if (
                    !string.IsNullOrWhiteSpace(stagedDirectory) && Directory.Exists(stagedDirectory)
                )
                    RobustIO.DeleteDirectory(stagedDirectory, true);
            }
            catch (Exception) { }
        }
