...
namespace Bloom.Publish.BloomPub
{
    ...
    public class BulkBloomPubPublishSettings
    {
        ...
        {
            var settingsElement = xml.Elements()
                .Where(e => e.Name == kXmlAttrName)
                .FirstOrDefault();
            ...
            publishSettings.makeBookshelfFile = CollectionSettings.ReadBoolean(
                settingsElement,
                "MakeBookshelfFile",
                true
            );
            publishSettings.makeBloomBundle = CollectionSettings.ReadBoolean(
                settingsElement,
                "MakeBloomBundle",
                true
            );
            publishSettings.bookshelfColor = CollectionSettings.ReadString(
                settingsElement,
                "BookshelfColor",
                Palette.kBloomLightBlueHex
            );
            ...
            publishSettings.distributionTag = CollectionSettings.ReadString(
                settingsElement,
                "DistributionTag",
                ""
            );
            publishSettings.bookshelfLabel = CollectionSettings.ReadString(
                settingsElement,
                "BookshelfLabel",
                ""
            );
