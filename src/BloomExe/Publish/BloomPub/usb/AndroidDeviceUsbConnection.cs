...
namespace Bloom.Publish.BloomPub.usb
{
    ...
    {
        public OneReadyDeviceNotFoundEventArgs(
    ...
    class AndroidDeviceUsbConnection
    {
        ...
        /// </summary>
        public void ConnectAndSendToOneDevice(
            Book.Book book,
            Color backColor,
        ...
        public void SendBook(string bloomPubPath)
        {
            ...
            using (
                var targetStream = _device.OpenWrite(
                    Path.Combine(_bloomFolderPath, "something.modified"),
                    buffer.Length,
                    true
                )
            )
        ...
        private bool ConnectAndSendToOneDeviceInternal(
        ...
        )
        {
            ...
            foreach (var device in devices)
            {
                ...
                if (androidFolderPath != null)
                {
                    ...
                    _bloomFolderPath =
                        Path.GetDirectoryName(androidFolderPath) + "\\" + kBloomFolderOnDevice;
                    _device = device;
                }
            }
