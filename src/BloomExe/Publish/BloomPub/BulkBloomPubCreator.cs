...
namespace Bloom.Publish.BloomPub
{
    public class BulkBloomPubCreator
    {
        ...
        public delegate BulkBloomPubCreator Factory(
            BookServer bookServer,
            CollectionModel collectionModel,
            BloomWebSocketServer webSocketServer
        ); //autofac uses this

        public BulkBloomPubCreator(
            BookServer bookServer,
            CollectionModel collectionModel,
            BloomWebSocketServer webSocketServer
        )
            ...
            await BrowserProgressDialog.DoWorkWithProgressDialogAsync(
                ...
                async (progress, worker) =>
                {
                    ...
                    var filenameWithoutExtension =
                        _collectionModel.CollectionSettings.DefaultBookshelf.SanitizeFilename(
                            ' ',
                            true
                        );
                    ;
                    if (bulkSaveSettings.makeBookshelfFile)
                    {
                        ...
                        System.Diagnostics.Debug.Assert(
                            !bulkSaveSettings.bookshelfColor.Contains("\n")
                                && !bulkSaveSettings.bookshelfColor.Contains("\r"),
                            "(BL-10190 Repro) Invalid bookshelfColor setting (contains newline). Please investigate!"
                        );
                        var colorString = getBloomReaderColorString(
                            bulkSaveSettings.bookshelfColor
                        );
                        System.Diagnostics.Debug.Assert(
                            !colorString.Contains("\n") && !colorString.Contains("\r"),
                            "(BL-10190 Repro) Invalid computed colorString value (contains newline). Please investigate!"
                        );
                        ...
                        var json = template
                            ...
                            .Replace(
                                "bookshelf-name",
                                bulkSaveSettings.bookshelfLabel.Replace('"', '\'')
                            )
                            .Replace(
                                "id-of-the-bookshelf",
                                _collectionModel.CollectionSettings.DefaultBookshelf
                            )
                    ...
                    foreach (
                        var bookInfo in _collectionModel.TheOneEditableCollection.GetBookInfos()
                    )
                    {
                        ...
                        progress.MessageWithoutLocalizing(
                            $"Making BloomPUB for {bookInfo.QuickTitleUserDisplay}...",
                        ...
                        if (bulkSaveSettings.makeBookshelfFile)
                        {
                            settings.BookshelfTag = _collectionModel
                                .CollectionSettings
                                .DefaultBookshelf;
                        }
                    ...
                    {
                        var bloomBundlePath = Path.Combine(
                            dest.FolderPath,
                            $"{filenameWithoutExtension}.bloombundle"
                        );

                        var bloomBundleFile = new BloomTarArchive(bloomBundlePath);
                        bloomBundleFile.AddDirectoryContents(
                            dest.FolderPath,
                            new string[] { ".bloombundle" }
                        );
