...
//			_model.BookletPortion = PublishModel.BookletPortions.BookletPages;
		...
		public void SetDisplayMode(PublishModel.DisplayModes displayMode)
		{
			...
			switch (displayMode)
			{
				...
				case PublishModel.DisplayModes.EPUB:
					...
					var pathToHtml = BloomFileLocator.GetBrowserFile(false, "publish", "epub", "epubPublishUI.html");
					var panel = new EpubPublishPanel(pathToHtml, _model, _model.ThumbNailer, _model.BookServer);
					panel.CollectionSettings = _model.CollectionSettings;
					panel.WebSocketServer = _webSocketServer;
					panel.BookSelection = _model.BookSelection;
					...
					_publishEpubApi.CurrentView = panel;
					ShowHtmlPanel(pathToHtml, panel);
					panel.Initialize();
					break;
			}
			UpdateSaveButton();
		...
		private void ShowHtmlPanel(string pathToHtml, HtmlPublishPanel panel = null)
		{
