		...
		private void UpdateDisplayMode()
		{
			if (IsHandleCreated) // May not be when bulk uploading
			{
				if (_uploadRadio.Checked)
					_model.DisplayMode = PublishModel.DisplayModes.Upload; 	// We CAN upload even without a preview, and epubs are fine.
...
//			_layoutChoices.SelectedText = _model.BookSelection.CurrentSelection.GetLayout().ToString();
		...
		private void SetupEpubControl()
		{
			...
			// All this of course has nothing to do with S3. BloomS3Client just happened to have a CopyDirectory
			// method I could re-use. It should be moved to some utility class, probably in libpalaso.
			BloomS3Client.CopyDirectory(root, tempFolder);
