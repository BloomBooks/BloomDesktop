	...
	public class AudioProcessor
	{
		...
		public static string MergeAudioFiles(IEnumerable<string> mergeFiles, string combinedAudioPath)
		{
			...
			if (RobustFile.Exists(ffmpeg))
			{
				...
				var monoFiles = EnsureConsistentInputFiles(ffmpeg, mergeFiles);
				try
				{
		...
		/// </remarks>
		private static IEnumerable<string> EnsureConsistentInputFiles(string ffmpeg, IEnumerable<string> mergeFiles)
		{
			...
			{
				var args = String.Format("-hide_banner -i \"{0}\" -f null -", file);
				...
				if (match.Success)
				{
					...
					if (recordType == "stereo" || recordRate != "44100")
					{
						...
						tempFile.Detach();
						args = String.Format("-i \"{0}\" -ac 1 -ar 44100 -b:a {1}k \"{2}.mp3\"", file, bitRate, tempFile.Path);
