	...
	public class AudioProcessor
	{
		...
		public static string MergeAudioFiles(IEnumerable<string> mergeFiles, string combinedAudioPath)
		{
			...
			if (PatientFile.Exists(ffmpeg))
			{
				...
				try
				{
					...
					var result = CommandLineRunnerExtra.RunWithInvariantCulture(ffmpeg, argsBuilder.ToString(), "", 60 * 10, new NullProgress());
		...
		private static IEnumerable<string> TryEnsureConsistentInputFiles(string ffmpeg, IEnumerable<string> mergeFiles, out IEnumerable<string> mergeFileSpecs)
		{
			...
			foreach (var file in mergeFiles)
			{
				...
				var result = CommandLineRunnerExtra.RunWithInvariantCulture(ffmpeg, args, "", 60, new NullProgress());
				...
				if (match.Success)
				{
					...
					if (recordType == "stereo" || recordRate != "44100")
					{
						...
						result = CommandLineRunnerExtra.RunWithInvariantCulture(ffmpeg, args, "", 120, new NullProgress());
						if (result.ExitCode == 0)
						{
