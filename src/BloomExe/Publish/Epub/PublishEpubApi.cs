...
namespace Bloom.Publish.Epub
{
	...
	public class PublishEpubApi
	{
		...
		{
			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "save", request =>
			{
				{
			...
			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "epubSettings", request =>
			{
			...
			server.ApiHandler.RegisterEnumEndpointHandler(kApiUrlPart + "imageDescriptionSetting",
			...
			server.ApiHandler.RegisterBooleanEndpointHandler(kApiUrlPart + "removeFontSizesSetting",
			...
			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "updatePreview", request =>
			{
				RefreshPreview(_desiredEpubSettings);
				request.PostSucceeded();
			}, false); // in fact, must NOT be on UI thread

			server.ApiHandler.RegisterEndpointHandler(kApiUrlPart + "abortPreview", request =>
			{
				lock (_epubMakerLock)
				{
