	...
	public class EpubMaker : IDisposable
	{
		...
		public void StageEpub(WebSocketProgress progress, bool publishWithoutAudio = false)
		{
			...
			foreach (XmlElement pageElement in Book.GetPageElements())
			{
				...
				if (AbortRequested)
					break;
				if (MakePageFile(pageElement, ref warningMessages))
				{
		...
		/// </remarks>
		private bool MakePageFile(XmlElement pageElement, ref ISet<string> warningMessages)
		{
			...
			_publishHelper.RemoveUnwantedContent(pageDom, this.Book, ref warningMessages, this);
