...
namespace Bloom.Publish.Epub
{
	...
	public class EpubMaker : IDisposable
	{
		...
		private void FixDivOrdering(HtmlDom pageDom)
		{
			...
			{
				// Ensure the local language title precedes the national language title, the national language
				// title precedes the regional language title, and the regional language title precedes the
				// "other" language title.  (This code contains a bit of "future-proofing".)
				...
				var regionalDiv = titleDiv.SelectSingleNode("./div[contains(@class, 'bloom-contentNational2')]") as XmlElement;
				var otherlangDiv = titleDiv.SelectSingleNode("./div[contains(@class, 'bloom-contentNational3')]") as XmlElement;
				if (regionalDiv != null)
				{
					if (otherlangDiv != null)
						titleDiv.InsertBefore(regionalDiv, otherlangDiv);
				}
				if (nationalDiv != null)
				{
					if (regionalDiv != null)
						titleDiv.InsertBefore(nationalDiv, regionalDiv);
					else if (otherlangDiv != null)
						titleDiv.InsertBefore(nationalDiv, otherlangDiv);
				}
				if (localDiv != null)
				{
					if (nationalDiv != null)
						...
						titleDiv.InsertBefore(localDiv, regionalDiv);
					else if (otherlangDiv != null)
						titleDiv.InsertBefore(localDiv, otherlangDiv);
				}
			}
		}
