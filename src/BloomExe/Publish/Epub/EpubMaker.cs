			...
			foreach(var audioSentenceElement in audioSentenceElementsWithRecordedAudio)
			{
				...
				else
				{
					try
					{
						...
						if (timeString == null || !TimeSpan.TryParseExact(timeString, @"hh\:mm\:ss\.ff", null, TimeSpanStyles.None, out clipTimeSpan))
						{
							throw new Exception();	// bogus, but avoids duplicating error handling code below.
						}
#else
