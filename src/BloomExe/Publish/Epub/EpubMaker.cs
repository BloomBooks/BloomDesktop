...
namespace Bloom.Publish.Epub
{
	...
	public class EpubMaker : IDisposable
	{
		...
		private bool IsBlankPage(XmlElement pageElement)
		{
			...
			foreach (XmlElement vid in pageElement.SafeSelectNodes("//div[contains(@class,'bloom-videoContainer')]/video/source").Cast<XmlElement>())
			{
		...
		{
			foreach (XmlElement vid in pageDom.SafeSelectNodes("//div[contains(@class,'bloom-videoContainer')]/video/source").Cast<XmlElement>())
			{
				var src = vid.GetOptionalStringAttribute("src", null);
				if (String.IsNullOrEmpty(src))
					continue;
