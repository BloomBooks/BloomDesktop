...
namespace Bloom.Publish.Epub
{
	...
	public class EpubMaker : IDisposable
	{
		...
		private void AddAccessibilityMetadata(XElement metadataElt, XNamespace opf, BookMetaData metadata)
		{
			...
			// Hazards section
			if (!hasAudio && !hasVideo) // Can be no hazard
			{
			...
			{
				// motion hazard ("Motion" books don't exist in ePUB-land yet, so just base this on video)
				if (!hasVideo || !string.IsNullOrEmpty(metadata.Hazards) && metadata.Hazards.Contains("noMotionSimulationHazard"))
				{
					metadataElt.Add(new XElement(opf + "meta", new XAttribute("property", "schema:accessibilityHazard"), "noMotionSimulationHazard"));
				}
				else if (!string.IsNullOrEmpty(metadata.Hazards) && metadata.Hazards.Contains("motionSimulationHazard"))
				{
					metadataElt.Add(new XElement(opf + "meta", new XAttribute("property", "schema:accessibilityHazard"), "motionSimulationHazard"));
				}

				// flashing hazard
				if (!hasVideo || !string.IsNullOrEmpty(metadata.Hazards) && metadata.Hazards.Contains("noFlashingHazard"))
				{
					metadataElt.Add(new XElement(opf + "meta", new XAttribute("property", "schema:accessibilityHazard"), "noFlashingHazard"));
				}
				else if (!string.IsNullOrEmpty(metadata.Hazards) && metadata.Hazards.Contains("flashingHazard"))
				{
					metadataElt.Add(new XElement(opf + "meta", new XAttribute("property", "schema:accessibilityHazard"), "flashingHazard"));
				}

				// sound hazard
				if (!hasAudio || !string.IsNullOrEmpty(metadata.Hazards) && metadata.Hazards.Contains("noSoundHazard"))
				{
					metadataElt.Add(new XElement(opf + "meta", new XAttribute("property", "schema:accessibilityHazard"), "noSoundHazard"));
				}
				else if (!string.IsNullOrEmpty(metadata.Hazards) && metadata.Hazards.Contains("soundHazard"))
				{
