	...
	public class EpubMaker : IDisposable
	{
		...
		public void StageEpub(WebSocketProgress progress, bool publishWithoutAudio = false)
		{
			...
			if (Platform.IsWindows)
			{
				...
				var longestName = Path.GetFileName(DirectoryUtils.GetLongestPathname(_book.FolderPath));
				...
				if (maxPathNeeded >= kPathMax)
				{
					...
					var needToShrink = kPathMax - maxPathNeeded;
					if (needToShrink < id64.Length)
						BookInStagingFolder = Path.Combine(_outerStagingFolder.FolderPath, id64.Substring(0, needToShrink));
					else
