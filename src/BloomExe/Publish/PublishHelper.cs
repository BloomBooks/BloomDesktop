	...
	public class PublishHelper : IDisposable
	{
		...
		private void RemoveUnwantedContentInternal(HtmlDom dom, Book.Book book, bool removeInactiveLanguages, EpubMaker epubMaker, ISet<string> warningMessages, bool keepPageLabels = false)
		{
				...
				false))
			{
				...
				if (epubMaker != null)
				{
					// BL-11513: We might end up with an ePUB that hasn't had everything removed that should be...
					// but if we don't get out of here, we'll just keep saying "Building ePUB" every few seconds
					// and be stuck. I tried increasing the timeout on the 'NavigateAndWaitTillDone()', but that
					// didn't seem to help. Trying to continue on is just asking for trouble in 'IsDisplayed(elt)'
					// below.
					Cancel();
					return;
				}
