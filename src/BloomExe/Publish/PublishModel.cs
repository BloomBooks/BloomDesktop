...
namespace Bloom.Publish
{
	...
	public class PublishModel : IDisposable
	{
		...
		{
			get { return _currentlyLoadedBook.GetLanguage(LanguageSlot.Language1).IsRightToLeft;  }
		}
...
//			System.Diagnostics.Process.Start(tempHtml.Path);
		...
		public IEnumerable<ToolStripItem> GetExtensionMenuItems()
		{
			...
			if (SHRP_PupilBookExtension.ExtensionIsApplicable(BookSelection.CurrentSelection))
			{
				...
				container.ComposeExportedValue<string>("Language1Iso639Code", _currentlyLoadedBook.GetLanguage(LanguageSlot.Language1).Iso639Code);
			...
			foreach (XmlElement pageElement in this.BookSelection.CurrentSelection.GetPageElements())
			{
				...
				try
				{
					...
					if (mergeFiles.Any())
					{
						...
						//}
						var filename =
							$"{this.BookSelection.CurrentSelection.Title}_{this._currentlyLoadedBook.GetLanguage(LanguageSlot.Language1).Name}_{pageIndex:0000}.mp3".Replace(' ','_');
