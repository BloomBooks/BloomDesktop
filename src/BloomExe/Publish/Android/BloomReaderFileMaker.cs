	...
	public static class BloomReaderFileMaker
	{
			...
			WebSocketProgress progress, bool hasEnterpriseFeatures)
		{
			...
			if (hasEnterpriseFeatures)
				ProcessInteractivePages(modifiedBookFolderPath, modifiedBook.RawDom);
			...
			using (var helper = new PublishHelper())
			{
				...
				helper.RemoveUnwantedContent(modifiedBook.OurHtmlDom, modifiedBook, ref warningMessages);
		...
		private static void ProcessInteractivePages(string bookFolderPath, XmlDocument bookDom)
		{
