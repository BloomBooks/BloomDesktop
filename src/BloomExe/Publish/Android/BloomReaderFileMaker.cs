	...
	public static class BloomReaderFileMaker
	{
			...
			WebSocketProgress progress, string creator=kCreatorBloom, AndroidPublishSettings settings = null)
		{
			...
			if (settings?.LanguagesToInclude != null)
				PublishModel.RemoveUnwantedLanguageData(modifiedBook.OurHtmlDom, settings.LanguagesToInclude, modifiedBook.GetLanguage(LanguageSlot.Language2).Iso639Code);
			else if (Program.RunningHarvesterMode && modifiedBook.OurHtmlDom.SelectSingleNode(BookStorage.ComicalXpath) != null)
			{
				...
				var languagesToInclude = new string[1] { modifiedBook.GetLanguage(LanguageSlot.Language1).Iso639Code };
				PublishModel.RemoveUnwantedLanguageData(modifiedBook.OurHtmlDom, languagesToInclude, modifiedBook.GetLanguage(LanguageSlot.Language2).Iso639Code);
			}
