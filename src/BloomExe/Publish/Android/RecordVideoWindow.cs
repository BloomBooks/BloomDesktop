...
{
	/// <summary>
	/// This class manages a windows that appears when the 'Record' button is pressed in the
	/// Video sub-pane of the Publish tab. The entire content of the windows is (and must remain)
	...
	public partial class RecordVideoWindow : Form
	{
		...
		public void StartFfmpeg()
		{
			...
			// Todo Linux: gdigrab is Windows-only, we'll need to find something else.
			...
			_ffmpegProcess.Start(); // recording now!
			_startTime = DateTime.Now;
		...
		public void StopRecording(string soundLogJson)
		{
			...
			// We may not need this approach here, since we're not using 'q' to force an early exit.
			_ffmpegProcess.BeginOutputReadLine();
			_ffmpegProcess.BeginErrorReadLine();
			_ffmpegProcess.WaitForExit();
		...
		// Some logic I found...combined with increating the MaximumSize of the window, it allows
