	...
	public class PublishToAndroidApi
	{
		...
		private AndroidPublishSettings GetSettings()
		{
			return new AndroidPublishSettings()
			{
				...
				LanguagesToInclude = new HashSet<string>(_bookForLanguagesToPublish.BookInfo.MetaData.TextLangsToPublish.ForBloomReader.Where(kvp => kvp.Value.IsIncluded()).Select(kvp => kvp.Key)),
				AudioLanguagesToExclude = new HashSet<string>(_bookForLanguagesToPublish.BookInfo.MetaData.AudioLangsToPublish.ForBloomReader.Where(kvp => !kvp.Value.IsIncluded()).Select(kvp => kvp.Key))
			};
		}
		...
		public void RegisterWithApiHandler(BloomApiHandler apiHandler)
		{
			...
			apiHandler.RegisterEndpointHandler(kApiUrlPart + "languagesInBook", request =>
			{
				try
				{
					...
					var result = "[" + string.Join(",", _allLanguages.Select(kvp =>
					{
						...
						{
							includeText = includeTextValue.IsIncluded();
						}
						...
						{
							includeAudio = includeAudioValue.IsIncluded();
						}
			...
			apiHandler.RegisterEndpointHandler(kApiUrlPart + "includeLanguage", request =>
			{
				...
				if (request.HttpMethod == HttpMethods.Post)
				{
					...
					{
						var checkboxVal = includeTextValue == "true" ? LangToPublishCheckboxValue.Include : LangToPublishCheckboxValue.Exclude;
						request.CurrentBook.BookInfo.MetaData.TextLangsToPublish.ForBloomReader[langCode] = checkboxVal;
					}

					var includeAudioValue = request.GetParamOrNull("includeAudio");
					if (includeAudioValue != null)
					{
						var checkboxVal = includeAudioValue == "true" ? LangToPublishCheckboxValue.Include : LangToPublishCheckboxValue.Exclude;
						request.CurrentBook.BookInfo.MetaData.AudioLangsToPublish.ForBloomReader[langCode] = checkboxVal;
					}
		...
		private void InitializeLanguagesInBook(ApiRequest request)
		{
			lock (_lockForLanguages)
			{
				...
				foreach (var kvp in _allLanguages)
				{
					...
					if (_bookForLanguagesToPublish.BookInfo.MetaData.TextLangsToPublish.ForBloomReader.TryGetValue(langCode, out LangToPublishCheckboxValue checkboxValFromSettings))
					{
						if (checkboxValFromSettings.IsUserSpecified())
						{
							continue;
						}
					}
					...
					var newInitialValue = isChecked ? LangToPublishCheckboxValue.IncludeByDefault : LangToPublishCheckboxValue.ExcludeByDefault;
					_bookForLanguagesToPublish.BookInfo.MetaData.TextLangsToPublish.ForBloomReader[langCode] = newInitialValue;
				}
				...
				if (_bookForLanguagesToPublish.BookInfo.MetaData.AudioLangsToPublish.ForBloomReader == null)
				{
					...
					{
						_bookForLanguagesToPublish.BookInfo.MetaData.AudioLangsToPublish.ForBloomReader[langCode] = LangToPublishCheckboxValue.IncludeByDefault;
					}					
				}
