		...
		public void RegisterWithApiHandler(BloomApiHandler apiHandler)
		{
			...
			apiHandler.RegisterEndpointHandler(kApiUrlPart + "languagesInBook", request =>
			{
				...
				foreach (var kvp in allLanguages)
				{
					...
					sb.Append($"{{\"code\":\"{kvp.Key}\", \"name\":\"{request.CurrentBook.PrettyPrintLanguage((kvp.Key))}\",\"complete\":{complete},\"include\":{include}}},");
				}

				sb.Remove(sb.Length - 1, 1); // drop final comma

				sb.Append("]");

				request.ReplyWithText(sb.ToString());
			}, false);
