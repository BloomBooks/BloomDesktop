	...
	public class UsbPublisher
	{
		...
		public void Connect(Book.Book book, Color backColor)
		{
			try
			{
				...
				_progress.Message(idSuffix: "LookingForDevice",
					comment: "Looking for an Android device connected by USB cable and set up for MTP...",
					message: "This is a progress message; MTP is an acronym for the system that allows computers to access files on devices.");
		...
		private void UsbFailConnect(Exception e)
		{
			...
			_progress.Message(idSuffix: "UnableToConnect",
				comment: "Unable to connect to any Android device which has Bloom Reader.");
			...
			switch (reportType)
			{
				...
				case DeviceNotFoundReportType.NoBloomDirectory:
					...
					_progress.Message(idSuffix: "DeviceWithoutBloomReader",
						comment: "The following devices are connected but do not seem to have Bloom Reader running:");
				...
				case DeviceNotFoundReportType.MoreThanOneReadyDevice:
					_progress.Message(idSuffix: "MoreThanOne",
						comment: "The following connected devices all have Bloom Reader installed. Please connect only one of these devices.");
					foreach (var deviceName in deviceNames)
						_progress.MessageWithoutLocalizing($"\t{deviceName}");
					break;
			}
