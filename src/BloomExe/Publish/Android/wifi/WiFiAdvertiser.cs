...
namespace Bloom.Publish.Android.wifi
{
	...
	public class WiFiAdvertiser : IDisposable
	{
		...
		private void Work()
		{
			_progress.Message(idSuffix: "beginAdvertising", comment: "Advertising book to Bloom Readers on local network...");
			try
			{
				while (true)
				{
			...
			{
				_progress.Message(idSuffix: "Stopped", comment: "Stopped Advertising.");
				_client.Close();
			}
