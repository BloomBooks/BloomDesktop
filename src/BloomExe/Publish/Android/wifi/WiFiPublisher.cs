...
namespace Bloom.Publish.Android.wifi
{
	...
	public class WiFiPublisher
	{
		...
		public void Stop()
		{
			// Locked to avoid contention with the thread that reports a transfer complete and tries to restart
			// the advertiser. Also to avoid contention with code in the thread that reports a transfer complete
			// and sets _wifiSender to null.
			lock (this)
			{
				if (_wifiAdvertiser != null)
				{
			...
			if (_wifiListener != null)
			{
				...
				// stop, nor does anything else I've tried, so we just have to guard against the
				// file transfer thread outliving everything else in the application.)
		...
		private void StartSendBookToClientOnLocalSubNet(Book.Book book, string androidIpAddress, string androidName, Color backColor)
		{
			...
			_wifiSender.UploadDataCompleted += (sender, args) =>
			{
				if (args.Error != null)
				{
					ReportExcepttion(args.Error);
				}
				// Should we report if canceled?
		...
		private void ReportExcepttion(Exception e)
		{
