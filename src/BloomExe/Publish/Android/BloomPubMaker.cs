	...
	public static class BloomPubMaker
	{
		...
		private static void FixCssReferencesForBadFonts(Book.Book book, string defaultFont, HashSet<string> badFonts)
		{
			...
			if (RobustFile.Exists(defaultLangStyles))
			{
				...
				// Note that the font may be referred to in customCollectionStyles.css or in a style defined in the HTML.
			...
			{
				var cssTextOrig = userStylesNode.InnerXml;	// need to preserve CDATA markup
				var cssText = cssTextOrig;
				foreach (var font in badFonts)
				{
					var cssRegex = new System.Text.RegularExpressions.Regex($"font-family:\\s*{font}\\s*!important;");
