...
namespace Bloom.Publish
{
	...
	public class ProcessPdfWithGhostscript
	{
		...
		public ProcessPdfWithGhostscript(bool shrink, string rgbProfile, string cmykProfile)
		{
			_shrink = shrink;
			...
			Debug.Assert((String.IsNullOrEmpty(rgbProfile) && String.IsNullOrEmpty(cmykProfile)) ||
				(!String.IsNullOrEmpty(rgbProfile) && !String.IsNullOrEmpty(cmykProfile)));
			_rgbProfile = rgbProfile;
			_cmykProfile = cmykProfile;
		}

		/// <summary>
		/// Process the input PDF file by compressing images and possibly by converting color to CMYK.
		/// </summary>
		public void ProcessPdfFile(string inputFile, string outputFile)
		{
			...
			// Both the RGB profile and the CMYK profile are needed to handle color conversion.  They should both
			// be set if either one is set.  But let's be paranoid and check both anyway.
			...
			using (_progress = new SIL.Windows.Forms.Progress.ProgressDialog())
			{
				if (_shrink)
				{
					_progress.Overview = L10NSharp.LocalizationManager.GetString(@"PublishTab.ProcessPdf.Shrink",
				...
				{
					_progress.StatusText = L10NSharp.LocalizationManager.GetString(@"PublishTab.ProcessPdf.Color",
				...
				{
					string shortMsg;
					if (_shrink && !String.IsNullOrEmpty(_rgbProfile) && !String.IsNullOrEmpty(_cmykProfile))
					{
						shortMsg = L10NSharp.LocalizationManager.GetString(@"PublishTab.ProcessPdf.ErrorProcessing",
							"Error compressing & recoloring the PDF file",	
							@"Message briefly displayed to the user in a toast");
					}
					else if (!_shrink && !String.IsNullOrEmpty(_rgbProfile) && !String.IsNullOrEmpty(_cmykProfile))
					{
						shortMsg = L10NSharp.LocalizationManager.GetString(@"PublishTab.ProcessPdf.ErrorColoring",
							"Error changing the PDF file colors",	
							@"Message briefly displayed to the user in a toast");
					}
					else
					{
						shortMsg = L10NSharp.LocalizationManager.GetString(@"PublishTab.ProcessPdf.ErrorCompressing",
							"Error compressing the PDF file",	
							@"Message briefly displayed to the user in a toast");
					}
					...
					NonFatalProblem.Report(ModalIf.None, PassiveIf.All, null, longMsg, _progress.ProgressStateResult.ExceptionThatWasEncountered);
				}
			}
		}
		...
		private void DoFixPdfWork(object sender, DoWorkEventArgs args)
		{
			...
			if (!String.IsNullOrEmpty(exePath) && File.Exists(exePath))
			{
				using (var tempPdfFile = TempFile.WithExtension(".pdf"))
				{
					...
					if (newInfo.Length < oldInfo.Length || (!String.IsNullOrEmpty(_rgbProfile) && !String.IsNullOrEmpty(_cmykProfile)))
