...
namespace Bloom.Publish
{
	...
	public class EpubMaker : IDisposable
	{
		...
		private string AudioPathForId(string id)
		{
			...
			// IDs for audio spans are typically guids generated by code in audioRecording.ts.
			// To make them valid xhtml IDs that code prepents an 'i' if the guid stars with a digit.
			// At this point we're not adding it to the filename though...maybe we should?
			// Anyway, for now, if it starts with 'i' and the next character is a digit
			// we may very well find a matching audio file without the initial 'i' in its name.
			if (id.StartsWith("i") && id.Length > 1 && Char.IsDigit(id[1]))
				fileNames.Add(id.Substring(1));
