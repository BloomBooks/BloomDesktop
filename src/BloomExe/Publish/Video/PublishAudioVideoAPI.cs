	...
	public class PublishAudioVideoAPI
	{
		...
		public void RegisterWithApiHandler(BloomApiHandler apiHandler)
		{
			...
			apiHandler.RegisterEndpointHandler(kApiUrlPart + "startRecording", request =>
			{
				...
				var audioInVideo = _recordVideoWindow?.AnyVideoHasAudio(videoList, request.CurrentBook.FolderPath) ?? false;
				if (audioInVideo)
				{
					var messageBoxButtons = new[]
					{
						new MessageBoxButton() { Text = LocalizationManager.GetString("Common.Cancel","Continue"), Id = "continue" },
