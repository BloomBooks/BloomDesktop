...
<html>
	...
	<body style=""margin: 0; width: {_videoHeight}px; height: {_videoWidth}px"">
		...
		private void StopRecordingInternal(IWebSocketProgress progress, string soundLogJson)
		{
			...
			// major part of the time this typically takes.
			...
			using (var filterFile = new TempFile(complexFilter)) {
				...
				using (var tempProgressOutputFile = TempFile.CreateAndGetPathButDontMakeTheFile())
				{
					...
					if (args.Length >= 32699)
					{
						...
						progress.Message("PublishTab.RecordVideo.TooManyAudioFiles", "This recording has more audio files than Bloom can handle", ProgressKind.Error);
					}
					else
					{
				...
				// Review: should we try harder to make sure this happens even if something goes wrong?
				// Note that we're only restoring files in a preview directory that we will shortly throw away;
				// the worst problem if we don't restore them is that a subsequent attempt in the same session to record or
				// preview the book will fail to find them.
