	...
	<body style=""margin: 0; width: {_videoHeight}px; height: {_videoWidth}px"">
		...
		private void StopRecordingInternal(IWebSocketProgress progress, string soundLogJson)
		{
			...
			if (haveVideo)
			{
				...
				//		"The parameter is a duration in hours, minutes, and seconds"),
				//	videoDuration);
			...
			if (soundLog.Length == 0)
			{
				...
				progress.MessageWithParams("Common.DoneAt", "The parameter is a time", "Done at {0:hh:mm tt}", ProgressKind.Progress, DateTime.Now);
				return;
			}
			...
			var estimateStr = millisRemaining < 60000 ?
				...
				string.Format(
					LocalizationManager.GetString(
						...
						"about {0} minutes",
						"The parameter is a number of minutes"),
					Math.Ceiling(millisRemaining / 60000));
			return string.Format(
				LocalizationManager.GetString(
					"PublishTab.RecordVideo.EstimatedTimeRemaining",
					"Estimated time remaining: {0}",
					"The parameter is text describing the amount of time such as 'less than one minute' or 'about 3 minutes'"),
				estimateStr);
		}
		...
		private bool GetLastOccurenceOfKeyValue(string content, string key, out string value)
		{
			...
			var pattern = $"\\n{key}=(.*)\\n";
			var lastOccurenceRegex = new Regex($"{pattern}(?![\\s\\S]*{pattern})");
			var match = lastOccurenceRegex.Match(content);
