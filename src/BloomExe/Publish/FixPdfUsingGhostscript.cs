	...
	public class FixPdfUsingGhostscript
	{
		...
		public void CreateFixedPdfFile(string inputFile, string outputFile)
		{
			...
			using (_progress = new SIL.Windows.Forms.Progress.ProgressDialog())
			{
				...
				{
					var shortMsg = String.Format("Exception encountered fixing PDF: {0}", _progress.ProgressStateResult.ExceptionThatWasEncountered);
					NonFatalProblem.Report(ModalIf.None, PassiveIf.Beta, shortMsg, null, _progress.ProgressStateResult.ExceptionThatWasEncountered);
				}
			}
		}
		...
		private void DoFixPdfWork(object sender, DoWorkEventArgs args)
		{
			...
			if (!String.IsNullOrEmpty(exePath) && File.Exists(exePath))
			{
				using (var tempPdfFile = TempFile.WithExtension(".pdf"))
				{
					...
					{
						NonFatalProblem.Report(ModalIf.None, PassiveIf.Beta, "Fixing the PDF failed, using the original PDF file", null,
							_progress.ProgressStateResult.ExceptionThatWasEncountered);
						if (_inputPdfPath != _outputPdfPath)
							RobustFile.Copy(_inputPdfPath, _outputPdfPath, true);
						return;
					}
