	...
	public class PdfMaker
	{
			...
			PublishModel.BookletLayoutMethod booketLayoutMethod, PublishModel.BookletPortions bookletPortion, BackgroundWorker worker, DoWorkEventArgs doWorkEventArgs, Control owner)
		{
			...
			if (!RobustFile.Exists(outputPdfPath) && owner != null)
			{
				...
				doWorkEventArgs.Result = new MakingPdfFailedException(LocalizationManager.GetString("PublishTab.PdfMaker.BadPdfShort", "Bloom had a problem making a PDF of this book."));
				return;
			}
		...
		{
			public MakingPdfFailedException(string message) : base(message)
			{
			}
		}
