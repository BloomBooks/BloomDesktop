...
namespace Bloom.Publish.PDF
{
	...
	public partial class PdfViewer : UserControl, nsIWebProgressListener, nsISupportsWeakReference
	{
		...
		private bool TryGhostcriptPrint()
		{
			...
			systemSpecificArgs = "";
#else
			var exePath = ProcessPdfWithGhostscript.FindGhostcriptOnWindows();
			...
			// I think containing this string is the best I can do to identify the error that
			// it returns if the user cancels when selecting a file on something like write to XPS.
			if (!string.IsNullOrEmpty(errors) && !errors.Contains("000004c7"))
			{
				Logger.WriteEvent($"Ghostscript printing failed with error '{errors}' and output '{output}'");
				ErrorReport.NotifyUserOfProblem(
					"Printing seems to have failed. It may help to install Adobe Acrobat. To print without Acrobat, you must set a correct page size that your printer is capable of printing.");
			}
