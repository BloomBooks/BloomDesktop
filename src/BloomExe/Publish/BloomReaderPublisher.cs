...
namespace Bloom.Publish
{
	public class BloomReaderPublisher
	{
		...
		private void OneReadyDeviceNotFound(object sender, OneReadyDeviceNotFoundEventArgs eventArgs)
		{
			...
			if (eventArgs.ReportType == DeviceNotFoundReportType.MoreThanOneReadyDevice)
			{
			...
			{
				_progress.WriteWarning(string.Format(LocalizationManager.GetString("Publish.BloomReaderPublisher.DeviceWithoutBloomReader",
					"The {0} device was found but does not seem to have Bloom Reader installed.", "{0} is a mobile device name"), eventArgs.DeviceNames[0]));
			}
		}
