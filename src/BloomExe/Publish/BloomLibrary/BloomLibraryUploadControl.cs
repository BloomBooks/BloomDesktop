...
namespace Bloom.Publish.BloomLibrary
{
	...
	public partial class BloomLibraryUploadControl : UserControl
	{
		...
		{
			// If we have two or more languages, we want the Background Music check box to align with the second language (for aesthetics)
			if (_languagesFlow.Controls.Count > 1)
			{
				var firstLanguageCheckBox = _languagesFlow.Controls[0];
				var firstLanguageWidth = firstLanguageCheckBox.Width;
				var narrationWidth = GetWidthForCheckBox(_narrationAudioCheckBox);
				var newWidth = Math.Max(firstLanguageWidth, narrationWidth);

				firstLanguageCheckBox.Width = newWidth;
				_narrationAudioCheckBox.Width = newWidth;
			}
