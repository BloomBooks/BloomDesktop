...
namespace Bloom.WebLibraryIntegration
{
	...
	public class BookTransfer
	{
		...
		private string UploadBook(string bookFolder, IProgress progress, out string parseId,
			string pdfToInclude = null, ISet<string> audioFilesToInclude = null, IEnumerable<string> videoFilesToInclude = null, string[] languages = null, IEnumerable<string> collectionLanguages = null)
		{
			...
			try
			{
				...
				try
				{
					_s3Client.UploadBook(s3BookId, bookFolder, progress, pdfToInclude, audioFilesToInclude, videoFilesToInclude, languages, collectionLanguages);
		...
		internal string FullUpload(Book.Book book, LogBox progressBox, PublishView publishView, string[] languages, bool excludeNarrationAudio, bool excludeMusic, out string parseId)
		{
			...
			return UploadBook(bookFolder, progressBox, out parseId, Path.GetFileName(uploadPdfPath),
				GetAudioFilesToInclude(book, excludeNarrationAudio, excludeMusic), GetVideoFilesToInclude(book), languages, book.CollectionSettings.CollectionLanguageCodes);
		}
