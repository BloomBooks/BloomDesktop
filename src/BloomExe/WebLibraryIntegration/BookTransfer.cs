...
namespace Bloom.WebLibraryIntegration
{
	...
	public class BookTransfer
	{
		...
		public string UploadBook(string bookFolder, IProgress progress, out string parseId)
		{
			...
			// Occasionally we may want to upload a new authoring template. Such books are marked to indicate that lockdown is
			// not allowed.
			...
			if (!string.IsNullOrEmpty(htmlFile))
			{
				...
				allowLocking = Book.Book.IsLockDownAllowed(domForLocking);
		...
		internal string DownloadBook(string s3BookId, string dest)
		{
			...
			if (Book.Book.IsLockDownAllowed(dom))
			{
