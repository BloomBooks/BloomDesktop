	...
	public class BookTransfer
	{
		...
		public static string UploadBucketNameForCurrentEnvironment
		{
			get
			{
				...
				if (Destination == UploadDestination.Production)
				{
					return BloomS3Client.ProductionBucketName;
				}
				// dry run or development
			...
			CollectionSettings collectionSettings = null, bool dryrun = false)
		{
			...
			try
			{
				...
				catch (WebException e)
				{
					...
					if (!IsTesting) // don't make it seem like there are more upload failures than their really are if this a tester pushing to the sandbox
				...
				{
					DisplayNetworkUploadProblem(e, progress);
					if (!IsTesting) // don't make it seem like there are more upload failures than there really are if this a tester pushing to the sandbox
						Analytics.Track("UploadBook-Failure", new Dictionary<string, string>() { { "url", metadata.BookOrder }, { "title", metadata.Title }, { "error", e.Message } });
					return "";
				}
				catch (Exception e)
		...
		public static string Destination
		{
			get
			{
				if (_destination == null)
					Destination = UseSandbox ? UploadDestination.Development : UploadDestination.Production;
				return _destination;
			}
			set
			{
		...
		/// <summary>
		/// Either in a dry run or uploading to the sandbox.
		/// </summary>
		public static bool IsTesting => Destination == UploadDestination.DryRun || Destination == UploadDestination.Development;
		...
		internal string FullUpload(Book.Book book, LogBox progressBox, PublishView publishView, BookUploadParameters bookParams, out string parseId)
		{
			...
			return UploadBook(bookFolder, progressBox, out parseId, Path.GetFileName(uploadPdfPath),
				...
				bookParams.LanguagesToUpload, book.CollectionSettings, IsDryRun);
		}
