...
namespace Bloom.WebLibraryIntegration
{
	...
	public class BookTransfer
	{
		...
		public static string UploadBucketNameForCurrentEnvironment
		{
			get
			{
				...
				if (!String.IsNullOrEmpty(BookTransfer.Destination))
				{
					switch (BookTransfer.Destination)
					{
						case UploadDestination.DryRun: return BloomS3Client.DryRunBucketName;
						case UploadDestination.Development: return BloomS3Client.SandboxBucketName;
						case UploadDestination.Production: return BloomS3Client.ProductionBucketName;
					}
				}
		...
		private string UploadBook(string bookFolder, IProgress progress, out string parseId,
			...
			CollectionSettings collectionSettings = null, bool dryrun = false)
		{
			...
			try
			{
				...
				try
				{
					...
					if (!dryrun)
					{
						...
						//   if (!UseSandbox) // don't make it seem like there are more uploads than there really are if this a tester pushing to the sandbox
						{
				...
				{
					DisplayNetworkUploadProblem(e, progress);
					if (!UseSandbox) // don't make it seem like there are more upload failures than there really are if this a tester pushing to the sandbox
						Analytics.Track("UploadBook-Failure", new Dictionary<string, string>() { { "url", metadata.BookOrder }, { "title", metadata.Title }, { "error", e.Message } });
					return "";
				}
				catch (Exception e)
		...
		/// <summary>
		/// The upload destination set from the command line.  This must be set even before calling
		/// the constructor of this class because it is used in UploadBucketNameForCurrentEnvironment
		/// which is called by the BloomParseClient constructor.  And the constructor for this
		/// class has a BloomParseClient argument.
		/// </summary>
		...
		{
			if (options.Dest != UploadDestination.DryRun && !IsThisVersionAllowedToUpload())
			{
			...
			{
				if (options.Dest != UploadDestination.DryRun && !LogIn(options.UploadUser, options.UploadPassword))
					{
						...
						Console.WriteLine();
						Console.WriteLine("Failed to login as {0} with password {1}.", options.UploadUser, options.UploadPassword);
						...
						Console.WriteLine("signing out.  This will enable logging in for the bulk upload operation.");
						return;
					}
			...
			_singleBookshelfLevel = options.SingleBookshelfLevel;
		...
		private void BackgroundUpload(object sender, DoWorkEventArgs doWorkEventArgs)
		{
			...
			try
			{
				...
				if (bookParams.Dest != UploadDestination.DryRun)
		...
		internal string FullUpload(Book.Book book, LogBox progressBox, PublishView publishView, BookUploadParameters bookParams, out string parseId)
		{
			...
			return UploadBook(bookFolder, progressBox, out parseId, Path.GetFileName(uploadPdfPath),
				...
				bookParams.LanguagesToUpload, book.CollectionSettings, bookParams.IsDryRun);
		}
	...
	public class BookUploadParameters
	{
		...
		public bool IsDryRun { get { return Dest == UploadDestination.DryRun; } }
	}
}
