...
namespace Bloom.WebLibraryIntegration
{
	...
	public class BookTransfer
	{
		...
		internal string FullUpload(Book.Book book, LogBox progressBox, PublishView publishView, string[] languages, bool excludeNarrationAudio, bool excludeMusic, out string parseId)
		{
			...
			return UploadBook(bookFolder, progressBox, out parseId, Path.GetFileName(uploadPdfPath), GetAudioFilesToInclude(book, excludeNarrationAudio, excludeMusic), GetVideoFilesToInclude(book));
		}

		/// <summary>
		/// Figure out which video files are used in this book, in case we haven't had them stripped out by opening
