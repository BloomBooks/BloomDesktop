            ...
            try
            {
                if (!IsDryRun)
                {
                    ...
                    using (var stagingParentDirectory = new TemporaryFolder("BloomUploadStaging"))
                    {
                        ...
                        // The server determines our upload location (_storageKeyOfBookFolderParentOnS3)
                        // and, for an existing book, copies the files which haven't been modified.
                        // After we are done uploading new and modified files, the server handles pointing the book
