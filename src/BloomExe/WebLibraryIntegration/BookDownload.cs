	...
	public class BookDownload
	{
		...
		internal void HandleBloomBookOrder(string bookOrderUrl)
		{
			...
			if (!IsThisVersionAllowedToDownload(bookOrderUrl))
			{
				...
				MessageBox.Show(LocalizationManager.GetString("PublishTab.Upload.OldVersion",
		...
		internal static bool IsThisVersionAllowedToDownloadInner(string bookOrderUrl, string appVersion)
		{
			int requiredMajorVersion;
			int requiredMinorVersion;
			try
			{
				var minVersionStr = HttpUtility.ParseQueryString(new Uri(bookOrderUrl).Query)["minVersion"];
