...
//			//focus() made it worse, select has no effect
		...
		private void OnShowBookMetadataEditor()
		{
			try
			{
				...
				//not relevant anymore, as we already have the default on the page for new books	metadata.License = new CreativeCommonsLicense(true, true, CreativeCommonsLicense.DerivativeRules.Derivatives);
				}
				...
				using (var dlg = new SIL.Windows.Forms.ClearShare.WinFormsUI.MetadataEditorDialog(metadata))
				{
					...
					{
						_model.ChangeBookMetadata(dlg.Metadata);
					}
				}
