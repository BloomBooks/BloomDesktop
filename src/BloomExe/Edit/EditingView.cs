...
namespace Bloom.Edit
{
	public partial class EditingView : UserControl, IBloomTabArea, IZoomManager
	{
			...
			CopyrightAndLicenseApi copyrightAndLicenseApi)
		{
			...
			bookRenamedEvent.Subscribe((oldNew) =>
			{
				// If the selected book is renamed, we should update our saved CurrentBookPath
				if (oldNew.Value == _model.CurrentBook.FolderPath)
				{
					Settings.Default.CurrentBookPath = oldNew.Value;
				}
				UpdatePageList(true);
			});
#if __MonoCS__
...
// See http://issues.bloomlibrary.org/youtrack/issue/BL-3714.
		...
		private void SetupBrowserContextMenu()
		{
			...
			_browser1.ContextMenuProvider = args =>
			{
				...
				{
					// This case can be generated by the Overlay tool. It inserts an SVG of all the bubbles/tails/etc,
