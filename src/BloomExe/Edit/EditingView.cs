...
//			//focus() made it worse, select has no effect
			...
			if (oom != null)
			{
				...
				try
				{
					...
					var msgFmt = LocalizationManager.GetString("EditTab.Image.TooLarge",
						"Bloom ran out of memory loading this image ({0}), which is quite large ({1} by {2} pixels). Press Help for suggestions.");
		...
		{
			System.Diagnostics.Process.Start("https://community.software.sil.org/t/running-out-of-memory-loading-images/3956");
		}
