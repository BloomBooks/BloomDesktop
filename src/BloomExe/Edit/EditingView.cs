...
//#endif
		...
		private void OnChangeImage(DomEventArgs ge)
		{
			...
			using(var dlg = new ImageToolboxDialog(imageInfo, null))
			{
				...
				if (DialogResult.OK == result && dlg.ImageInfo != null)
				{
					...
					int height, width;
					TryGetOriginalImageDimensions(dlg.ImageInfo, out height, out width);
					var copyOriginalImage = height == dlg.ImageInfo.Image.Height && width == dlg.ImageInfo.Image.Width;
					...
					try
					{
						if (copyOriginalImage)
							...
							dlg.ImageInfo.Image.Save(newImagePath);
					...
					finally
					{
						...
						Debug.WriteLine($"total image preparation time = {finished - started}");
					}
#if MEMORYCHECK
		...
		/// </summary>
		private void TryGetOriginalImageDimensions(PalasoImage palasoImage, out int height, out int width)
		{
			height = -1;
			width = -1;
			try
			...
			{
				// Ignore any errors.  We tried and failed...
			}
		}
