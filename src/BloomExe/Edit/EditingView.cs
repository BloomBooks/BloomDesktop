...
// See http://issues.bloomlibrary.org/youtrack/issue/BL-3714.
		...
		public void CleanHtmlAndCopyToPageDom()
		{
			...
			//				if (endHtml > 0)
			//				{
			//					html = combinedData.Substring(0, endHtml);
			//					css = combinedData.Substring(endHtml + "<SPLIT-DATA>".Length);
			//				}
			//			}
...
if (typeof(editTabBundle) !=='undefined' && typeof(editTabBundle.getEditablePageBundleExports()) !=='undefined')
  editTabBundle.getEditablePageBundleExports().userStylesheetContent();";
			var bodyHtml = RunJavascriptWithStringResult_Sync_Dangerous(getHtmlBodyScript);
			var userCssContent = RunJavascriptWithStringResult_Sync_Dangerous(getCssContentScript);
