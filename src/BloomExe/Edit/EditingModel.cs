...
namespace Bloom.Edit
{
	public class EditingModel
	{
		...
		private void OnTabAboutToChange(TabChangedDetails details)
		{
			...
			{
				SaveNow();
				_view.RunJavaScript("if (typeof(editTabBundle) !=='undefined' && typeof(editTabBundle.getPageeditTabBundle()) !=='undefined') {editTabBundle.getPageeditTabBundle().disconnectForGarbageCollection();}");
		...
		private void OnPageSelectionChanging(object sender, EventArgs eventArgs)
		{
			...
			if (_view != null && !_inProcessOfDeleting && !_inProcessOfLoading)
			{
				...
				_view.RunJavaScript("if (typeof(editTabBundle) !=='undefined' && typeof(editTabBundle.getPageeditTabBundle()) !=='undefined') {editTabBundle.getPageeditTabBundle().pageSelectionChanging();}");
				FinishSavingPage();
				_view.RunJavaScript("if (typeof(editTabBundle) !=='undefined' && typeof(editTabBundle.getPageeditTabBundle()) !=='undefined') {editTabBundle.getPageeditTabBundle().disconnectForGarbageCollection();}");
			}
		}
