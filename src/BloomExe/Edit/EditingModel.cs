...
namespace Bloom.Edit
{
    public class EditingModel
    {
        ...
        public void SaveNow(bool forceFullSave = false)
        {
            ...
            {
                Logger.WriteEvent($"DEBUG: EditingModel.SaveNow() starting");
#if MEMORYCHECK
                ...
                try
                {
                    ...
                    if (_view.InvokeRequired)
                    {
                        ...
                        Logger.WriteEvent(
                            $"DEBUG: EditingModel.SaveNow() finished after Invoke to get on UI thread"
                        );
                        return;
                    }
...
#endif
                Logger.WriteEvent($"DEBUG: EditingModel.SaveNow() finished");
            }
            else
            {
                Logger.WriteEvent(
                    $"DEBUG: EditingModel.SaveNow() called when _inProcessOfSaving={_inProcessOfDeleting}, NavigatingSoSuspendSaving={NavigatingSoSuspendSaving}, _domForCurrentPage={_domForCurrentPage}"
                );
            }
        }
