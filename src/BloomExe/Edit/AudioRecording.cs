...
namespace Bloom.Edit
{
	...
	public class AudioRecording :IDisposable
	{
		...
		/// </summary>
		object beginMonitoringLock = new object();
		...
		public void BeginMonitoring()
		{
			...
			// See BL-13003.
			lock (beginMonitoringLock)
			{
				if (!RecordingDevices.Contains(RecordingDevice))
				{
					RecordingDevice = RecordingDevices.FirstOrDefault();
				}
				if (RecordingDevice != null && /* Don't check _monitoringAudio because we can have a new Recorder after resuming */
					(Recorder.RecordingState == RecordingState.NotYetStarted || Recorder.RecordingState == RecordingState.Stopped))
				{
					try
					{
