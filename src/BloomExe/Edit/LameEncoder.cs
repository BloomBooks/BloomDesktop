...
namespace Bloom.Edit
{
    ...
    public class LameEncoder
    {
        ...
        {
            string destinationPath = Path.ChangeExtension(
                sourcePath,
            ...
            catch (Exception)
            {
                var shortMsg = LocalizationManager.GetString(
                    "LameEncoder.DeleteFailedShort",
                    "Cannot replace mp3 file. Check antivirus"
                );
                var longMsg = LocalizationManager.GetString(
                    "LameEncoder.DeleteFailedLong",
                    "Bloom could not replace an mp3 file. If this continues, check your antivirus."
                );
            ...
            ExecutionResult result = CommandLineRunnerExtra.RunWithInvariantCulture(
                GetLamePath(),
                arguments,
                null,
                60,
...
#else
            return s_pathToLame = FileLocationUtilities.GetFileDistributedWithApplication(
                "lame.exe"
            );
#endif
        }
