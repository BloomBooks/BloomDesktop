...
namespace Bloom.Edit
{
	...
	public class Configurator
	{
		...
		public static bool IsConfigurable(string folderPath)
		{
			...
			return SafeFile.Exists(Path.Combine(folderPath, "configuration.htm"));
		}
		...
		/* setup jquery in chrome console (first open a local file):
			...
			SafeFile.Delete(bookPath);
			SafeFile.Move(temp.Path, bookPath);
			...
			{
				Logger.WriteMinorEvent(SafeFile.ReadAllText(bookPath)); //this will come to us if they report it
		...
		public void CollectJsonData(string newDataString)
		{
			...
			SafeFile.WriteAllText(PathToLibraryJson, libraryData.ToString());
		...
		{
			if (!SafeFile.Exists(PathToLibraryJson))
				return "{}";//return "{\"dummy\": \"x\"}";//TODO

			var s= SafeFile.ReadAllText(PathToLibraryJson);
