...
namespace Bloom.Edit
{
	public class PageEditingModel
	{
			...
			IProgress progress)
		{
			...
			{
				// We must have bad metadata in the original file.  We can't fix it, but we can
				// avoid crashing.
				// Enhance: strip the metadata out of the file and then try saving it anew.
				Debug.WriteLine($"Error saving metadata to image file: {ex}");
			}
		}
