...
namespace Bloom.Edit
{
	public class PageEditingModel
	{
		...
		{
			var isSameFile = IsSameFile(bookFolderPath, img.GetAttribute("src"), imageInfo);
		...
		public void ChangePicture(string bookFolderPath, XmlDocument dom, string imageId, PalasoImage imageInfo)
		{
			...
			var isSameFile = IsSameFile(bookFolderPath, img.GetAttribute("src"), imageInfo);
			var imageFileName = ImageUtils.ProcessAndSaveImageIntoFolder(imageInfo, bookFolderPath, false);
			img.SetAttribute("src", imageFileName);
		...
		/// revised name.  We could perhaps use a tool to remove unused picture files
		/// from a book's folder.
		/// </remarks>
		private bool IsSameFile(string bookFolderPath, string src, PalasoImage imageInfo)
		{
