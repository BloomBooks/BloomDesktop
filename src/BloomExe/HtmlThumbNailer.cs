					...
					_syncControl.Invoke((Action) (() =>
					{
						try
						{
							var where = Browser.RunJavaScriptOn(browser,
								"{var r = document.getElementsByClassName('bloom-imageContainer')[0].getBoundingClientRect();return JSON.stringify({top:r.top, bottom:r.bottom, doc:document.firstElementChild.clientHeight});}");
