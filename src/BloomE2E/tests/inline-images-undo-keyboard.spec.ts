...
test("Ctrl+Z reaches the page, and takes back the last change to a picture [Test Case ID 815]", async ({
    ...
    const textAfterCtrlZ = await page.evaluate(
        () =>
                    ...
                    () =>
                        resolve(
                            (
                                ...
                                '.bloom-translationGroup > .bloom-editable[lang="en"]',
                            )?.textContent ?? "",
                        ),
                    500,
                ),
    ...
    console.log(
        `[probe] Ctrl+Z on text: ${
            textAfterCtrlZ.includes("nobody wanted")
                ? "did nothing -- the key does not reach the page"
                : "undid the typing -- the key reaches CKEditor"
        }`,
    );
    ...
    console.log(`[probe] every copy after Ctrl+Z: ${JSON.stringify(blocks)}`);
    const copies = blocks
        .flatMap((b) => b.images)
