...
test("a drag follows the pointer at every zoom [Test Case ID 815]", async ({
    ...
    console.log(`[probe] zooms: ${JSON.stringify(zoomsToTry)}`);
    ...
    try {
        for (const zoom of zoomsToTry) {
            ...
            console.log(
                `[probe] at ${zoom}%: pointer moved ${pointerTravelViewportPx}px, picture moved ` +
                    `${Math.round(pictureMovedViewportPx)}px, a line on screen being ` +
                    `${Math.round(lineHeightViewportPx)}px`,
            );
            expect(
