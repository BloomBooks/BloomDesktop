...
test("a picture stays inside its block when the book is drawn at another size [Test Case ID 815]", async ({
    ...
    console.log(`[probe] at ${atA5.pageSize}: ${JSON.stringify(atA5)}`);
    expect(
        atA5.dock,
    ...
    console.log(`[probe] sizes this book offers: ${sizes.join(", ")}`);
    for (const size of ["A6Portrait", "A5Landscape"].filter((s) =>
        ...
        console.log(`[probe] at ${size}: ${JSON.stringify(after)}`);
        expect(
            after.overflowPx,
