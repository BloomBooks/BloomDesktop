...
test("pasting text that contains a picture does not produce two pictures with one identity [Test Case ID 815]", async ({
    ...
    console.log(`[probe] before the paste: ${JSON.stringify(before)}`);
    ...
    // The paste is asynchronous in CKEditor, and so is anything the page does about it.
    await page.waitForTimeout(1500);

    const textAfter = ((await block.textContent()) ?? "").length;
    console.log(`[probe] block text length ${textBefore} -> ${textAfter}`);
    expect(
        textAfter,
    ...
    console.log(`[probe] after the paste: ${JSON.stringify(after)}`);
...
test("pasting a picture into a front-matter field does not put markup in the data div [Test Case ID 815]", async ({
    ...
    await page.waitForTimeout(1500);
    console.log(
        `[probe] the front-matter block after the paste: ${await title.evaluate((e) => e.innerHTML)}`,
    );
    ...
    console.log(`[probe] stored cover credits: ${storedCredits.slice(0, 300)}`);
    expect(
        storedCredits,
