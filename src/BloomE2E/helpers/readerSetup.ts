...
/** One file as the Sample Words tab lists it: its name, and whether Bloom can read it. */
export interface ISampleTextFile {
    name: string;
    readable: boolean;
    /** The explanation shown beside an unreadable file, or "" for a readable one. */
    explanation: string;
}
    ...
    return page
        ...
        .evaluateAll((rows) =>
            rows.map((row) => {
                ...
                const readable = row.getAttribute("data-readable") === "true";
                return {
