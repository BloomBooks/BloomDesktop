...
// machine, not about Bloom. "The two columns fill the table with no gap and no overlap" and "the
// picture lies inside its cell" are facts about Bloom, and they are what these check.
//
 ...
 * to be able to hit independently: the row and column pills, the "+" buttons, the canvas element's
 * own toolbar.
 */
...
/**
 * Assert that `inner` lies within `outer`. Use this for a picture inside its cell, or a cell inside
 * its table: it says the thing is where it belongs without saying where that is.
 */
