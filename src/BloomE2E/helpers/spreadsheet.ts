...
export async function importSpreadsheetIntoBook(
    ...
    const bookPath = bookHtmlPath(bookFolder);
    const before = fs.statSync(bookPath).mtimeMs;
    ...
    await expect
        .poll(() => fs.statSync(bookPath).mtimeMs, {
            timeout: 120000,
            message:
                `The import never rewrote ${bookPath}. An import that fails leaves the book ` +
                "alone and says so in Bloom's progress dialog.",
        })
        .not.toBe(before);
