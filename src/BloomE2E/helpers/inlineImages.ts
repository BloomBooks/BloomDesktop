        ...
        .poll(
                ...
                ).filter((image) => image.dock === dock).length,
            {
                ...
                message:
                    ...
                    `dock it there in every language. Bloom puts a move back where it started ` +
                    `when the move would make the block overflow.`,
            },
        )
        .toBeGreaterThan(0);
}
    ...
    await expect
        .poll(
            async () => {
                ...
                return copies.every((copy) => copy.fileName === fileName)
                    ? copies.length
                    : 0;
            },
            {
                timeout: 30000,
                message: `${fileName} never reached every language's copy of inline image ${id}.`,
            },
        )
        .toBeGreaterThan(0);
}
