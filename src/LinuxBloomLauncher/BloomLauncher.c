...
int main(int argc, char *argv[])
{
	...
	gtk_window_set_title(GTK_WINDOW(window), "Instant Bloom Splash Screen");
...
{
	++counter;
	if (counter >= 60 || access(flagFile, F_OK))
	{
		gtk_main_quit();
		return FALSE;
	}
...
{
	gchar ** argvNew = g_new(gchar *, argcOrig + 3);	// 2 new args + terminating NULL
	...
	for (int i = 1; i < argcOrig; ++i)
		argvNew[i+2] = g_strdup(argvOrig[i]);
	argvNew[argcOrig+2] = NULL;
