...
export const BooksOnBlorgProgressBar: React.FunctionComponent = () => {
    ...
    // Verify 'translated' status. It was harder than expected to avoid a momentary "-1 language books..."
    // We don't set the percentage until the label we are going to use no longer has a placeholder and
    // not longer has a "-1" in it.
    // N.B. The percentage value governs the height of the progress bar, so no percentage, no progress bar.
    ...
    }, [webGoal, bookCount, languageName, translationComplete]);
