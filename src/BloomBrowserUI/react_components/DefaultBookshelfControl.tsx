...
export const DefaultBookshelfControl: React.FunctionComponent = props => {
    // Things get tricky because we have to run three queries here to get the
    // data we need, each of which depends on the results of the previous one.
    ...
    // to retrieve the actual contentful ID of the Enterprise Subscription
    // object to which the collections we want refer. The third retrives the
    // collections that refer to that object in their enterpriseSubscriptions
    // field. (Because this is 1:many, contentful does not allow us to retrieve
    // collections using it without an exact object ID.)
    ...
    // The currentShelf retrieved from the settings/bookShelfData API, or 'none'
    // currentShelf is falsy. Using 'none' as the value here in this control
    ...
    React.useEffect(() => {
        BloomApi.get("settings/bookShelfData", data => {
            ...
            setCurrentShelf(data.data.defaultBookshelf || "none");
        });
    }, []);

    // Preliminary query to get the ID of the indicated enterpriseSubscription
    // (assuming we have a project...if the project is not an enterprise one,
    // or we haven't yet retrieved the project, no query will run.)
    const {
        loading: loadingES,
        result: resultES,
        error: errorES
    ...
    const projectId =
        loadingES || !resultES || resultES.length < 1 ? "" : resultES[0].sys.id;

    // The main query that gets the names and urlKeys of the collections connected to this
    // enterpriseSubscription. If we don't (yet) have a projectID, this query does not run.
    const { loading, result, error } = useContentful(
        projectId
            ? {
                  ...
                  // As of Jan 2021, we save about 17% by restricting fields this way. Replacing all the fields.* items with just "fields"
                  // is also worth considering...we save 16% just by cutting out unused sys fields.
                  select: "fields.label,fields.urlKey",
                  include: 10, // depth
                  // To test behavior with error results from the query, a simple trick is to set this to something
                  // invalid, such as deleting the .sys.
                  "fields.enterpriseSubscriptions.sys.id": `${projectId}`
              }
            : undefined // no project means we don't want useContentful to do a query
    );
    ...
    if (!project) {
        ...
        bookshelves = [];
    } else if (!loading && !loadingES && result && !errorES && !error) {
        // If all is well an we've completed all the queries, we got a list of
        // bookshelf contentful collections connected to this branding, and will
        // now generate the list of items (prepending the 'None' we already made).
        bookshelves = bookshelves.concat(
            result.map<{ value: string; label: string }>((c: any) => ({
        ...
        // Note that as we don't yet have any better label, we use currentShelf.
        bookshelves.push({ value: currentShelf, label: currentShelf });
    }
    ...
    // the max-height is close than usual to 100% and helps the menu not have to
    // scroll, even in a fairly small space (it is confined to its own control,
    // not the whole surface of the dialog).
    ...
    // a Windows combo. Reduced padding makes the menu and items take up more
    // the usual Windows amount of space.
        ...
        <ThemeProvider theme={theme}>
            ...
            >
                ...
                <Select
                    ...
                    // Windows-style arrows; these don'e seem to be configurable (except perhaps by
