...
// For 6.5 the checkbox is ALSO gated on visibility: the host reports showAllowCloudTeamCollection
// only when the `cloudCollections` environment variable is set (CollectionSettingsApi.
// CloudTeamCollectionOptionVisible), so most end users never see it.
...
// showAllowCloudTeamCollection defaults to true here so the enabled/disabled tests below still
// find the checkbox; the env-var gating (it only appears when true) is covered by its own tests.
function respondWith(overrides: {
    allowCloudTeamCollection?: boolean;
    allowCloudTeamCollectionEnabled?: boolean;
    showAllowCloudTeamCollection?: boolean;
}) {
                ...
                callback({
                    data: {
                        ...
                        showAllowCloudTeamCollection:
                            overrides.showAllowCloudTeamCollection ?? true,
                    },
                });
            }
        },
...
describe("AdvancedSettingsPanel: Cloud Team Collections (experimental) checkbox", () => {
    ...
    it("does NOT render the checkbox when showAllowCloudTeamCollection is false (cloudCollections env var not set)", () => {
        respondWith({ showAllowCloudTeamCollection: false });

        const container = render();

        // Sanity: a normally-present control still renders, so an absent cloud checkbox is the
        // gate at work, not a failed render.
        expect(
            container.querySelector(
                '[data-testid="configr-boolean-showQrCode"]',
            ),
        ).not.toBeNull();
        expect(
            container.querySelector(
    ...
    it("renders the checkbox when showAllowCloudTeamCollection is true (cloudCollections env var set)", () => {
        respondWith({ showAllowCloudTeamCollection: true });
