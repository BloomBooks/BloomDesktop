...
describe("CollectionSettingsDialog", () => {
    ...
    it("ignores Cancel while a save is in flight", async () => {
        // The save is never reported back, so the dialog stays mid-save. Cancelling then would
        // end the C# session the save still needs.
        mockPostJson.mockImplementation(() => {});
        await renderDialog();
        click("dialog-ok");
        expect(mockPostJson).toHaveBeenCalledTimes(1);
    ...
    it("ignores a settings reply that arrives after its open has closed", async () => {
        let deliverFirstReply: (() => void) | undefined;
        ...
        await renderDialog();
        if (!deliverFirstReply) {
            throw new Error(
                "The dialog did not ask for the settings; the rest of this test proves nothing.",
            );
        }
    ...
    it("shows only the reason and a Close button when the user may not edit", async () => {
        ...
        // No session was opened, so closing has nothing to cancel on the C# side.
        click("dialog-close");
        expect(mockCloseDialog).toHaveBeenCalledTimes(1);
        expect(mockPost).not.toHaveBeenCalled();
    });
...
describe("showCollectionSettingsDialog", () => {
    it("sends the launch event the dialog listens for, with the page to open on", () => {
        const received: unknown[] = [];
        const listener = (event: Event) =>
            received.push((event as CustomEvent).detail);
        document.addEventListener("LaunchDialog", listener);
        try {
            showCollectionSettingsDialog("subscription");
        } finally {
            document.removeEventListener("LaunchDialog", listener);
        }

        expect(received).toEqual([
            { id: "CollectionSettingsDialog", initialPageKey: "subscription" },
        ]);
    });
});
