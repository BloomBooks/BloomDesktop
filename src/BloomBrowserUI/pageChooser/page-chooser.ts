...
class PageChooser {
    ...
    loadNextPageGroup(queue, groupHTML, gridItemHTML, defaultPageToSelect:string): void {
        ...
        axios.get("/bloom/" + order.templateBookPath).then( result =>{
            ...
            // If the replace method had group selection and replacement, we could revise the src attributes into api
            // calls, but that isn't possible in Typescript without extremely convoluted string manipulation.  See
            // https://silbloom.myjetbrains.com/youtrack/issue/BL-3819 for details of the symptoms experienced when
            // running Bloom without this ugly hack.  Note that none of the img elements in the template file are
            // needed at this point for laying out the Add Page dialog, or for creating thumbnails, so it's safe to
            // delete them.
