...
export class PageChooser {
    ...
    private getTextOfFirstElementByClassNameSafely(
        ...
        if (collection.length === 0 || collection.item(0) == null) {
            return "";
        }
        const text = (collection.item(0) as Element).textContent;
        return text ? text : "";
    }
    ...
    public loadPageGroups(): void {
        ...
        this._scrollingDiv = document.getElementsByClassName(
            "gridItemDisplay"
        )[0] as HTMLDivElement;
        ...
        const originalGroupDiv = this._scrollingDiv.getElementsByClassName(
            "group"
        )[0];
        const groupHtml = originalGroupDiv.cloneNode(true) as HTMLElement;
        // there should only be the one default 'gridItem' at this point
        const gridItemHtml = groupHtml
            .getElementsByClassName("gridItem")[0]
            .cloneNode(true) as HTMLElement;
        if (this._templateBookUrls.length > 0) {
            // Remove original stub section
            this._scrollingDiv
                .getElementsByClassName("outerGroupContainer")[0]
                .removeChild(originalGroupDiv);
    ...
    private loadNextPageGroup(
        ...
        axios
            ...
            .then(result => {
                ...
                const originalPages = pageData.querySelectorAll(".bloom-page");
                let pages = new Array<HTMLElement>();
                for (let i = 0; i < originalPages.length; i++) {
                    const currentElement = originalPages.item(i);
                    if (!currentElement.id) {
                        return;
                    }
                    if (
                        currentElement.hasAttribute("data-page") &&
                        currentElement.getAttribute("data-page") === "extra"
                    ) {
                        pages.push(currentElement as HTMLElement);
                    }
                }
                if (pages.length == 0) {
                    console.log(
                ...
                const captionDiv = groupToAdd.getElementsByClassName(
                    "groupCaption"
                )[0] as HTMLElement;
                ...
                if (this._forChooseLayout) {
                    ...
                    const temp: Array<HTMLElement> = new Array<HTMLElement>();
                    for (let i = 0; i < pages.length; i++) {
                        const currentPage = pages[i];
                        if (
                            currentPage.id ===
                            "5dcd48df-e9ab-4a07-afd4-6a24d0398386"
                        ) {
                            continue;
                        }
            ...
            .catch(e => {
                ...
                const groupToAdd = groupHTML.cloneNode(true) as HTMLElement;
                this.setLocalizedText(
                    groupToAdd.getElementsByClassName(
                        "groupCaption"
                    )[0] as HTMLElement,
                    "TemplateBooks.BookName.",
                    templateTitle
                );
                const innerGroup = groupToAdd.getElementsByClassName(
                    "innerGroupContainer"
                )[0];
                innerGroup.remove();
    ...
    private loadPageFromGroup(
        ...
        pageArray.forEach((currentPageDiv, index) => {
            ...
            currentGridItemHtml.getElementsByClassName(
                "pageDescription"
            )[0].textContent = pageDescription;
            ...
            currentGroup
                .getElementsByClassName("innerGroupContainer")[0]
                .appendChild(currentGridItemHtml);
        }); // each
        // once the template pages are installed, attach click handler to them.
        const thumbCovers = currentGroup.getElementsByClassName(
            "invisibleThumbCover"
        );
        for (let i = 0; i < thumbCovers.length; i++) {
            const thumbCover = thumbCovers.item(i) as HTMLElement;
            if (thumbCover) {
                // satisfy compiler
    ...
    private countTranslationGroupsForChangeLayout(
        ...
        const allTranslationGroups = pageDiv.getElementsByClassName(
            "bloom-translationGroup"
        );
        const filteredTranslationGroups = new Array<HTMLElement>();
        for (let i = 0; i < allTranslationGroups.length; i++) {
            const current = allTranslationGroups[i] as HTMLElement;
            if (current.classList.contains("box-header-off")) {
                continue;
            }
            if (current.closest(".bloom-imageContainer") !== null) {
                continue;
            }
            filteredTranslationGroups.push(current);
        }
        return filteredTranslationGroups.length;
    }
