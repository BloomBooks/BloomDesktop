...
$(window).ready(() => {
    checkAxiosError(axios.get("/bloom/api/pageTemplates").then(result => {
...
class PageChooser {
    ...
    addPageClickHandler(): void {
        ...
        if (this._forChooseLayout) {
            checkAxiosError(axios.post("/bloom/api/changeLayout", { pageId: id, templateBookPath: templateBookPath }).catch(error => {
                // we seem to get unimportant errors here, possibly because the dialog gets closed before the post completes.
                console.log(error);
            }).then(() => this.closeup()));
        } else {
            checkAxiosError(axios.post("/bloom/api/addPage", { templateBookPath: templateBookPath, pageId: id }).catch(error => {
                console.log(error);
            }).then(() => this.closeup()));
        }
    }
