...
export interface ITemplateBook {
    ...
    pages: HTMLElement[];
    id: string; // from meta blookBookId
}
        ...
        const getTemplateFileDataAsync = async () => {
            ...
            for (const bookInfo of templateBooks) {
                ...
                try {
                    ...
                    const idMeta = fullDom.head.querySelector(
                        `meta[name="bloomBookId"]`
                    );
                    if (idMeta) {
                        id = idMeta.getAttribute("content") ?? "";
                    }
                    const idTool = fullDom.head.querySelector(
                        `meta[name="pageToolId"]`
                    );
                    if (idTool) {
                        pageToolId = idTool.getAttribute("content") ?? "";
                    }
                } catch (error) {
                    console.error(error);
