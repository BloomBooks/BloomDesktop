...
It lives here rather than under `toolbox/canvas/` because it is not a canvas utility and not a
toolbox tool: it is a command on the image context menu plus a dialog-like overlay, which makes
`bookEdit/copyrightAndLicense/` its closest structural neighbour. The canvas menu only *offers*
the feature — `canvasControlRegistry` imports `launchAiImageEditor`, and
`buildCanvasElementControlRegistryContext` imports `isAiEditableImageSrc` to decide whether the
item is enabled. Those two imports are the whole coupling; keep it that way.
