...
// Puts the fixture in the document and returns its test elements. Like the old Karma
// fixtures, it goes in a container rather than replacing the whole body, so it leaves
// alone anything else there, such as the hidden div MeasureText reuses between calls.
...
describe("OverflowChecker.IsOverflowingSelf", () => {
    ...
    // The old Karma test checked every case in turn in one document, which is also how
    // Bloom checks a page (and MeasureText keeps state between calls), so do that too.
