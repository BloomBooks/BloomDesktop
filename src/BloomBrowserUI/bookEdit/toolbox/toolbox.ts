...
export function cleanUpNbsps(editableDiv: HTMLElement) {
    ...
    setCkeditorBookmarkContent(editableDiv, "");
    ...
    // The text shows us what the adjacent characters so we can make a replacement decision, but
...
function setCkeditorBookmarkContent(editableDiv: HTMLElement, content: string) {
    // We actually only expect one; but being safe.
    const ckeBookmarks = editableDiv.querySelectorAll("[id^='cke_bm_']");
    ckeBookmarks.forEach(bm => (bm.innerHTML = content));
}
