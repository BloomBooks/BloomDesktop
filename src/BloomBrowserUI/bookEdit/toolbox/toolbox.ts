...
function RemoveSpanTags(editableDivHtml: string): string {
    return editableDivHtml.replace(/<span.*?>/g, "").replace(/<\/span.*?>/g, "");
}

// Check if the &nbsp; is at the start or end of a paragraph, regardless of any span tags
function NbspIsOnEdgeOfParagraph(editableDivHtml: string, nbspIndex: number): boolean {
    ...
    const beforeNbspWithoutSpanTags = RemoveSpanTags(beforeNbsp);
    const afterNbspWithoutSpanTags = RemoveSpanTags(afterNbsp);

    return (
        beforeNbspWithoutSpanTags.match(/<p[^>]*>$/) !== null ||
        afterNbspWithoutSpanTags.substring(0, 4) === "</p>"
    );
}
