...
/* Summary of how custom covers work
    - the pug for a standard front cover now has two margin boxes, one with class
    bloom-customMarginBox and one without. One is hidden and the other is visible
    (or in edit mode, one or the other is deleted except during operations that
    manipulate them, then restored when the page is saved. I decided to do this
    because various code searching the document was finding elements in the hidden
    marginBox...for example, code might try to initially focus a box there. I didn't
    want to have to keep on and on changing things to filter stuff in the inactive one).
    - the content of the custom cover is stored in the bloom-customMarginBox.
    The auto-layout content is kept in the regular marginBox. It's easy to switch
    which is shown, and our data-book mechanism keeps things that should match
    in sync.
    - a custom page is indicated by the presence of the class
    bloom-custom-cover on the bloom-page div.
    - a custom cover page margin box has a single bloom-canvas div,
    ...
    is saved in the data-div. We change the data-div copy a bit so that
    operations looking for all elements of certain kinds in the DOM don't
    find these data-div copies (even in older versions of Bloom).
...
function finishReactivatingPage(page: HTMLElement): void {
    ...
    ensureDerivedFieldsFitOnCustomCover(page);
    getToolboxBundleExports()?.applyToolboxStateToPage();
}

function ensureDerivedFieldsFitOnCustomCover(page: HTMLElement): void {
...
export function setupCoverMenu(): void {
    ...
    if (usingLegacyTheme && page.classList.contains("bloom-custom-cover")) {
        postString("editView/toggleCustomCover", page.getAttribute("id")!);
        return;
    }
    ...
    if (page.classList.contains("bloom-custom-cover")) {
        ensureDerivedFieldsFitOnCustomCover(page as HTMLElement);
    }
}
...
function renderCoverMenu(page: HTMLElement, container: HTMLElement): void {
    ...
    ReactDOM.render(
        <CustomCoverMenu
            ...
            disableCustomCover={usingLegacyTheme}
            setCustom={(selection) => {
