...
/* Summary of how custom covers work
	...
	into the new page as canvas elements. We'll put a class on such a page, bloom-hasCustomLayout,
	so that future switches to custom don't do this, but just reinstate the data-div material
	...
	a current Bloom, the custom layout will be restored as usual when updating xmatter.
...
export function convertXmatterPageToCustom(page: HTMLElement): void {
    ...
    // Remember that the page has custom layout, so that if the user switches
    // back to standard and then back to custom, we will reinstate it.
    page.classList.add("bloom-hasCustomLayout");
...
function renderPageLayoutMenu(page: HTMLElement, container: HTMLElement): void {
    ...
    const alreadyHasCustomLayout = page.classList.contains(
        "bloom-hasCustomLayout",
    );
    const usingLegacyTheme = isUsingLegacyTheme();
    ReactDOM.render(
        <CustomPageLayoutMenu
            ...
            setCustom={(selection) => {
                ...
                if (
                    selection === "standard" ||
                    (selection === "custom" && alreadyHasCustomLayout)
                ) {
                    // We'll leave it up to toggleCustomPageLayout to actually change the class on
                    ...
                    // making a custom cover for the first time, or forced start-over
