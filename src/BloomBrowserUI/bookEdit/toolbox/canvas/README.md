...
Both give way when the context's `tablesMayBeRestructured` is false (below Pro, or with the Tables experiment off). A right-click in such a table opens no Cell menu (`installHostHooks` in `tableEditing.ts`), and the "..." button follows the same rule: every cell's picture gets `menuSections: ["image"]` without `becomeBackground`, and neither `opensTableCellMenu` nor `includesTableCellMenuItems`.
