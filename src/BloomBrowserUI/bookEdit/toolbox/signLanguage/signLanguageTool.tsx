...
export class SignLanguageTool extends ToolboxToolReactAdaptor {
    ...
    // Make an overlay and slap it over the edit pane video still while we're recording
    public static showOverlayToHideVideo(): void {
        const containers = SignLanguageTool.getVideoContainers(true);
        for (var i = 0; i < containers.length; i++) {
            let containerElement: Element = containers[i];
            if (!containerElement.previousElementSibling.classList.contains("bloom-videoOverlay")) {
                ...
                containerElement.parentElement.insertBefore(
                    SignLanguageTool.createNode(containerElement.ownerDocument, overlayDiv),
                    containerElement);
            }
        }
    }
    ...
    public static addRecordingLabelToOverlay(reactDoc: Document): void {
        ...
        for (var i = 0; i < containers.length; i++) {
            containers[i].previousElementSibling.firstChild.textContent = recordingLabel;
        }
    }
    ...
    public static removeVideoOverlay(): void {
        const containers = SignLanguageTool.getVideoContainers(true);
        for (var i = 0; i < containers.length; i++) {
            containers[i].previousElementSibling.remove();
        }
    }
