...
export default class AudioRecording {
    ...
    public async pullDefaultRecordingModeAsync(): Promise<void> {
        const promise = BloomApi.getWithPromise(
            "talkingBook/asdfdefaultAudioRecordingMode"
        );
        const result: void | AxiosResponse = await promise;

        if (result) {
            // This means it's not void... because void would imply result is null or undefined
            ...
            // Well, the reason this returned void is because BloomApi.getWithPromise promised to do our error handling for us.
            // So, let's just return void here instead of creating a new rejected promise.
