...
export default class AudioRecording {
    ...
    public removeRecordingSetup() {
        ...
        page.find('.ui-audioCurrent').removeClass('ui-audioCurrent').removeClass("checking");
        var socket = this.getWebSocket();
        if (socket) {
    ...
    private setCurrentSpan(current: JQuery, changeTo: JQuery, checking?: boolean): void {
        if (current) {
            current.removeClass('ui-audioCurrent').removeClass("checking");
        }
        if (checking) {
            changeTo.addClass('checking'); // prevents highlight showing at once
            axios.get("/bloom/api/audio/checkForSegment?id=" + changeTo[0].id).then(response => {
                if (response.data === "exists") {
                    changeTo.removeClass("checking");
                }
            }).catch(error => {
