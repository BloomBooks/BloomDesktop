...
export default class AudioRecording {
    ...
    public getCurrentAudioSentence(): HTMLElement | null {
        ...
        while (
            ...
            // This is a special case. When we have switched back from sentence recording to text box recording,
            // but not yet made a new recording, the current highlight is the bloom-editable, so that's where
            // currToExamine starts out. But as yet, that element does not have the audio-sentence class,
            // so if we don't also check for bloom-editable, we'll search all the way up and return nothing.
