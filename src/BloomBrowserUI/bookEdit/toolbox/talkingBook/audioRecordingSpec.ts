    ...
    */
    ...
    describe("- custom split highlights", () => {
        ...
        it("uses only ui-enableHighlight descendants for current highlight when its own background is disabled", async () => {
            SetupIFrameFromHtml(
                '<div id="page1"><div class="bloom-editable" data-audiorecordingmode="Sentence"><p><span id="span1" class="audio-sentence ui-audioCurrent ui-disableHighlight"><span class="ui-enableHighlight">One</span>&nbsp;&nbsp; <span class="ui-enableHighlight">Two</span></span></p></div></div>',
            );
        ...
        it("copies the current highlight colors from computed highlight styles", async () => {
            SetupIFrameFromHtml(
                '<div id="page1"><div class="bloom-editable" data-audiorecordingmode="Sentence"><p><span id="span1" class="audio-sentence ui-audioCurrent" style="background-color: rgb(1, 2, 3); color: rgb(4, 5, 6);">One.</span></p></div></div>',
            );
        ...
        it("clears split highlights even if ui-audioCurrent has already been lost", () => {
            SetupIFrameFromHtml(
                '<div id="page1"><div class="bloom-translationGroup"><div id="box1" class="bloom-editable audio-sentence bloom-postAudioSplit" data-audiorecordingmode="TextBox" data-audiorecordingendtimes="1.0 2.0"><p><span id="span1" class="bloom-highlightSegment">One.</span><span id="span2" class="bloom-highlightSegment">Two.</span></p></div></div></div>',
            );

            const recording = new AudioRecording();
            const highlightManager = new AudioTextHighlightManager();
            const currentTextBox = getFrameElementById(
                "page",
                "box1",
            ) as HTMLElement;
            highlightManager.refreshHighlights(currentTextBox, currentTextBox);

            expect(getSplitHighlightTexts()).toEqual([["One."], ["Two."], []]);

            const clearAudioSplit = (
                recording as unknown as {
                    clearAudioSplit(): void;
                }
            ).clearAudioSplit.bind(recording);

            clearAudioSplit();

            expect(
                currentTextBox.classList.contains("bloom-postAudioSplit"),
            ).toBe(false);
            expect(
                currentTextBox.hasAttribute("data-audiorecordingendtimes"),
            ).toBe(false);
            expect(getSplitHighlightTexts()).toEqual([[], [], []]);
        });
