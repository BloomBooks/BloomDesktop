...
describe("talking book tests", () => {
    ...
    describe("- tests for sentence splitting", () => {
        ...
        describe("showTool(checksum=missing, audio=missing, scenario=*) => UPDATE", () => {
            ...
            it("showTool(checksum=missing, audio=missing, scenario=PureSentence) => UPDATE", async () => {
                return runNoChecksumNoAudioTestAsync(AudioMode.PureSentence);
            });

            it("showTool(checksum=missing, audio=missing, scenario=PreTextBox) => UPDATE", async () => {
                return runNoChecksumNoAudioTestAsync(AudioMode.PreTextBox);
            });

            it("showTool(checksum=missing, audio=missing, scenario=PureTextBox) => UPDATE", async () => {
                return runNoChecksumNoAudioTestAsync(AudioMode.PureTextBox);
            });

            it("showTool(checksum=missing, audio=missing, scenario=HardSplitTextBox) => UPDATE", async () => {
                return runNoChecksumNoAudioTestAsync(
                    AudioMode.HardSplitTextBox
                );
            });
            it("showTool(checksum=missing, audio=missing, scenario=SoftSplitTextBox) => UPDATE", async () => {
                return runNoChecksumNoAudioTestAsync(
        ...
        describe("showTool(checksum=missing, audio=present, scenario=*) => SKIP UPDATE", () => {
            ...
            it("showTool(checksum=missing, audio=present, scenario=PureSentence) => SKIP UPDATE", async () => {
                return runNoChecksumYesAudioTestAsync(AudioMode.PureSentence);
            });

            it("showTool(checksum=missing, audio=present, scenario=PreTextBox) => SKIP UPDATE", async () => {
                return runNoChecksumYesAudioTestAsync(AudioMode.PreTextBox);
            });

            it("showTool(checksum=missing, audio=present, scenario=PureTextBox) => SKIP UPDATE", async () => {
                return runNoChecksumYesAudioTestAsync(AudioMode.PureTextBox);
            });

            it("showTool(checksum=missing, audio=present, scenario=HardSplitTextBox) => SKIP UPDATE", async () => {
                return runNoChecksumYesAudioTestAsync(
                    AudioMode.HardSplitTextBox
                );
            });

            it("showTool(checksum=missing, audio=present, scenario=SoftSplitTextBox) => SKIP UPDATE", async () => {
                return runNoChecksumYesAudioTestAsync(
                    AudioMode.SoftSplitTextBox
                );
            });
        ...
        describe("showTool(checksum=same, audio=missing, scenario=*) => UPDATE", () => {
            ...
            it("showTool(checksum=same, audio=missing, scenario=PureSentence) => UPDATE", async () => {
                return runSameChecksumNoAudioTestAsync(AudioMode.PureSentence);
            });

            it("showTool(checksum=same, audio=missing, scenario=PreTextBox) => UPDATE", async () => {
                return runSameChecksumNoAudioTestAsync(AudioMode.PreTextBox);
            });

            it("showTool(checksum=same, audio=missing, scenario=PureTextBox) => UPDATE", async () => {
                return runSameChecksumNoAudioTestAsync(AudioMode.PureTextBox);
            });

            it("showTool(checksum=same, audio=missing, scenario=HardSplitTextBox) => UPDATE", async () => {
                return runSameChecksumNoAudioTestAsync(
                    AudioMode.HardSplitTextBox
                );
            });

            it("showTool(checksum=same, audio=missing, scenario=SoftSplitTextBox) => UPDATE", async () => {
                return runSameChecksumNoAudioTestAsync(
                    AudioMode.SoftSplitTextBox
                );
            });
        ...
        describe("showTool(checksum=same, audio=partial, scenario=*) => UPDATE", () => {
            ...
            it("showTool(checksum=same, audio=partial, scenario=PureSentence) => UPDATE", async () => {
                return runSameChecksumPartialAudioTestAsync(
                    AudioMode.PureSentence
                );
            });

            it("showTool(checksum=same, audio=partial, scenario=PreTextBox) => UPDATE", async () => {
                return runSameChecksumPartialAudioTestAsync(
                    AudioMode.PreTextBox
                );
            });

            it("showTool(checksum=same, audio=partial, scenario=HardSplitTextBox) => UPDATE", async () => {
                return runSameChecksumPartialAudioTestAsync(
                    AudioMode.HardSplitTextBox
                );
            });
        ...
        describe("showTool(checksum=same, audio=present, scenario=*) => SKIP UPDATE", () => {
            ...
            it("showTool(checksum=same, audio=present, scenario=PureSentence) => SKIP UPDATE", async () => {
                return runSameChecksumYesAudioTestAsync(AudioMode.PureSentence);
            });

            it("showTool(checksum=same, audio=present, scenario=PreTextBox) => SKIP UPDATE", async () => {
                return runSameChecksumYesAudioTestAsync(AudioMode.PreTextBox);
            });

            it("showTool(checksum=same, audio=present, scenario=PureTextBox) => SKIP UPDATE", async () => {
                return runSameChecksumYesAudioTestAsync(AudioMode.PureTextBox);
            });

            it("showTool(checksum=same, audio=present, scenario=HardSplitTextBox) => SKIP UPDATE", async () => {
                return runSameChecksumYesAudioTestAsync(
                    AudioMode.HardSplitTextBox
                );
            });

            it("showTool(checksum=same, audio=present, scenario=SoftSplitTextBox) => SKIP UPDATE", async () => {
                return runSameChecksumYesAudioTestAsync(
                    AudioMode.SoftSplitTextBox
                );
            });
        ...
        describe("showTool(checksum=differs, recording=*, scenario=*) => UPDATE", () => {
            ...
            it("showTool(checksum=differs, recording=*, scenario=PureSentence) => UPDATE", async () => {
                return runDifferentChecksumTestAsync(AudioMode.PureSentence);
            });

            it("showTool(checksum=differs, recording=*, scenario=PreTextBox) => UPDATE", async () => {
                return runDifferentChecksumTestAsync(AudioMode.PreTextBox);
            });

            it("showTool(checksum=differs, recording=*, scenario=PureTextBox) => UPDATE", async () => {
                return runDifferentChecksumTestAsync(AudioMode.PureTextBox);
            });

            it("showTool(checksum=differs, recording=*, scenario=HardSplitTextBox) => UPDATE", async () => {
                return runDifferentChecksumTestAsync(
                    AudioMode.HardSplitTextBox
                );
            });

            it("showTool(checksum=differs, recording=*, scenario=SoftSlpitTextBox) => UPDATE", async () => {
                return runDifferentChecksumTestAsync(
                    AudioMode.SoftSplitTextBox
                );
            });
