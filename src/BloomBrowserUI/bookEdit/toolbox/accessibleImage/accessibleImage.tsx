...
export class AccessibleImageControls extends React.Component<{}, IState> {
            ...
            <RequiresBloomEnterpriseWrapper>
                <div className="accessibleImageBody">
                    ...
                    <ApiBackedCheckbox
                        ...
                        onCheckChanged={simulate =>
                            this.updateCbCheck(simulate)
                        }
                    >
                        Color Blindness
                    </ApiBackedCheckbox>
                    <RadioGroup
                        onChange={val => this.updateCbRadio(val)}
    ...
    private updateCbCheck(simulate: boolean) {
        this.simulatingColorBlindness = simulate;
        this.updateSimulations();
    }

    private updateCbRadio(mode: string) {
    ...
    public updateSimulations() {
        ...
        AccessibleImageControls.removeCbMarkup();
        if (this.simulatingColorBlindness) {
            ...
            for (var i = 0; i < containers.length; i++) {
                ...
                for (var imgIndex = 0; imgIndex < images.length; imgIndex++) {
                    this.makeCbOverlay(images[imgIndex]);
                }
            }
        } else {
    ...
    private makeCbOverlay(img: HTMLImageElement) {
        if (
            !img.complete ||
            ...
            window.setTimeout(() => this.makeCbOverlay(img), 100);
            return;
        }
        var page = ToolboxToolReactAdaptor.getPage();
