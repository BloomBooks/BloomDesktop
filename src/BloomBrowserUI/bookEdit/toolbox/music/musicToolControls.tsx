...
export default class MusicToolControls extends React.Component<{}, IMusicState> {
    ...
    getPlayer(): HTMLMediaElement {
        if (!this.player) {
            this.player = document.getElementById("musicPlayer") as HTMLMediaElement;
            this.player.addEventListener("ended", () => this.setState({ playing: false }));
        }
        return this.player;
    }
    ...
    previewMusic() {
        ...
        this.player.setAttribute("src", musicUrl + "?nocache=" + new Date().getTime());
        this.player.volume = this.getAudioVolume(audioStr);
        this.player.play();
