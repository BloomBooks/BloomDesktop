...
const ComicToolControls: React.FunctionComponent = () => {
    // Don't ever let this see the light of day; but it's a useful placeholder with the right type.
    const emptyTailSpec: TailSpec = {
        tipX: 0,
        tipY: 0,
        midpointX: 0,
        midpointY: 0
    ...
    const handleShowTailChanged = (value: boolean) => {
        ...
        // Update the Comical canvas on the page frame
        let tailSpec: TailSpec = emptyTailSpec;
        // Avoid getting a "real" tailspec from Comical, if we don't need one.
        if (value) {
            tailSpec = ComicTool.bubbleManager().getDefaultTailSpec() as TailSpec;
        }
        ComicTool.bubbleManager().updateSelectedItemBubbleSpec({
            tails: value ? [tailSpec] : []
        });
    };
