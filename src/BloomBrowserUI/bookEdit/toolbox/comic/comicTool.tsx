...
const ComicToolControls: React.FunctionComponent = () => {
    ...
    const handleRoundedCornersChanged = (newValue: boolean | undefined) => {
        ...
        const bubbleMgr = ComicTool.bubbleManager();
        if (bubbleMgr) {
            const radius = newValue ? 8 : undefined;
            bubbleMgr.updateSelectedItemBubbleSpec({
    ...
    const styleSupportsRoundedCorners = (
        ...
        switch (currentBubbleSpec.style) {
            ...
            case "none":
                ...
                return (
                    currentBubbleSpec.backgroundColors &&
                    currentBubbleSpec.backgroundColors.length > 0
                );
            default:
                return false;
        }
            ...
            >
                <form autoComplete="off">
                    <FormControl>
                        ...
                        <div
                            className={
                                // If rounded corners is not supported, the hidden class will make it not display,
                                // instead of merely disabled but still there (as with the MuiCheckbox disabled parameter).
                                styleSupportsRoundedCorners(currentBubbleSpec)
                                    ? ""
                                    : "hidden"
                            }
                        >
