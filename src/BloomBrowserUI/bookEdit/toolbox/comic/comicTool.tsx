...
const ComicToolControls: React.FunctionComponent = () => {
    ...
    // We keep track of the text color going into the color chooser, in case the user cancels.
    const [oldTextColor, setOldTextColor] = useState(defaultTextColors[0]);
    ...
    // We keep track of the background color going into the color chooser, in case the user cancels.
        ...
        if (currentBubbleSpec) {
            ...
            if (!isSpecialColorName(backColor)) {
                const newSwatch = getSwatchFromHex(
                    backColor,
                    "somename" // We need to be able to get a custom name from the dialog getNewCustomSwatchName()
                );
                ...
                setOldBackgroundColor(newSwatch);
            } else {
    ...
    const launchTextColorChooser = () => {
        const colorPickerDialogProps: IColorPickerDialogProps = {
            ...
            title: "Text Color",
            titleL10nKey: "EditTab.Toolbox.ComicTool.Options.TextColor",
            confirmButtonLabel: "OK",
            confirmButtonLabelL10nKey: "Common.OK",
            onDialogClose: dialogResult => {
                if (dialogResult === DialogResult.Cancel) {
                    updateTextColor(oldTextColor);
                } else {
                    setOldTextColor(textColorSwatch);
                }
            },
    ...
    const launchBackgroundColorChooser = () => {
        const colorPickerDialogProps: IColorPickerDialogProps = {
            ...
            titleL10nKey: "EditTab.Toolbox.ComicTool.Options.BackgroundColor",
            confirmButtonLabel: "OK",
            confirmButtonLabelL10nKey: "Common.OK",
            onDialogClose: dialogResult => {
                if (dialogResult === DialogResult.Cancel) {
                    updateBackgroundColor(oldBackgroundColor);
                } else {
                    setOldBackgroundColor(backgroundColorSwatch);
                }
            },
