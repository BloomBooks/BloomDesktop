...
describe("Bloom Edit Controls tests", function() {
    ...
    beforeEach(function() {
        ...
        getTheOneReaderToolsModel().setElementAttribute = function(elementId, attrName, val) {
            classValues[elementId] = val;
        };

        //noinspection JSUnusedLocalSymbols
        getTheOneReaderToolsModel().getElementAttribute = function(elementId, attrName) {
...
*/
    it("increments level to limit on level right button", function() {
        ...
        (<any>getTheOneReaderToolsModel().updateElementContent).calls.reset();
        getTheOneReaderToolsModel().incrementLevel();
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("levelNumber", "3");
...
*/
    ...
    it("updates level button visibility when setting level", function() {
        getTheOneReaderToolsModel().setLevelNumber(3);
        expect(getTheOneReaderToolsModel().getElementAttribute("decLevel", "class")).toBe("something");
        expect(getTheOneReaderToolsModel().getElementAttribute("incLevel", "class")).toBe("something disabledIcon");

        getTheOneReaderToolsModel().decrementLevel();
        expect(getTheOneReaderToolsModel().getElementAttribute("incLevel", "class")).toBe("something");
        expect(getTheOneReaderToolsModel().getElementAttribute("decLevel", "class")).toBe("something");

        getTheOneReaderToolsModel().decrementLevel();
        expect(getTheOneReaderToolsModel().getElementAttribute("incLevel", "class")).toBe("something");
        expect(getTheOneReaderToolsModel().getElementAttribute("decLevel", "class")).toBe("something disabledIcon");
...
/* skipping due to mistery. See BL-3554
    ...
    */
    ...
    it ("sets selected class when sort button clicked", function() {
        ...
        getTheOneReaderToolsModel().sortByLength();
        expect(getTheOneReaderToolsModel().getElementAttribute("sortAlphabetic", "class")).toBe("sortItem");
        expect(getTheOneReaderToolsModel().getElementAttribute("sortLength", "class")).toBe("sortItem sortIconSelected");

        getTheOneReaderToolsModel().sortByFrequency();
        expect(getTheOneReaderToolsModel().getElementAttribute("sortLength", "class")).toBe("sortItem");
        expect(getTheOneReaderToolsModel().getElementAttribute("sortFrequency", "class")).toBe("sortItem sortIconSelected");

        getTheOneReaderToolsModel().sortAlphabetically();
        expect(getTheOneReaderToolsModel().getElementAttribute("sortFrequency", "class")).toBe("sortItem");
        expect(getTheOneReaderToolsModel().getElementAttribute("sortAlphabetic", "class")).toBe("sortItem sortIconSelected");
    ...
    it ("updates stage count and buttons on init", function() {
        getTheOneReaderToolsModel().updateControlContents();
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("numberOfStages", "3");
        expect(getTheOneReaderToolsModel().getElementAttribute("decStage", "class")).toBe("something disabledIcon");
    });

    it ("updates level buttons on init", function() {
        getTheOneReaderToolsModel().updateControlContents();
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("numberOfLevels", "3");
        expect(getTheOneReaderToolsModel().getElementAttribute("decLevel", "class")).toBe("something disabledIcon");
    });
    ...
    it("sets level max values on init", function() {
        getTheOneReaderToolsModel().updateControlContents();
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("maxWordsPerPage", "6");
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("maxWordsPerPageBook", "6");
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("maxWordsPerBook", "90");
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("maxWordsPerSentence", "3");
        //expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("maxUniqueWordsPerBook", "0");
        expect(getTheOneReaderToolsModel().getElementAttribute("maxWordsPerBook", "class")).toBe("");
    });

    it("updates max values when level changes", function() {
        getTheOneReaderToolsModel().incrementLevel();
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("maxWordsPerPage", "10");
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("maxWordsPerPageBook", "10");
        expect(getTheOneReaderToolsModel().updateElementContent).not.toHaveBeenCalledWith("maxWordsPerBook", "0");
        expect(getTheOneReaderToolsModel().updateElementContent).toHaveBeenCalledWith("maxWordsPerSentence", "5");
