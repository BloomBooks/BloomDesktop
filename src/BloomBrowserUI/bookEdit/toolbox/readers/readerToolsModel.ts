...
export class ReaderToolsModel {
    ...
    public updateWordList(): void {
        ...
        if (mlwld) {
            ...
            // Can't use the newly refactored 'setDisplayForHTMLElement()' here
            // because here we're setting the parentElement's style. Not sure why...
            const ltrList = document.getElementById("letterList");
            if (ltrList && ltrList.parentElement) {
                ltrList.parentElement.style.display = useAllowedWords
                    ? "none"
                    : "";
            }
            this.setDisplayForHTMLElement(
            ...
            this.setDisplayForHTMLElement("sortFrequency", useAllowedWords);
            this.setDisplayForHTMLElement(
    ...
    private setDisplayForHTMLElement(
        ...
        const elt = document.getElementById(elementName);
        if (!elt) return;
        elt.style.display = useAllowedWords ? "" : "none";
    }
    ...
    public setMarkupType(markupType: number): boolean {
        ...
        if (newMarkupType !== this.currentMarkupType) {
            const window = this.safelyGetContentWindow();
            if (window)
                $(".bloom-editable", window.document).removeSynphonyMarkup();
    ...
    public getElementsToCheck(): JQuery {
        ...
        const cover = $("body", window.document).find("div.cover");
        if (cover["length"] > 0) return $();

        // not a cover page, return elements to check
        return (
            $(".bloom-page", window.document)
                .not(".bloom-frontMatter, .bloom-backMatter")
    ...
    public getPageWindow(): Window | null {
        ...
        switch (this.currentMarkupType) {
            case MarkupType.Leveled:
                if (editableElements.length > 0) {
                    ...
                    const window = this.getPageWindow();
                    if (window) {
                        const pageDiv = $("body", window.document).find(
                            "div.bloom-page"
                        );
                        if (pageDiv.length) {
                            if (pageDiv[0].id)
    ...
    public getToolboxWindow(): Window | null {
        // ReaderToolsModel is only loaded into the toolbox, so the toolbox window is simply the current window.
        const toolboxElt = document.getElementById("toolbox");
        if (!toolboxElt) return null;
        return (<HTMLIFrameElement>toolboxElt).contentWindow;
    }
