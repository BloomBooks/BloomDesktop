...
var synphony_lib_1 = require('./synphony_lib');
require('./bloom_lib.js'); //add several functions to LanguageData
/**
...
(function ($) {
    ...
    $.fn.checkLeveledReader = function (options) {
        ...
        var checkLeaf = function (leaf) {
            ...
            var fragments = synphony_lib_1.theOneLibSynphony.stringToSentences($(leaf).html());
            var newHtml = '';
    ...
    $.fn.checkDecodableReader = function (options) {
         ...
         */
        var results = synphony_lib_1.theOneLibSynphony.checkStory(opts.focusWords, opts.previousWords, opts.knownGraphemes, text, opts.sightWords.join(' '));
        // markup
        this.each(function () {
            ...
            if ((html.trim().length > 0) && (html !== '<br>')) {
                html = synphony_lib_1.theOneLibSynphony.wrap_words_extra(html, results.sight_words, cssSightWord, ' data-segment="word"');
                html = synphony_lib_1.theOneLibSynphony.wrap_words_extra(html, results.possible_words, cssPossibleWord, ' data-segment="word"');
                // remove numbers from list of bad words
                var notFound = _.difference(results.remaining_words, results.getNumbers());
                html = synphony_lib_1.theOneLibSynphony.wrap_words_extra(html, notFound, cssWordNotFound, ' data-segment="word"');
                $(this).html(html);
            }
    ...
    $.fn.getMaxSentenceLength = function () {
        ...
        this.each(function () {
            // split into sentences
            var fragments = synphony_lib_1.theOneLibSynphony.stringToSentences(removeAllMarkup($(this).html()));
...
//# sourceMappingURL=jquery.text-markup.js.map
