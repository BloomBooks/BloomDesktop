...
const DragActivityControls: React.FunctionComponent<{
    ...
    const showDialogToChooseSoundFileAsync = async (which: soundType) => {
        ...
        setSound(which, result.data, false); // already copied, and not in our sounds folder
    ...
    const onSoundItemChosen = (which: soundType, newSoundId: string) => {
        if (newSoundId === "choose") {
            showDialogToChooseSoundFileAsync(which);
            return;
        }
        if (
            (newSoundId === correctSound && which === "correct") ||
            (newSoundId === wrongSound && which === "wrong") ||
            (newSoundId === imageSound && which === "image")
        ) {
        ...
        setSound(which, newSoundId, true);
    };
    const setSound = (
        which: soundType,
        newSoundId: string,
...
const CorrectWrongControls: React.FunctionComponent<{
    ...
    currentSound: string;
    onSoundItemChosen: (which: soundType, value: string) => void;
}> = props => {
    return (
        <div>
...
const soundSelect = (
    ...
    value: string,
    setValue: (which: soundType, value: string) => void
) => {
    return (
        <Select
            css={css`
