...
// I've called it animationControl to distinguish it from other code in various places that
// actually performs the animation.
...
export default class AnimationControl implements ITabModel {
    ...
    showTool() {
        $("input[name='zoomAndPan']").change(() => this.zoomAndPanChanged());
        $('#animation-play-wrapper').click(() => this.previewAnimation())
        this.updateMarkup();
    }
    hideTool() {
    ...
    zoomAndPanChanged() {
        ...
        const zoomAndPanEnabled = $("input[name='zoomAndPan']").prop("checked");
    ...
    // - This code is simpler because there is only ever one animation-capable image in a document (for now, anyway, since bloom only
    // displays one page at a time in edit mode and we only support animating the first image)
    ...
    previewAnimation() {
        ...
        const firstImage = page.find(".bloom-imageContainer").first();
        if (!firstImage.length || !$("input[name='zoomAndPan']").prop("checked")) {
            return;
        }
        var duration = 0;
...
ToolBox.getTabModels().push(new AnimationControl());
