...
export function inlineImageCanUndo(): boolean {
    ...
    // person has not edited since. Without that second condition we would go first and bring
    // the picture back before their edit, which is the wrong order and the one thing these
    // independent stacks can get wrong (see the comment on the chain in
    // workspaceRoot.handleUndo). Editing after a delete is what hands ctrl+z back to CKEditor.
