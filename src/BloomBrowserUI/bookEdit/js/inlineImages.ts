...
type InlineImageUndoItem = {
    ...
    // again -- and CKEditor holds undo points for both halves of it.
...
/**
 * Whether the person has edited the block since this snapshot was taken. Two ways to tell,
 * because neither sees everything: the content comparison catches an edit nobody reported
 * (including one that changes only markup, such as bolding a word), and the reported flag
 * catches an edit that left the content identical.
 */
function hasEditedSinceInlineImageSnapshot(item: InlineImageUndoItem): boolean {
    if (item.wasEditedSinceSnapshot) return true;
    return (
...
export function noteInlineImageBlockWasEdited(element: HTMLElement): void {
    ...
    if (!translationGroup) return;
    const top = inlineImageUndoStack[inlineImageUndoStack.length - 1];
    if (top?.translationGroup === translationGroup)
        top.wasEditedSinceSnapshot = true;
