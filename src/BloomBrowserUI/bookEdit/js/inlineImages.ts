...
export function inlineImageCanUndo(): boolean {
    ...
    // AND that the person has not edited since the snapshot. Without that second condition we
    // would go first and bring the picture back before their edit, which is the wrong order and
    // the one thing these independent stacks can get wrong (see the comment on the chain in
    // workspaceRoot.handleUndo). Editing after an image operation is what hands ctrl+z back to
    // CKEditor.
    const focused = getElementWithFocusOrSelection();
    if (!focused || !top.translationGroup.contains(focused)) return false;
    return (
        getInlineImageUndoContentFingerprint(top.translationGroup) ===
        top.contentAtSnapshot
    );
}
