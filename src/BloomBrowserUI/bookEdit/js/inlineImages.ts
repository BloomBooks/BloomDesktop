 ...
 * nothing at all once CKEditor ran out. So it is paired with CKEditor's own answer: while
 * CKEditor still holds an undo point the person's editing really is ahead of ours, and when
 * it has none there is nothing left for the flag to be deferring to.
 */
function hasEditedSinceInlineImageSnapshot(item: InlineImageUndoItem): boolean {
    if (item.wasEditedSinceSnapshot && ckeditorHasSomethingToUndo())
        return true;
    return (
...
/**
 * Whether CKEditor has text editing it could undo. Read off the global the page's CKEditor
 * puts up, since there is no api for it -- the same implementation secret editablePage.ts's
 * ckeditorCanUndo relies on, and the same fallback: no CKEditor means nothing to undo.
 */
function ckeditorHasSomethingToUndo(): boolean {
    ...
    ).CKEDITOR?.currentInstance?.undoManager as
        | { undoable?: () => boolean }
        | undefined;
