...
export function SetupElements(
    ...
    runWhenEditTabBundleAvailable(
        // This function will be called when the toolbox is loaded and the functions
        // it exports to other iframes are available to be called. Anything that depends
        // on the toolbox that you want to do while the page is being initialized should
        // be called from inside here, since this method is called at startup and the
        // toolbox may not have loaded.
        // Note that there is some randomness involved in whether this function will be called immediately
        // (before the rest of this method executes) or later.
        // If this method is being called at a time other than startup, it should always execute immediately.
        (rootFrameExports) => {
            ...
            // HACK for BL-1139: For some reason when the Reader tools are active, focusing causes
            ...
            // The setTimeout is so the rest of SetupElements (and any toolbox-driven DOM changes)
            // finishes before we try to set focus.
            window.setTimeout(() => {
                focusFirstEditableField();
            }, 0);
        },
        // This function is called (hopefully never) only if the toolbox code fails
        // to load after the longest delay we think likely. It's doubtful we can
        // continue usefully.
        // We could, in theory, still try to set focus, but the timeout is long enough (seconds)
        // that modifying focus at this point could be quite confusing to the user.
        // So we'll do nothing.
        () => {},
    );
