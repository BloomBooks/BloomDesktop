...
function requestPageContentInternal() {
    ...
    try {
        ...
        // Arguably, now we have the content to save, we might want to hideInactiveMarginBox() again,
        // but there are already many reasons why we must reload the page entirely after saving,
        // so doing so is a waste of time.
        postString(
...
export function hideInactiveMarginBox() {
    ...
    if (customMarginBox) {
        ...
        const marginBoxes = Array.from(
            customMarginBox.parentElement!.getElementsByClassName("marginBox"),
        ) as HTMLElement[];
        if (marginBoxes.length <= 1) {
            // If there's only one, we don't want to hide it! Possibly this function got called
            // twice by mistake, without an intervening restore. In that case, we'd like to keep
            // the value of inactiveMarginBox that we saved previously.
            return;
        }
        if (page.classList.contains("bloom-custom-cover")) {
            inactiveMarginBox = marginBoxes.find(
