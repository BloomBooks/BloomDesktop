...
function UpdateOverlay(container, img) {
    ...
    if (!copyright || copyright.length === 0) {
        ...
        theOneLocalizationManager.asyncGetText("EditTab.Image.MissingInfo",
                "Image is missing information on Credits, Copyright, or License", "tooltip text")
            .done(translation => {
               var title = translation.replace(/'/g, "&apos;");
                $(container).prepend("<button class='editMetadataButton imageButton imgMetadataProblem " + buttonModifier +
                    "' title='" + title + "'></button>");
                })
            .fail(() => {
                $(container).prepend("<button class='editMetadataButton imageButton imgMetadataProblem " + buttonModifier +
                    "' title='Image is missing information on Credits, Copyright, or License'></button>");
             });
    }
}
...
function SetAlternateTextOnImages(element) {
    if (GetRawImageUrl(element).length > 0) { //don't show this on the empty license image when we don't know the license yet
        ...
        theOneLocalizationManager.asyncGetText("EditTab.Image.AltMsg", "This picture, {0}, is missing or was loading too slowly.",
