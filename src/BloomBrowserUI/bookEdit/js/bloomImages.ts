...
// Returns true when `color` is a non-empty, non-near-white CSS color string — i.e., when
// a page with this background color needs image transparency applied. Combines the empty
// check (no background set) with the near-white check so callers don't duplicate the logic.
export function pageBackgroundNeedsTransparency(color: string): boolean {
    return !!color && !isNearWhite(color);
}
