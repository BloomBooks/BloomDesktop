 ...
 *
 * On top of that, a calendar month grid never gets the structural chrome (see
 * tableGetsStructuralChrome), its Cell menu offers only what cellMenuOffersItemInBloom
 * allows, and a picture in one gets Bloom's own menu (see bloomOpensCellMenu).
 */
function installHostHooks(): void {
    setStructuralChromeGate(
        (table) =>
            tablesMayBeRestructured() && tableGetsStructuralChrome(table),
    );
    setCellMenuItemFilter(cellMenuOffersItemInBloom);
    setCellMenuOpenHandler(
        (cell, table, position) =>
            !tablesMayBeRestructured() ||
            bloomOpensCellMenu(cell, table, position),
    );
}
