...
jQuery.fn.IsOverflowingSelf = function () {
    var element = this[0];
...
jQuery.fn.IsOverflowingMargins = function () {
    var element = this[0];
    // Ignore Topic divs as they are chosen from a list
    if (element.hasAttribute('data-book') && element.getAttribute('data-book') == "topic") {
        return false;
...
// but is also called from within StyleEditor
function MarkOverflowInternal(box) {
    ...
    if ($box.IsOverflowingSelf()) {
        ...
        if (HasSplitParentThatOverflows($box)) {
            $box.addClass('overflow');
        }
    }
    ...
    editablePageElements.each(function() {
       $this = $(this);
        if ($this.IsOverflowingMargins()) {
            ...
            if (!$this.IsOverflowingSelf() && !HasSplitParentThatOverflows($this)) {
                $this.removeClass('overflow'); // might be a remnant from earlier overflow
            }
        }
    });
    UpdatePageOverflow(container.closest('.bloom-page'));
}

function HasSplitParentThatOverflows(jQueryBox) {
    var splitterParents = jQueryBox.parents('.split-pane-component-inner');
    if (splitterParents.length == 0) { return false; }
    return $(splitterParents[0]).IsOverflowingSelf();
}
...
function UpdatePageOverflow(page) {
    var $page = $(page);
...
function CheckOnMinHeight(box) {
    ...
    if (overflowy == 'hidden') {
        ...
        // box format.
        $box.css('min-height', '');
        var lineHeight = parseFloat($box.css("line-height"), 10);
        var minHeight = parseFloat($box.css("min-height"), 10);
    ...
    //    $box.addClass('Layout-Problem-Detected');
    //    $box.attr("LayoutProblem", "min-height is less than lineHeight");
    //} else {
    //    $box.removeClass('Layout-Problem-Detected');
    //}
}
...
function AddOverflowHandlers(container) {
    //NB: for some historical reason in March 2014 the calendar still uses textareas
    var queryElementsThatCanOverflow = ".bloom-editable, textarea";
    var editablePageElements = $(container).find(queryElementsThatCanOverflow);
