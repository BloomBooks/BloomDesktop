...
export class LineNavigator {
    ...
    private moveAnchor(event: Event): void {
        console.assert(this.direction, "LineNavigator.direction must be set");
    ...
    private getAnchorClosestToTargetX(
        ...
        try {
            ...
            // ENHANCE: Maybe the other similar places should get a finally.
            clone.remove();
        }
    ...
    private printNode(node: Node | null | undefined, prefix: string = "") {
        if (this.debug) {
            LineNavigator.printNode(node, prefix);
        }
    }
