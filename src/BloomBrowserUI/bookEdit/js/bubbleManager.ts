...
export class BubbleManager {
    ...
    private makeTOPBoxesDraggableAndClickable(
        ...
        thisTOPBoxes.each((index, element) => {
            ...
            // Save the two dragHandles and setup the 'mouseover' event to determine if we
            // should be able to drag with them or not.
    ...
    private setDraggableStateOnDragHandles(
        image: HTMLElement,
        transparentHandle: HTMLElement,
        visibleHandle: HTMLElement
    ) {
        if (!image || !transparentHandle || !visibleHandle) {
            return; // paranoia
        }
        if (this.isVisibleHandleOccluded(image, transparentHandle)) {
            transparentHandle.classList.remove("ui-draggable-handle");
            // We really don't want this dragHandle to function, so we attach some stopPropagation
            // handlers to its events.
            this.setStopPropagationHandlers(transparentHandle);
        } else {
            transparentHandle.classList.add("ui-draggable-handle");
            this.unsetStopPropagationHandlers(transparentHandle);
        }
    }
