...
export class BubbleManager {
    ...
    // These variables can be passed on addEventListener and will result in a call
    // to the appropriate method with the expected 'this'.
    // Unlike passing e.g. (ev) => this.onMouseDown(ev), since each of these is a fixed
    // function rather than a new one on each callto setDragHandlers, we won't get duplicates if we add more
    // than once.
