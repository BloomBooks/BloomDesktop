...
export class BubbleManager {
    ...
    private isVideoOverPictureElement(
        ...
        for (let i = 0; i < overPictureElement.childElementCount; i++) {
            const child = overPictureElement.children[i] as HTMLElement;
            if (child && child.classList.contains(kVideoContainerClass)) {
                return true;
            }
        }
        return false;
    ...
    private cleanClonedNode(element: Element) {
        ...
        // Picture over picture elements need the tabindex in order to be focusable.
        // If this is needed for correct duplication somewhere else, we will need to try
        // something different.
