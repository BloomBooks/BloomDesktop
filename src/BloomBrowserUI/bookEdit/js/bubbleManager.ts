...
export class BubbleManager {
    ...
    private getAllVisibleFocusableDivs(
        ...
        // Otherwise, look for video and image elements.
        ...
        if (focusableDivs.length === 0) {
            ...
            // But 'activeElement' here is a div.bloom-textOverPicture element, so if we find
            // any imageContainers inside of that, they are picture over picture elements.
            focusableDivs = Array.from(
                overPictureContainerElement.getElementsByClassName(
    ...
    private static focusableClasses = [
        "bloom-editable",
    ...
    public static onFocusSetActiveElement(event: Event) {
        ...
        if (
            Array.from(focusedElement.classList).some(el =>
                BubbleManager.focusableClasses.includes(el)
            )
        ) {
