...
export class BubbleManager {
    ...
    setupControlFrame() {
        ...
        if (!controlFrame) {
            ...
            const centerHandle = eltToPutControlsOn.ownerDocument.createElement(
                "div"
            );
            centerHandle.classList.add("bloom-ui-overlay-center-handle");
            controlFrame?.appendChild(centerHandle);
            centerHandle.addEventListener("mousedown", event => {
                if (event.buttons !== 1 || !this.activeElement) {
                    return;
                }
        ...
        // that it is wanted.
        controlFrame.classList.remove("middleButton");
    ...
    private continueMoveCrop = (event: MouseEvent) => {
        ...
        // the right of the overlay (this.activeElement.clientLeft + this.activElement.clientRight)
    ...
    private alignControlFrameWithActiveElement = () => {
        ...
        this.adjustMiddleButtonVisibility();
    };
    ...
    private adjustMiddleButtonVisibility() {
        const controlFrame = document.getElementById("overlay-control-frame");
        if (!controlFrame || !this.activeElement) return;
        const ic = this.activeElement.getElementsByClassName(
            kImageContainerClass
        )[0];
        const img = ic?.getElementsByTagName("img")[0];
        let wantMiddleButton = false;
        if (img) {
        ...
        controlFrame.classList.toggle("middleButton", wantMiddleButton);
    }
