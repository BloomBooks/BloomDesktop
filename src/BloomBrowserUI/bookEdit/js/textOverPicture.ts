...
class TextOverPictureManager {
    public initializeTextOverPictureManager(): void {
        ...
        // or 2- by switching to using addEventListener("message", eventhandler), which I've done.
    ...
    private getWebSocket(): WebSocket {
        if (typeof window.top[socketName] === "undefined") {
            ...
            window.top[socketName] = new WebSocket("ws://127.0.0.1:" + websocketPort.toString());
        }
        return window.top[socketName];
    }

    // mouseX and mouseY are the location of the mouse when right-clicking to create the context menu
    private addFloatingTOPBox(mouseX: number, mouseY: number) {
        ...
        const handleHtml = "<div class='dragHandleTOP'></div>";
        ...
        // tried to do without this... didn't work.
    ...
    // Gets the bloom-imageContainer that is "underneath" this TextOverPictureManager textbox.
    // The imageContainer will define the dragging boundaries for the textbox.
    private getImageContainer(wrapperBox: JQuery): JQuery {
        var marginBoxElem = $(document).find(".marginBox")[0];
        var styleLocationArray = wrapperBox.attr("style").match(/\d+/g); // something like "left: 288px; top: 370px;"
        // x and y need to be adjust by marginBox offset because elementFromPoint() is relative to the viewport
        var x = parseInt(styleLocationArray[0], 10) + marginBoxElem.offsetLeft;
        var y = parseInt(styleLocationArray[1], 10) + marginBoxElem.offsetTop;
        var elemAtStyleLoc = document.elementFromPoint(x, y);
        var image = $(elemAtStyleLoc).closest(".bloom-imageContainer");
        if (image.length === 0) {
            image = $(marginBoxElem).find(".bloom-imageContainer").first(); // if we messed up, return the first pic
        }
        return image;
    }
}
