...
export class EditableDivUtils {
    ...
    public static removeTrackedCkEditorFillingChar(element: HTMLElement): void {
        ...
        // caret was placed lands in the same node. Edit in place with deleteData/replaceData
        // rather than assigning the whole text, so live ranges (the caret, reader highlights)
        // in the rest of the node keep their places.
        const text = trackedNode as Text;
        const fillingChar = "\u200B";
        for (
            let i = text.data.indexOf(fillingChar);
            i >= 0;
            i = text.data.indexOf(fillingChar, i)
        ) {
            text.deleteData(i, 1);
            if (text.data[i] === " ") {
