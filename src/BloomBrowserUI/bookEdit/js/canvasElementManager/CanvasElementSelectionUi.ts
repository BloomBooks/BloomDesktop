...
export function checkActiveElementIsVisible(
    activeElement: HTMLElement | undefined,
    setActiveElement: (element: HTMLElement | undefined) => void,
): void {
    if (!activeElement) {
...
export function setupControlFrame(
    ...
    checkActiveElementIsVisible(
        ...
        callbacks.setActiveElement,
    );
    const eltToPutControlsOn = callbacks.getActiveElement();
    let controlFrame = document.getElementById("canvas-element-control-frame");
    if (!eltToPutControlsOn) {
        removeControlFrame(undefined);
        return;
    }

    if (!controlFrame) {
        controlFrame = eltToPutControlsOn.ownerDocument.createElement("div");
        ...
        sides.forEach((side) => {
            const sideControl =
                eltToPutControlsOn.ownerDocument.createElement("div");
            sideControl.classList.add("bloom-ui-canvas-element-side-handle");
            ...
            sideControl.addEventListener("mousedown", (event) => {
                ...
                callbacks.startSideControlDrag(event, side);
            });
        });
        const sideHandle =
            eltToPutControlsOn.ownerDocument.createElement("div");
        sideHandle.classList.add("bloom-ui-canvas-element-move-crop-handle");
    ...
    const imageContainer =
        eltToPutControlsOn.getElementsByClassName(kImageContainerClass)?.[0];
    ...
    const controlFrameClassStates = [
        ...
        {
            className: "is-button",
            enabled: eltToPutControlsOn.classList.contains(kBloomButtonClass),
        },
    ...
    callbacks.adjustContainerAspectRatio(eltToPutControlsOn);
    renderCanvasElementContextControls(eltToPutControlsOn, false);
}

export async function getHandleTitlesAsync(
...
export function adjustContextControlPosition(
    ...
    contextControl.style.transform = scalingContainer?.style.transform ?? "";
    ...
    if (controlsAbove) {
        ...
        if (
            bloomCanvasRect &&
            controlFrameRect.bottom > bloomCanvasRect.bottom
        ) {
...
export function restoreFocus(
    ...
    if (thingToFocusAfterSettingColor) {
        thingToFocusAfterSettingColor.focus();
...
function pxToNumber(value: string, fallback: number = 0): number {
    if (!value) {
        return fallback;
    }
