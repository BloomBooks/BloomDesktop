...
function clearImageLoadListener(
    ...
    const loadListener = state.bgImageLoadListeners.get(img);
    if (loadListener) {
        img.removeEventListener("load", loadListener);
        state.bgImageLoadListeners.delete(img);
    }
}
...
export function revertBackgroundCanvasElements(): void {
    for (const bgo of Array.from(
        ...
        if (bgImage && mainImage) {
            ...
            mainImage.setAttribute("src", bgImage.getAttribute("src") || "");
...
export function handleResizeAdjustments(
    ...
    bloomCanvases.forEach((bloomCanvas) => {
        switchBackgroundToCanvasElementIfNeeded(
            state,
            bloomCanvas,
            getActiveElement,
            alignControlFrameWithActiveElement,
        );
        adjustChildrenIfSizeChanged(bloomCanvas);
    });
}
...
function switchBackgroundToCanvasElementIfNeeded(
    state: BackgroundImageManagerState,
...
function switchBackgroundToCanvasElement(
    ...
    if (!bgCanvasElement) {
        ...
        if (oldBgImage) {
            bloomCanvas.insertBefore(bgCanvasElement, oldBgImage.nextSibling);
        } else {
...
export function setupBackgroundImageAttributes(
    ...
    const sizeAdjustment = adjustBackgroundImageSize(
        state,
        bloomCanvas,
        bgElement,
        useSizeOfNewImage,
    ...
    putBubbleBefore(bgElement, canvasElementElements, 1);
...
export function adjustBackgroundImageSize(
    state: BackgroundImageManagerState,
    bloomCanvas: HTMLElement,
    ...
    return wrapWithRequestPageContentDelay(
        () =>
            adjustBackgroundImageSizeToFit(
                state,
                bloomCanvas,
                bgCanvasElement,
                useSizeOfNewImage,
...
function adjustBackgroundImageSizeToFit(
    state: BackgroundImageManagerState,
    bloomCanvas: HTMLElement,
    ...
    if (failedImage) {
        ...
        return new Promise<void>((resolve) => {
            ...
            clearImageLoadListener(state, img);
            const loadListener = () => {
                clearTimeout(handle);
                adjustBackgroundImageSizeToFit(
                    state,
            ...
            state.bgImageLoadListeners.set(img, loadListener);
    ...
    alignControlFrameWithActiveElement();
    if (bgCanvasElement === getActiveElement()) {
