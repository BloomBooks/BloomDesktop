...
export function adjustCanvasElementChildrenIfSizeChanged(
    ...
    if (!oldSizeData) {
        if (
            bloomCanvas.getElementsByClassName(kCanvasElementClass).length > 0
        ) {
            updateBloomCanvasSizeData(bloomCanvas);
        }
        return;
    }
    ...
    const children = (Array.from(bloomCanvas.children) as HTMLElement[]).filter(
        (c) =>
            c.style.left !== "" &&
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if (child.classList.contains(kBackgroundImageClass)) {
            ...
            if (
                ...
                adjustBackgroundImageSize(bloomCanvas, child, false);
    ...
    const oldLeftPaddingFraction = widthPadding > 1 ? left / widthPadding : 0.5;
    const oldTopPaddingFraction = heightPadding > 1 ? top / heightPadding : 0.5;
    ...
    if (backgroundCanvasElement) {
        adjustBackgroundImageSize(bloomCanvas, backgroundCanvasElement, false);
    }
