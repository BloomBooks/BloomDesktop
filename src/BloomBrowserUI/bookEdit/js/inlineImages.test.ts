...
// Stands in for CKEditor holding (or not holding) an undo point for the block, which is what
// says whether the person's own text editing is still ahead of ours in the queue. The real
// thing is a global the page's CKEditor puts up; absent, as in these tests unless a test says
// otherwise, it reads as "nothing to undo".
function setCkeditorUndoable(undoable: boolean | undefined): void {
    const global = globalThis as unknown as { CKEDITOR?: unknown };
    if (undoable === undefined) delete global.CKEDITOR;
    else
        global.CKEDITOR = {
            currentInstance: { undoManager: { undoable: () => undoable } },
        };
}
