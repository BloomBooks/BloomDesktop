    ...
    describe("backspacePreventionTests", () => {
        ...
        function runBackspacePreventionTest(
            ...
            if (editable) {
                ...
                //
                // Our code still checks KeyboardEventInit's 'which', but nowadays it's deprecated
                // in favor of key, so KeyboardEventInit doesn't officially recognize it in the type
                // definition, but we need it anyway, so just force the types to work.
                // Review: (gjm) is the above comment still valid? I see no 'which' here.
                const keyEventInit = {
                    key: "Backspace",
