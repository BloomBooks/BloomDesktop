...
export default class BloomField {
    ...
    */
            ...
            if (e.which === 8 /* backspace*/) {
                ...
                if (closestP.length > 0 && closestP[0] !== sel.anchorNode) {
                    if (sel.anchorNode instanceof Element) {
                        //If it's not the 1st child, then nothing to worry about that.
                        if (
                            !this.isElementALeftMostDescendant(
                                closestP[0],
                        ...
                        //I guess that means anchorNode is a Text Node. (Or... theoretically, a comment)
                        //That means there's no complicated child elements to it... we know we're at the start
                        //We can just go ahead and fall through to the code that prevent the backspace.
                    }
                }
    ...
    // That is, it's on the "left"-most path if you drew it as at tree structure with the earlier children on the left.
    private static isElementALeftMostDescendant(
        rootElement: Element,
        targetElement: Element
    ) {
        // Note: we use firstElementChild because firstChild also returns nodes. (e.g., the text node within a leaf element)
        let curr: Element | null = rootElement;
        while (curr) {
            if (targetElement === curr) {
                return true;
            }
            if (curr.firstChild !== curr.firstElementChild) {
                // That means the left-most node is not an element node, so targetElement (which is an element node) can't be equal to it.
                return false;
            }
