...
export default class BloomField {
    ...
    public static copyAudioFilesWithNewIdsDuringPasting(
        ...
        while ((regexMatch = audioRegex.exec(inputText)) !== null) {
            ...
            const group1Text: string = regexMatch[1];
            const group2Text: string = regexMatch[2];
            ...
            newText =
                newText +
                `<span id="${newId}" class="audio-sentence">${group2Text}</span>`;
            BloomApi.post(
                `audio/copyAudioFile?oldId=${group1Text}&newId=${newId}`
            );
