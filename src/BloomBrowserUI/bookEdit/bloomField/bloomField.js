...
var BloomField = (function () {
    ...
    BloomField.ManageField = function (target) {
        $(target).focus(function () {
            BloomField.PrepareField(this);
        });
        $(target).blur(function () {
            BloomField.CleanupAfterBlur(this);
        });
        BloomField.PreventRemovalOfSomeElements(target);
    };
    BloomField.CleanupAfterBlur = function (field) {
        ...
        $(field).children("span.bloom-ui").remove();
        //This might mess some things up, so we're only applying it selectively
        if ($(field).closest('.bloom-requiresParagraphs').length === 0 && ($(field).css('border-top-style') !== 'dashed'))
            return;
        var x = $(field).html();
        //the first time we see a field editing in Firefox, it won't have a p opener
        if (!x.trim().startsWith('<p') && !x.trim().startsWith('<div')) {
            x = "<p class='me-red'>" + x;
        }
        ...
        $(field).find('p').reverse().each(function () {
            if ($(this).text() === "") {
                $(this).remove();
            }
            else {
    ...
    BloomField.PrepareField = function (field) {
        ...
        var requireParagraphs = $(field).closest('.bloom-requiresParagraphs').length > 0 || ($(field).css('border-top-style') === 'dashed'); //this signal used to let the css add this conversion after some SIL-LEAD SHRP books were already typed
        ...
        //TODO if you do Ctrl+A and delete, you're now outside of our <p></p> zone. clicking out  
        //will trigger the blur handerl above, which will restore it.
    };
    ...
    BloomField.PreventRemovalOfSomeElements = function (field) {
        ...
        if (numberThatShouldBeThere > 0) {
            //Note, the input event is *not* fired on the element itself in the (ctrl+a, del) scenario,
            //hence the need to attach the event to the parent bloom-editable
    ...
    BloomField.FixUpOnFirstInput = function () {
