...
class BloomField {
    ...
    static CleanupAfterBlur(field:HTMLElement) {
        ...
        if ($(field).closest('.bloom-requiresParagraphs').length === 0
            && ($(field).css('border-top-style') !== 'dashed')) //this signal used to let the css add this conversion after some SIL-LEAD SHRP books were already typed
            return;
    ...
    static PrepareField(field : HTMLElement) {
        ...
        var requireParagraphs = $(field).closest('.bloom-requiresParagraphs').length > 0
            || ($(field).css('border-top-style') === 'dashed'); //this signal used to let the css add this conversion after some SIL-LEAD SHRP books were already typed
