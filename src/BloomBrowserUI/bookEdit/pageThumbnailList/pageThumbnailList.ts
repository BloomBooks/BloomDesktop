...
$(function () {
    ...
    window[socketName] = new WebSocket("ws://127.0.0.1:" + websocketPort.toString());

    theOneLocalizationManager.asyncGetText("EditTab.SavingNotification", "Saving...").done(savingNotification =>
        (<WebSocket>window[socketName]).addEventListener("message", event => {
...
export function stopListeningForSave() {
    (<WebSocket>window[socketName]).close();
}
