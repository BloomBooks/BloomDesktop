...
export function useAppBuilderPublisherScreen(
    ...
    React.useEffect(() => {
        return () => {
            ...
            // Both leaving the Publish tab and switching to another publish sub-tab unmount this
            // screen (react-tabs renders only the selected panel's children), so this cleanup is
            // the single place that cancels a running action. Cancel any in-progress
            // prepare/build/install so a long RAB build doesn't keep going in the background after
