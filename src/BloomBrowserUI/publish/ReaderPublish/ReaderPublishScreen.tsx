...
export const ReaderPublishScreen = () => {
    ...
    const [keyForUpadtingPreview, setKeyForUpdatingPreview] = useState(0);
    return (
        <ReaderPublishScreenInternal
            key={keyForUpadtingPreview} // NOTE: Updating the key mounts a new component instance (beware of trying to keep state after reset... it's not the same instance!)
            onUpdatePreview={() => {
                // This causes props.showPreview to be true, which cases the progress dialog to show,
                // which causes the preview to be generated.
                setKeyForUpdatingPreview(keyForUpadtingPreview + 1);
            }}
            showPreview={keyForUpadtingPreview > 0}
        />
    );
};
