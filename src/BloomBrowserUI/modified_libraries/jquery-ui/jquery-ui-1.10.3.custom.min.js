...
// This file is currently imported in order to make various side effects happen,
// particularly adding various jQuery extensions that we use in Bloom.
// Since we're no longer making jQuery a universally defined global in the Vite build,
// we must import it here so the extensions can be added to the right jQuery object.
// Note that this means we can't load this file directly using a script tag.
// If we declare it to be text/javascript, the browser will choke on this import statement.
// If we declare it to be type="module", it doesn't have enough context to find "jquery".
