...
function SetupEditableElements(container) {
    ...
    $(container).find(".bloom-editable").blur(function () {
        ...
        if ($(this).closest('.bloom-requiresParagraphs').length == 0 && ($(this).css('border-top-style') != 'dashed'))
            return;
        var x = $(this).html();
    ...
    $(container).find('.bloom-editable').focus(function () {
        ...
        var requireParagraphs = $(this).closest('.bloom-requiresParagraphs').length > 0 || ($(this).css('border-top-style') == 'dashed'); //this signal used to let the css add this conversion after some SIL-LEAD SHRP books were already typed
