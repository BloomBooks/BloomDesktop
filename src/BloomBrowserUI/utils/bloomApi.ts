...
export class BloomApi {
    ...
    public static get(urlSuffix: string, acceptResult?: (r: AxiosResponse) => void) {
        if (acceptResult) {
            BloomApi.wrapAxios(axios.get("/bloom/" + urlSuffix).then(acceptResult));
        } else {
            BloomApi.wrapAxios(axios.get("/bloom/" + urlSuffix));
        }
    }

    // This method is used to get a result from Bloom, passing paramaters to the nested axios call.
    public static getP(urlSuffix: string, config: AxiosRequestConfig, acceptResult: (r: AxiosResponse) => void) {
        BloomApi.wrapAxios(axios.get("/bloom/" + urlSuffix, config).then(acceptResult));
    }

    // This method is used to post something from Bloom.
    public static post(urlSuffix: string, acceptResult?: (r: AxiosResponse) => void) {
        if (acceptResult) {
            BloomApi.wrapAxios(axios.post("/bloom/" + urlSuffix).then(acceptResult));
        } else {
    ...
    public static postD(urlSuffix: string, data: any, acceptResult?: (r: AxiosResponse) => void) {
        if (acceptResult) {
            BloomApi.wrapAxios(axios.post("/bloom/" + urlSuffix, data).then(acceptResult));
        } else {
    ...
    public static postP(urlSuffix: string, data: any, config: AxiosRequestConfig, acceptResult?: (r: AxiosResponse) => void) {
        if (acceptResult) {
            BloomApi.wrapAxios(axios.post("/bloom/" + urlSuffix, data, config).then(acceptResult));
        } else {
