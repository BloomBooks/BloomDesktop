...
// sign-in modes (task 06's `sharing/loginState` mode field): local-mode's email/password form,
// and "cloud" mode's browser-sign-in button (task 12; Option A decided 8 Jul 2026).

const localMode: ISharingLoginState = { mode: "local", signedIn: false };
...
describe("SignInDialogBody", () => {
    it("local mode: shows the email/password form, not the not-yet-available message", () => {
        const container = render(<SignInDialogBody {...baseProps()} />);

        expect(
            container.querySelector('[data-testid="signin-local-form"]'),
        ).not.toBeNull();
        expect(
    ...
    it("local mode: clicking Sign In calls onSignIn", () => {
    ...
    it("local mode: shows the sign-in error when one is provided", () => {
        const container = render(
            <SignInDialogBody
    ...
    it("cloud mode: shows the browser sign-in button, not the local form or the not-yet-available message", () => {
        const container = render(
