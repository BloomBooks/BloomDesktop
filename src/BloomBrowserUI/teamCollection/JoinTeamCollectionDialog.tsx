            ...
            <DialogBottomButtons>
                {wantReportButton && (
                    <BloomButton
                        ...
                        enabled={true}
                        variant="text"
                        // margin-left counteracts Material-UI padding. The padding is used for hover effects,
                        // so we don't want to just override it. Important is needed to beat some Material-UI rules.
                        css={css`
                        ...
                        onClick={() => {
                            BloomApi.postJson("problemReport/showDialog", {
                                shortMessage:
                                    "Problem joining Team Collection due to conflicting local collection",
                                message: `trying to join ${props.joiningRepo} (${props.joiningGuid}), but local collection ${props.existingCollectionFolder} (${props.localGuid}) is linked to ${props.conflictingCollection}`
                            });
                            // It's tempting to add closeDialog() here; but if we do it after opening the problem dialog, it closes that instead.
