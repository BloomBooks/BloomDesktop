...
namespace BloomTests
{
    ...
    class BookThumbnailerTests : BookTestsBase
    {
        ...
        private Bloom.Book.Book SetupBook(
        ...
        {
            var imgTag =
                $"<img style='width: {coverImageWidth};' src='{coverImageFilename}' height='{coverImageHeight}' alt='missing'></img>";
            SetDom(
                @"<div id='bloomDataDiv'>
        ...
        )
        {
            ...
            // Verification
            string expectedThumbnailFilename = book.FolderPath.CombineForPath(
                $"thumbnail-{requestedSize}.png"
            );
            Assert.That(
                RobustFile.Exists(expectedThumbnailFilename),
                Is.True,
                "Thumbnail does not exist at expected path"
            );
        ...
        [TestCase(600, 600)]
        public void GenerateSocialMediaSharingThumbnail_LargerSquareImage_AllOpaque(
            int originalWidth,
            int originalHeight
        )
        {
            ...
            // Run the test helper
            RunSocialMediaSharingThumbnailTest(
                thumbnailSize,
                originalWidth,
                originalHeight,
                opaquePoints,
        ...
        private void RunSocialMediaSharingThumbnailTest(
            int thumbnailSize,
            int inputWidth,
            int inputHeight,
            IEnumerable<(int, int)> expectedOpaquePoints,
            IEnumerable<(int, int)> expectedTransparentPoints,
        ...
        )
        {
            ...
            // Verification
            Assert.That(
                RobustFile.Exists(thumbnailFilename),
                Is.True,
                "Thumbnail does not exist at expected path"
            );
            using (var image = System.Drawing.Image.FromFile(thumbnailFilename))
            {
                ...
                foreach (var (x, y) in expectedOpaquePoints)
                {
                    ...
                    Assert.That(
                        alphaOpacity,
                        Is.EqualTo(255),
                        $"Alpha at point ({x}, {y}) should be opaque."
                    );
                }
                ...
                foreach (var (x, y) in expectedTransparentPoints)
                {
                    ...
                    Assert.That(
                        alphaOpacity,
                        Is.EqualTo(0),
                        $"Alpha at point ({x}, {y}) should be transparent."
                    );
                }
            }
        }
        ...
        public void IsCoverImageSrcValid_PlaceholderImageButScenarioAllowsIt_Valid(
        ...
        public void IsCoverImageSrcValid_PlaceholderImageButScenarioDisallowsIt_NotValid(
            string newThumbnailFileName
        )
        {
            var options = new HtmlThumbNailer.ThumbnailOptions { FileName = newThumbnailFileName };
