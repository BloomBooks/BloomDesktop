...
namespace BloomTests
{
    ...
    class ExtensionsTests
    {
        ...
        public static void ToLocalhost_GivenALocalhostUrl_ReturnsUnchanged()
        {
            // Setup
            string input =
                $"http://localhost:{BloomServer.portForHttp}/bloom/C%3A/Directory/filename.txt";
        ...
        public static void ToLocalhost_GivenSimpleFilename_ConvertsToUrl()
        {
            ...
            Assert.That(
                result,
                Is.EqualTo(
                    $@"http://localhost:{BloomServer.portForHttp}/bloom/C%3A/Directory/Book%20Title/Book%20Title.htm"
                )
            );
        }
        ...
        [TestCase("A,", "A%2C")]
        [TestCase("A.", "A.")]
        public static void ToLocalhost_BloomPubPreviewBookTitleWithPunc_GeneratesWellFormedUrl(
            string bookTitle,
            string expectedEscapedTitle
        )
        {
            // Setup
            ...
            string expectedResult =
                $@"http://localhost:{BloomServer.portForHttp}/bloom/C%3A/PathToTemp/PlaceForStagingBook/{expectedEscapedTitle}/meta.json";
