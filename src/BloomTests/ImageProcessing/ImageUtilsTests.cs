...
namespace BloomTests.ImageProcessing
{
	...
	public class ImageUtilsTests
	{
		...
		{
			var inputPath = FileLocationUtilities.GetFileDistributedWithApplication(_pathToTestImages, "man.jpg");
			using (var image = PalasoImage.FromFileRobustly(inputPath))
			{
				using (var folder = new TemporaryFolder("TransparentPngTest"))
				{
					var basePath = Path.Combine(folder.Path, basename + ".jpg");
					RobustFile.Delete(basePath); // just in case
					RobustImageIO.SaveImage(image.Image, basePath, ImageFormat.Jpeg);
					var filename = ImageUtils.GetUnusedFilename(Path.GetDirectoryName(basePath), basename, ".jpg");
