    ...
    internal class WebSocketServerSpy : IBloomWebSocketServer
    {
        ...
        public void SendString(string clientContext, string eventId, string message)
        {
            if (!_isInitialized)
                throw new ApplicationException(
                    "WebSocketServerSpy: Send() attempted when not initialized!"
                );

            _events.Add(
                new KeyValuePair<string, Tuple<string, string, ProgressKind>>(
                    eventId,
        ...
        public List<KeyValuePair<string, Tuple<string, string, ProgressKind>>> Events
        {
            get
            {
                ...
                {
                    throw new ApplicationException(
                        "WebSocketServerSpy: EventDictionary access attempted when not initialized!"
                    );
                }
                return _events;
            }
