...
namespace BloomTests.web
{
	...
	public class DynamicJsonTests
	{
		...
		{
			var obj = DynamicJson.Parse("{'d': {'a1':2, 'a2': 0, 'a3': 1}}".Replace("'", "\"")) as DynamicJson;
			Assert.That(obj.TryGet<Dictionary<string, TestVal>>("d",out Dictionary<string, TestVal> val), Is.True);
		...
		{
			var obj = DynamicJson.Parse("{'d': {'a1':'Third', 'a2': 'First', 'a3': 'Second'}}".Replace("'", "\"")) as DynamicJson;
			Assert.That(obj.TryGet<Dictionary<string, TestVal>>("d", out Dictionary<string, TestVal> val), Is.True);
			Assert.That(val["a1"], Is.EqualTo(TestVal.Third));
			Assert.That(val["a2"], Is.EqualTo(TestVal.First));
			Assert.That(val["a3"], Is.EqualTo(TestVal.Second));
