...
//				BookStorageFactory, null, null, new CreateFromSourceBookCommand(), new EditBookCommand());
		...
		{
			return new Bloom.Book.Book(new BookInfo(storage.FolderPath),  storage, true, null, new CollectionSettings(new NewCollectionSettings() { PathToSettingsFile = CollectionSettings.GetPathForNewSettings(_folder.Path, "test"), Language1Iso639Code = "xyz" }), null,
													 new PageSelection(),
