...
namespace BloomTests.web.controllers
{
    public class WiFiPublisher
    {
        ...
        public void MakeVersionCode_DistinguishesTextChanges()
        {
            ...
            Assert.That(
                Bloom.Book.Book.MakeVersionCode(first),
                Is.Not.EqualTo(Bloom.Book.Book.MakeVersionCode(second))
            );
        }
        ...
        public void MakeVersionCode_DistinguishesSignificantWhitespaceChanges(
            string firstArg,
            string secondArg
        )
        {
...
<html>
	...
	<body> <div>abc</div>
            ...
            Assert.That(
                Bloom.Book.Book.MakeVersionCode(first),
                Is.EqualTo(Bloom.Book.Book.MakeVersionCode(second))
            );
        }
    ...
    <style type='text/css'>
        ...
        {
            var template =
                @"<!DOCTYPE html><html><head></head><body><div class='bloom-page' id='{0}'>abc</div></body></html>";
        ...
        {
            var template =
                @"<!DOCTYPE html><html><head></head><body><div class='bloom-page'>id='{0}'</div></body></html>";
            var first = string.Format(template, "934245d2-94dd-4f40-8b53-279867d8e07b");
            var second = string.Format(template, "d9a71953-6cf4-475a-8236-36d509ff8e1c");
            Assert.That(
                Bloom.Book.Book.MakeVersionCode(first),
        ...
        public void MakeVersionCode_ConsidersFilesInFolder_ButNotHtm()
        {
            ...
            using (
                var tempFolder = new TemporaryFolder(
                    "MakeVersionCode_ConsidersFilesInFolder_ButNotHtm"
                )
            )
            {
