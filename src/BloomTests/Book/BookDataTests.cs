...
namespace BloomTests.Book
{
	...
	""presets"": [{
		...
		{
			// This is not very realistic. We expect that the junk attributes will NOT ever get into the bloomDataDiv. But the source for copying
			// in SynchronizeDataItemsThroughoutDOM is the FIRST element with a given data-book.
			var dom = new HtmlDom(@"<html ><head></head><body>
				<div id='bloomDataDiv'>
		...
		public void SynchronizeDataItemsThroughoutDOM_DoesNotRemoveRemovableAttrsAndClasses_IfInSource()
		{
			...
			data.SynchronizeDataItemsThroughoutDOM();
			var foo = (XmlElement)dom.SelectSingleNodeHonoringDefaultNS("//*[@findMe='foo']");
			var foo2 = (XmlElement)dom.SelectSingleNodeHonoringDefaultNS("//*[@findMe='foo']");
