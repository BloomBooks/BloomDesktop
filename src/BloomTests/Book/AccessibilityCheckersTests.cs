...
namespace BloomTests.Book
{
    public class AccessibilityCheckersTests : BookTestsBase
    {
        ...
        public void CheckDescriptionsForAllImages_DescriptionInCorrectLang()
        {
            var testBook = GetBookWithImage(
                $@"<div class='bloom-translationGroup bloom-imageDescription'>
					<div class='bloom-editable' lang='{_bookData.Language1.Tag}'>
						<p>A flower.</p>
					</div>
				</div>"
            );
        ...
        )
        {
            ...
            Assert.AreEqual(
                $"Missing image description on page {expected}",
        ...
        [TestCase(
            @"<p><span id='iExist' class='audio-sentence'>A flower.</span>
					<span id='iExist' class='audio-sentence'>A dog.</span></p>"
        )]
        [TestCase(
            @"<p><div id='iExist' class='audio-sentence'>A flower.</div>
        ...
        {
            var testBook = MakeBookWithOneAudioFile(
        ...
        [TestCase(
            @"<p><span id='iExist' class='audio-sentence'>A flower.</span></p>
					<p>A dog.</p>"
        )]
        ...
        [TestCase(
            @"<p><div id='iExist' class='audio-sentence'>A flower.</div></p>
        ...
        public void CheckAudioForAllText_TextInRandomLangButVisibleAndNotRecorded_GivesError()
        {
            ...
            Assert.AreEqual(
                1,
                results.Count(),
                "The text has to be recorded because it is visible"
            );
        }
        ...
        public void CheckAudioForAllText_TextInNationalLanguageNotVisible_NotRecorded()
        {
            ...
            Assert.AreEqual(
                0,
                results.Count(),
                "Since the text is not visible, should not give error if not recorded"
            );
        }
        ...
        public void CheckAudioForAllImageDescriptions_AudioMissing(
            int numberOfErrorsExpected,
            string content
        )
        {
            var testBook = MakeBookWithOneAudioFile(
            ...
            Assert.AreEqual(
                ...
                results.Count(),
                "Number of errors does not match expected"
            );
        }
        ...
        public void CheckAudioForAllImageDescriptions_AudioFolderMissing_JustReturnsNormalMissingAudioError()
        {
            ...
            SIL.IO.RobustIO.DeleteDirectoryAndContents(
                AudioProcessor.GetAudioFolderPath(testBook.FolderPath)
            );
            var results = AccessibilityCheckers.CheckAudioForAllImageDescriptions(testBook);
            Assert.AreEqual(1, results.Count(), "Number of errors does not match expected");
        }

        private Bloom.Book.Book GetBookWithImage(
            string translationGroupText,
            string pageNumber = "1",
        ...
        private string MakeHtmlForPageWithImageAriaHidden(
            string translationGroupText,
            string pageNumber = "1",
            string pageLabel = "Some page label"
        )
        ...
        private Bloom.Book.Book MakeBookWithOneAudioFile(
            string translationGroupText,
            string pageNumber = "1",
            string pageLabel = "Some page label"
        )
