...
.MyTest-style[lang='en']
{
	font-size: smaller;
		...
		[Test]
		public void StripUnusedTagsPreservingText_StripsEmbeddedSpan()
		{
			...
			// SUT
			HtmlDom.StripUnusedTagsPreservingText(dom.RawDom, testableElement, tagsToPreserve);
			...
			AssertThatXmlIn.Dom(dom.RawDom).HasAtLeastOneMatchForXpath("//p[.='‌*Some right answer ']");
			AssertThatXmlIn.Dom(dom.RawDom).HasAtLeastOneMatchForXpath("//p[.='‌*Answer 2']");
			AssertThatXmlIn.Dom(dom.RawDom).HasAtLeastOneMatchForXpath("//p[.='My test text.']");
		}
	}
}
