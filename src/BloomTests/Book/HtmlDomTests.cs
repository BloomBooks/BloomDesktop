...
namespace BloomTests.Book
{
	...
	public sealed class HtmlDomTests
	{
		...
		public void MergeUserModifiedStyles_EmptyExisting_Works()
		{
			...
			pageDom.LoadXml(@"<html><head><style type='text/css' title='userModifiedStyles'>
.MyTest-style { font-size: ginormous; }</style></head><body></body></html>");

			var pageStyleNode = pageDom.SelectSingleNode("//style");
			var bookStyleNode = HtmlDom.GetUserModifiableStylesUsedOnPage(bookDom);
			// SUT
		...
		{
			var bookDom = new HtmlDom(
				@"<html><head><style type='text/css' title='userModifiedStyles'>
.MyTest-style { font-size: ginormous; }</style></head><body><div class='bogus MyTest-style'></div></body></html>");
			var pageDom = new XmlDocument();
			pageDom.LoadXml(@"<html><head></head><body></body></html>");
		...
		public void MergeUserModifiedStyles_NewStyleHasLangAttr_BothKept()
		{
			...
			pageDom.LoadXml(@"<html>
				<head>
					<style type='text/css' title='userModifiedStyles'>
						.MyTest-style[lang='en'] { font-size: smaller; }</style>
				</head>
				<body>
					<div class='MyTest-style bogus'></div>
				</body>
			...
			var xpath = "//style[@title=\"userModifiedStyles\" and contains(text(),'font-size: ginormous;')]";
			AssertThatXmlIn.Dom(bookDom.RawDom).HasSpecifiedNumberOfMatchesForXpath(xpath, 1);
			xpath = "//style[@title=\"userModifiedStyles\" and contains(text(),\".MyTest-style[lang='en'] { font-size: smaller;\")]";
			AssertThatXmlIn.Dom(bookDom.RawDom).HasSpecifiedNumberOfMatchesForXpath(xpath, 1);
		}
	}
}
