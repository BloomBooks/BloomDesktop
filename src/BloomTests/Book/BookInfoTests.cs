...
namespace BloomTests.Book
{
    ...
    public class BookInfoTests
    {
        ...
        public void Constructor_LoadsMetaDataFromJson()
        {
            ...
            File.WriteAllText(
                jsonPath,
                @"{""folio"":""true"",""experimental"":""true"",""suitableForMakingShells"":""true""}"
            );
            var playerSettingsPath = Path.Combine(_folder.Path, BookInfo.PublishSettingsFileName);
            File.WriteAllText(
                playerSettingsPath,
                @"{""audioVideo"": {""motion"":true, ""pageTurnDelay"":3500,
						""format"":""feature"",
            ...
            Assert.That(
                ps.AudioVideo.PlayerSettings,
                Is.EqualTo("{\"lang\":\"qaa\",\"imageDescriptions\":false}")
            );
            Assert.That(ps.BloomPub.PublishAsMotionBookIfApplicable, Is.True);
            Assert.That(
                ps.BloomLibrary.TextLangs["de"],
                Is.EqualTo(InclusionSetting.ExcludeByDefault)
            );
        ...
        public void LoadPublishSettings_AllSettings_Works()
        {
            ...
            Assert.That(
                ps.AudioVideo.PlayerSettings,
                Is.EqualTo("{\"lang\":\"fr\",\"imageDescriptions\":true}")
            );
            ...
            Assert.That(
                ps.BloomPub.AudioLangs["baa"],
                Is.EqualTo(InclusionSetting.IncludeByDefault)
            );
            Assert.That(
                ps.BloomPub.AudioLangs["es"],
                Is.EqualTo(InclusionSetting.ExcludeByDefault)
            );
            Assert.That(ps.BloomPub.SignLangs["asl"], Is.EqualTo(InclusionSetting.Include));

            Assert.That(
                ps.Epub.HowToPublishImageDescriptions,
            ...
            Assert.That(
                ps.BloomLibrary.TextLangs["abc"],
                Is.EqualTo(InclusionSetting.ExcludeByDefault)
            );
            Assert.That(
                ps.BloomLibrary.AudioLangs["def"],
                Is.EqualTo(InclusionSetting.IncludeByDefault)
            );
            Assert.That(
                ps.BloomLibrary.AudioLangs["xyz"],
                Is.EqualTo(InclusionSetting.ExcludeByDefault)
            );
        ...
        public void PublishSettings_RoundTrip_Works()
        {
            ...
            original.PublishSettings.AudioVideo.PlayerSettings =
                "{\"lang\":\"fr\",\"imageDescriptions\":true}";
            original.Save();
            var restored = new BookInfo(_folder.Path, true);
            Assert.That(
                restored.PublishSettings.AudioVideo.PlayerSettings,
                Is.EqualTo(original.PublishSettings.AudioVideo.PlayerSettings)
            );
        }
        ...
        public void PublishSettings_MigrateFromOldSettings()
        {
            ...
            File.WriteAllText(
                jsonPath,
                @"{""suitableForMakingShells"":""true"", ""epub_HowToPublishImageDescriptions"":0}"
            );
            var original = new BookInfo(_folder.Path, true);
            ...
            Assert.That(
                original.PublishSettings.Epub.HowToPublishImageDescriptions,
            ...
            Assert.That(
                restored.PublishSettings.Epub.HowToPublishImageDescriptions,
                Is.EqualTo(BookInfo.HowToPublishImageDescriptions.OnPage)
            );
            Assert.That(restored.PublishSettings.Epub.RemoveFontSizes, Is.True);

            Assert.That(
                restored.PublishSettings.BloomLibrary.TextLangs["en"],
                Is.EqualTo(InclusionSetting.Include)
            );
            Assert.That(
                restored.PublishSettings.BloomLibrary.TextLangs["fr"],
                Is.EqualTo(InclusionSetting.Exclude)
            );
            Assert.That(
                restored.PublishSettings.BloomLibrary.TextLangs["tpi"],
                Is.EqualTo(InclusionSetting.IncludeByDefault)
            );
            Assert.That(
                restored.PublishSettings.BloomPub.TextLangs["tpi"],
                Is.EqualTo(InclusionSetting.ExcludeByDefault)
            );
            Assert.That(
                restored.PublishSettings.BloomPub.TextLangs["en"],
                Is.EqualTo(InclusionSetting.Exclude)
            );

            Assert.That(
                restored.PublishSettings.BloomLibrary.AudioLangs["en"],
                Is.EqualTo(InclusionSetting.ExcludeByDefault)
            );
            Assert.That(
                restored.PublishSettings.BloomPub.AudioLangs["es"],
                Is.EqualTo(InclusionSetting.IncludeByDefault)
            );
            Assert.That(
                restored.PublishSettings.BloomLibrary.SignLangs["qaa"],
                Is.EqualTo(InclusionSetting.Include)
            );
            Assert.That(
                restored.PublishSettings.BloomPub.SignLangs["qed"],
                Is.EqualTo(InclusionSetting.Exclude)
            );
        }
        ...
        public void UpdateOneSingletonTag_GivenATag_WhenNewTagAdded_OldValueReplaced(
            object[] existingContents
        )
        {
            BookInfo bi = new BookInfo(_folder.Path, true);
        ...
        public void UpdateOneSingletonTag_GivenATag_WhenValueUpdatedToNullOrWhiteSpace_TagIsRemoved(
            string input
        )
        {
            BookInfo bi = new BookInfo(_folder.Path, true);
        ...
        public void InstallFreshGuid_works()
        {
            ...
            const string metaData =
                @"{""bookInstanceId"":"""
                + originalGuid
                + @""",""experimental"":""false"",""suitableForMakingShells"":""true""}";
        ...
        public void RoundTrips_AllowUploading()
        {
            ...
            Assert.False(
                bi.AllowUploading,
                "CHECK YOUR FixBloomMetaInfo ENV variable! Initial Read Failed to get false. Contents: "
            ...
            Assert.False(
                bi2.AllowUploading,
                "Read after Save() Failed  to get false. Contents: " + File.ReadAllText(jsonPath)
            );
            ...
            Assert.That(
                bi3.AllowUploading,
                "Initial Read Failed to get true. Contents: " + File.ReadAllText(jsonPath)
            );
            bi3.Save();
            var bi4 = new BookInfo(_folder.Path, true);
            Assert.That(
                File.ReadAllText(jsonPath).Contains("allowUploadingToBloomLibrary"),
                "The file doesn't contain 'allowUploadingToBloomLibrary'"
            );
            Assert.That(
                bi4.AllowUploading,
                "Read after Save() Failed  to get true. Contents: " + File.ReadAllText(jsonPath)
            );
        }
        ...
        public void WebDataJson_IncludesCorrectFields()
        {
            var meta = new BookMetaData()
            {
                ...
                Tags = new[] { "Animals" },
                CurrentTool = "mytool",
                BookletMakingIsAppropriate = false,
                PageCount = 7,
                LanguageDescriptors = new[]
                {
            ...
            Assert.That(
                meta2.AllowUploadingToBloomLibrary,
                Is.True,
                "AllowUploadingtoBloomLibrary was unexpectedly serialized"
            );
        ...
        public void GetRepairedMetaDataWithIdOnly_FindsId()
        {
            using (
                var tempFile = new TempFile(
                    "junk rubbish nonsense \"bookInstanceId\":\"abcdefg-1234\" lots more junk"
                )
            )
            {
                Assert.That(
                    BookMetaData.GetRepairedMetaDataWithIdOnly(tempFile.Path).Id,
        ...
        public void TopicsList_GetsTopicsAndOnlyTopicsFromTagsList(
            string jsonTagsList,
            string expectedTopicsList
        )
        {
            var jsonPath = Path.Combine(_folder.Path, BookInfo.MetaDataFileName);
        ...
        [TestCase(
            @"""Fiction"",""Math"",""media:audio""",
            "Math",
            "Math",
            new[] { "media:audio", "topic:Math" }
        )]
        [TestCase(@"""media:audio""", "topic:Math", "Math", new[] { "media:audio", "topic:Math" })]
        [TestCase(
            @"""media:audio"",""region:Asia""",
            "topic:Math,topic:Fiction",
            "Math, Fiction",
            new[] { "media:audio", "region:Asia", "topic:Math", "topic:Fiction" }
        )]
        [TestCase(
            @"""topic:Science""",
            "topic:Math,topic:Fiction",
            "Math, Fiction",
            new[] { "topic:Math", "topic:Fiction" }
        )]
        public void TopicsList_SetsTopicsWhileLeavingOtherTagsIntact(
            string jsonTagsList,
            string topicsListToSet,
            string expectedTopicsList,
            string[] expectedTags
        )
        {
            ...
            BookMetaData metadata = (BookMetaData)
                SIL.Reflection.ReflectionHelper.GetField(bi, "_metadata");
        ...
        [TestCase(
            null,
            new string[0],
            TestName = "FeaturesGetter_TalkingBookLangCodesNull_NoException"
        )]
        [TestCase(
            new string[0],
            new string[0],
            TestName = "FeaturesGetter_TalkingBookLangCodesEmpty_Empty"
        )]
        [TestCase(
            new string[] { "en", "es" },
            new string[] { "talkingBook", "talkingBook:en", "talkingBook:es" },
            TestName = "FeaturesGetter_TalkingBookLangCodesMultiple_OverallAndLangSpecificFeatures"
        )]
        public void FeaturesGetter_TalkingBook(
            IEnumerable<string> langCodes,
            string[] featuresExpected
        )
        {
            ...
            Assert.AreEqual(
                featuresExpected.Any(),
                featureTalkingBookResult,
                "Feature_TalkingBook"
            );
        }
        ...
        [TestCase(
            null,
            new string[0],
            TestName = "FeaturesGetter_SignLanguageLangCodesNull_NoException"
        )]
        [TestCase(
            new string[0],
            new string[0],
            TestName = "FeaturesGetter_SignLanguageLangCodesEmptyArray_Empty"
        )]
        [TestCase(
            new string[] { "" },
            new string[] { "signLanguage" },
            TestName = "FeaturesGetter_SignLanguageLangCodesEmptyString_OverallOnly"
        )]
        [TestCase(
            new string[] { "ase" },
            new string[] { "signLanguage", "signLanguage:ase" },
            TestName = "FeaturesGetter_SignLanguageLangCodeSet_OverallAndLangSpecificFeatures"
        )]
        public void FeaturesGetter_SignLanguage(
            IEnumerable<string> langCodes,
            string[] featuresExpected
        )
        {
        ...
        public void FeaturesGetter_SimpleDomChoice(bool containsSimpleDomChoice)
        {
            ...
            Assert.AreEqual(
                containsSimpleDomChoice,
                metadata.Features.Contains("simple-dom-choice")
            );
        }
        ...
        [TestCase(true, false)]
        [TestCase(true, true)]
        public void FeaturesGetter_IfQuizOrWidgetSet_ThenActivityIsTrue(
            bool containsQuiz,
            bool containsWidget
        )
        {
        ...
        public void FeaturesSetter_OverallFeaturesOnly_ConvertBackGetsSameResult()
        {
            ...
            CollectionAssert.AreEqual(
                expectedResult,
                metadata.Feature_Blind_LangCodes,
                "Blind Language Codes"
            );
            CollectionAssert.AreEqual(
                expectedResult,
                metadata.Feature_TalkingBook_LangCodes,
                "TB Language Codes"
            );
            CollectionAssert.AreEqual(
                expectedResult,
                metadata.Feature_SignLanguage_LangCodes,
                "SL Language Codes"
            );
        }
        ...
        public void RuntimeInformationInjector_PullInCollectionLanguagesDisplayNames_GetsItRight()
        {
            ...
            File.WriteAllText(
                jsonPath,
                @"{""language-display-names"":{""sok"":""Sokoro"",""en"":""English"",""de"":""Custom German Name"",""fr"":""French"",""tza"":""Tanzanian Sign Language""}}"
            );
