...
namespace BloomTests.Book
{
	...
	public class XMatterHelperTests
	{
		...
		public void Constructor_CreateDeviceHelperIfRequested(string baseXmatterName, bool deviceOverride, string expected)
		{
			var fileLocator = new FileLocator(new[] { _factoryXMatter, _testXmatter });
			var helper = new XMatterHelper(_dom, baseXmatterName, fileLocator, deviceOverride);
		...
		public void TestBookSpecifiesXMatter(string xmatterBook, string expected, bool deviceOverride = false)
		{
			var fileLocator = new FileLocator(new[] { _factoryXMatter, _testXmatter });
			...
			if (xmatterBook.Contains("DoesNotExist"))
			{
				...
				{
					helper1 = new XMatterHelper(dom1, "Factory", fileLocator, deviceOverride);
				}
			}
			else
			{
