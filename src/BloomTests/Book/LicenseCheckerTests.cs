        ...
        public void ProblemLanguages_KeepsExactMatchNeedingTrim_RemovesNotMatched_WritesOfflineCache()
        {
            using (
                var folder = new TemporaryFolder(
                    "ProblemLanguages_KeepsExactMatchNeedingTrim_RemovesNotMatched_WritesOfflineCache"
                )
            )
            {
      ...
      ""bjn""
        ...
        public void CheckBook_MetaLicenseId_ReturnsCantUse_IfLangsNotAllowed()
        {
            ...
            using (var folder = SetupDefaultOfflineLicenseInfo())
            {
                ...
                Assert.That(
                    checker.CheckBook(dom, new[] { "en", "fr", "ru", "bjn" }),
                    Is.EqualTo(expectedEnFr)
                );
        ...
        public void CheckBook_HasMeta_NoAccessToSpreadsheet_ReportsCantTest()
        {
            ...
            var checker = new LicenseChecker();
            Assert.That(
                checker.CheckBook(dom, new[] { "en" }),
                Is.EqualTo(LicenseChecker.kCannotReachLicenseServerMessage)
            );
        }
        ...
        public void GetAllowedLanguageCodes_JsonMissingContentId_SkipsOverLine()
        {
            ...
            var observedAllowedLangs =
                SIL.Reflection.ReflectionHelper.GetResult(
                    typeof(LicenseChecker),
                    "GetAllowedLanguageCodes",
                    new object[] { badJson, "" }
                ) as HashSet<string>;
            Assert.That(
                observedAllowedLangs.Contains(""),
                Is.False,
                "Empty string should be skipped over"
            );
            Assert.That(observedAllowedLangs.Count, Is.EqualTo(0));

            // Now look it up with a more "realisitc" key
            observedAllowedLangs =
                SIL.Reflection.ReflectionHelper.GetResult(
                    typeof(LicenseChecker),
                    "GetAllowedLanguageCodes",
                    new object[] { badJson, "kingstone.superbible.*" }
                ) as HashSet<string>;
            Assert.That(observedAllowedLangs.Contains(""), Is.False);
            Assert.That(observedAllowedLangs.Contains("tr"), Is.True);
            Assert.That(
                observedAllowedLangs.Count,
                Is.GreaterThan(0),
                "Search for Kingstone should return the other valid values"
            );
        }
