...
namespace BloomTests.Book
{
	...
	public class BookStorageTests
	{
		...
		{
			File.WriteAllText(_bookPath, "<html><head><link rel='stylesheet' href='BasicBook.css' type='text/css' /></head><body><div class='bloom-page'></div></body></html>");
			var storage = new BookStorage(_folder.FolderPath, _fileLocator);
			storage.Save();
			 AssertThatXmlIn.HtmlFile(_bookPath).HasSpecifiedNumberOfMatchesForXpath("//link[contains(@href, 'BasicBook')]", 1);
		}
