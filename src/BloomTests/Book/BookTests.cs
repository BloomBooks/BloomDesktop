...
");
			...
			Assert.AreEqual(2, imgsBefore.Count(), "2 bloom-imageContainer images had style/size attributes");
			imgsBefore = dom.SafeSelectNodes("//img[@style]").Cast<XmlElement>();
			Assert.AreEqual(2, imgsBefore.Count(), "2 images had style attributes");

			book.BringBookUpToDate(new NullProgress());

			var imgsAfter = dom.SafeSelectNodes("//div[contains(@class,'bloom-imageContainer')]/img[@style|@width|@height]").Cast<XmlElement>();
			Assert.AreEqual(0, imgsAfter.Count(), "No bloom-imageContainer images have style/size attributes after updating");
			imgsAfter = dom.SafeSelectNodes("//img[@style]").Cast<XmlElement>();
			Assert.AreEqual(1, imgsAfter.Count(), "1 image has style attribute after updating");
		}
