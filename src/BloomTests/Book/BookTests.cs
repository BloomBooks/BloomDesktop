...
");
		...
		public void InsertPageAfter_PageWasMarkedExtra_NewPageIsNotMarkedExtra()
		{
			...
			Assert.IsTrue(GetPageFromBookDom(book, 1).GetStringAttribute("class").Contains("bloom-page bloom-monolingual A5Portrait"));
		}
...
<body>
	<div class='bloom-page'><div class='bloom-translationGroup FancyText-style'></div></div>
		...
		private void TestTemplateInsertion(Bloom.Book.Book book, IPage existingPage, string divContent)
		{
			...
			Assert.IsTrue(GetPageFromBookDom(book, 1).GetStringAttribute("class").Contains("bloom-page somekind bloom-monolingual A5Portrait"));
		}
	...
	.normal-style { font-size: 9pt !important; }
		...
		{
			// This is from the Digital Comic Book template.
			const string xml = @"<html><head>
		...
		{
			// This is from the Moon and Cap example book. (lowercase 'div')
			const string xml = @"<html><head>
			    <style type='text/css'>
				    div.coverColor textarea {
