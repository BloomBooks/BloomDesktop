...
namespace BloomTests.Book
{
    class BookCompressorTests
    {
        ...
        private void SetupDirectoryWithHtml(
            string bookHtml,
        ...
        public void CompressDirectory_CompressBookWithExtraHtmlFiles_ExtrasAreIgnored()
        {
            ...
            SetupDirectoryWithHtml(
                ...
                actionsOnFolderBeforeCompressing: folderPath =>
                {
                    ...
                    File.WriteAllText(
                        Path.Combine(extraHtmDir, "causesException.htm"),
                        htmContents
                    );
                }
            );
            ...
            using (
                var bloomPubTempFile = TempFile.WithFilenameInTempFolder(
                    "BookCompressorBloomPub" + BloomPubMaker.BloomPubExtensionWithDot
                )
            )
            {
                BookCompressor.CompressBookDirectory(
                    ...
                    _bookFolder.Path,
                    BloomPubMaker.MakeFilter(_bookFolder.Path),
                    ""
                );
            }
        ...
        public void CompressDirectory_CompressBooksWithBackgroundAudioFiles()
        {
            ...
            SetupDirectoryWithHtml(
                ...
                actionsOnFolderBeforeCompressing: folderPath =>
                {
                    ...
                    File.WriteAllText(
                        Path.Combine(audioDir, "musicfile1.mp3"),
                        "dummy mp3 content"
                    );
                    File.WriteAllText(
                        Path.Combine(audioDir, "musicfile2.ogg"),
                        "dummy ogg content"
                    );
                    File.WriteAllText(
                        Path.Combine(audioDir, "musicfile3.wav"),
                        "dummy wav content"
                    );
                    File.WriteAllText(
                        Path.Combine(audioDir, "narration.mp3"),
                        "more dummy mp3 content"
                    ); // file should be included (since it is referenced)
                    File.WriteAllText(
                        Path.Combine(audioDir, "narration.wav"),
                        "more dummy wav content"
                    ); // file should not be included (only mp3 narration)
                    File.WriteAllText(
                        Path.Combine(folderPath, "temp.tmp"),
                        "dummy temporary file data"
            ...
            using (
                var bloomPubTempFile = TempFile.WithFilenameInTempFolder(
                    "BookCompressorWithAudio" + BloomPubMaker.BloomPubExtensionWithDot
                )
            )
            {
                BookCompressor.CompressBookDirectory(
        ...
        public void CompressDirectory_CompressBooksWithExtraFiles()
        {
            ...
            SetupDirectoryWithHtml(
                ...
                actionsOnFolderBeforeCompressing: folderPath =>
                {
                    ...
                    File.WriteAllText(
                        Path.Combine(audioDir, "midiMusic.mid"),
                        "dummy midi file data"
                    );
                    ...
                    File.WriteAllText(
                        Path.Combine(subAudioDir, "other.mp3"),
                    ...
                    File.WriteAllText(
                        Path.Combine(videoDir, "signlanguageVid.mp4"),
                        "dummy video content"
                    );
                    // These 2 should not be included (non-whitelisted extensions)
                    ...
                    File.WriteAllText(
                        Path.Combine(subVideoDir, "other.mp4"),
                        "dummy video file data"
                    );
                    ...
                    File.WriteAllText(
                        Path.Combine(widgetResources, "includable.blob"),
                    ...
                    File.WriteAllText(
                        Path.Combine(bookWithinABookDir, "extra book.html"),
                        "dummy html content"
                    );
                }
            );
            ...
            using (
                var bloomPubTempFile = TempFile.WithFilenameInTempFolder(
                    "BookCompressorWithExtraFiles" + BloomPubMaker.BloomPubExtensionWithDot
                )
            )
            {
                BookCompressor.CompressBookDirectory(
