...
namespace BloomTests.Edit
{
    ...
    public class ConfiguratorTest
    {
        ...
        {
            Assert.IsTrue(
                Configurator.IsConfigurable(Get_NotYetConfigured_CalendardBookStorage().FolderPath)
            );
        }
        ...
        public void CollectionSettingsAreRoundTriped()
        {
            ...
            var stringRep = DynamicJson.Serialize(new { library = new { stuff = "foo" } });
        ...
        {
            var firstData = DynamicJson.Serialize(
                new { library = new { one = "1", color = "red" } }
            );
            var secondData = DynamicJson.Serialize(
                new { library = new { two = "2", color = "blue" } }
            );
        ...
        {
            var first = new Configurator(_collectionFolder.Path);
            var stringRep = DynamicJson.Serialize(new { library = new { librarystuff = "foo" } });
        ...
        private BookStorage Get_NotYetConfigured_CalendardBookStorage()
        {
            ...
            var path = GetPathToHtml(
                _starter.CreateBookOnDiskFromTemplate(source, _collectionFolder.Path)
            );
...
<html>
  ...
  <body onload=""getYear()"">
    <form id=""form"">
            ...
            AssertXml.HasSpecifiedNumberOfMatchesForXpath(
                "//script[@type='text/javascript' and @src='jquery-1.10.1.js']",
                1
            );
            AssertXml.HasSpecifiedNumberOfMatchesForXpath(
                "//script[@type='text/javascript' and @src='form2object.js']",
                1
            );
            AssertXml.HasSpecifiedNumberOfMatchesForXpath(
                "//script[@type='text/javascript' and @src='js2form.js']",
                1
            );
            AssertXml.HasSpecifiedNumberOfMatchesForXpath(
                "//script[@type='text/javascript' and @src='underscore.js']",
                1
            );
            ...
            AssertXml.HasSpecifiedNumberOfMatchesForXpath(
                "//script[@type='text/javascript' and @id='configuredScript' and contains(text(), '\"dayAbbreviations\"')]",
                1
            );
        }
    }
