...
<Collection version='0.2'>
  ...
  {extraElements}
        ...
        /// <summary>
        /// The two sources can disagree, and neither wins outright: we take whichever demands more.
        /// The repository copy lives in a zip that can be mid-sync and momentarily missing the
        /// element, and this is a protection, so the failure we can live with is keeping someone out
        /// a little longer than necessary rather than letting them in when we should not have.
        /// </summary>
        [Test]
        public void IsThisBloomTooOld_RepoSaysNoRequirementButFileDoes_StillSaysTooOld()
        {
            var path = WriteSettingsFile(
                "RepoSilentFileDemands",
        ...
        /// <summary>
        /// The other direction of the same rule: a requirement in the file we already meet must not
        /// water down a stiffer one the repository has since told us about.
        /// </summary>
        [Test]
        public void IsThisBloomTooOld_FileAsksLessThanTheRepoDoes_TakesTheRepoRequirement()
        {
            var path = WriteSettingsFile(
                "FileAsksLess",
                "<MinimumBloomVersion>1.0</MinimumBloomVersion>"
            );

            // Sanity check: on the file alone, any Bloom is welcome.
            Assert.That(
                MinimumBloomVersionCheck.IsThisBloomTooOld(path, out _),
                Is.False,
                "Test setup problem: the file should demand nothing this Bloom can't meet."
            );
