...
namespace BloomTests.WebLibraryIntegration
{
    ...
    public class BloomLibraryBookApiClientTests
    {
        ...
        {
            Assert.IsTrue(
                _client.TestOnly_LegacyLogIn("unittest@example.com", "unittest"),
        ...
        [
            Test,
            Ignore(
                "This is failing sometimes on TeamCity; and since the main logic it tests is TestOnly_LegacyLogIn (which is no longer production code), it doesn't seem worth the intermittent failures."
            )
        ]
        ...
        private string CreateBookRecord()
        {
            ...
            var result = _client.TestOnly_CreateBookRecord(
                string.Format(
                    "{{\"bookInstanceId\":\"{0}\",\"title\":\"{1}\",{2}}}",
                    bookInstanceId,
                    title,
                    _client.UploaderJsonString
                )
            );
            ...
            RetryUtility.Retry(
                () =>
                {
                    ...
                    catch (RuntimeBinderException)
                    {
                        ...
                        Console.WriteLine(
                            "Attempt to deserialize response content session token failed while attempting log in to parse (BL-4099)."
                        );
                        Console.WriteLine(
                            $"username: {request.Parameters.Where(p => p.Name == "username")}"
                        );
                ...
                new HashSet<Type>
                {
                    typeof(RuntimeBinderException).Assembly.GetType(
