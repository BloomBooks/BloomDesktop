...
namespace BloomTests.WebLibraryIntegration
{
    ...
    public class BookTransferDupIdTests
    {
        ...
        public void Setup()
        {
            ...
            Assert.AreEqual(
                0,
                Directory.GetDirectories(_workFolderPath).Length,
                "Some stuff was left over from a previous test"
            );
            Assert.AreEqual(
                0,
                Directory.GetFiles(_workFolderPath).Length,
                "Some stuff was left over from a previous test"
            );
            SetupTestData();
        }
        ...
        private static void SetupMetaData(TemporaryFolder folder, string guid, DateTime timestamp)
        {
            ...
            var metaData =
                @"{""experimental"":""false"",""bookInstanceId"":"""
                + guid
        ...
        public void DuplicateIdTest()
        {
            ...
            Assert.That(
                GetInstanceIdFromMetadataFile(book1),
                Is.Not.EqualTo(_guid1),
                "book1 should have changed guid1"
            );
            Assert.That(
                GetInstanceIdFromMetadataFile(book2),
                Is.EqualTo(_guid1),
                "book2 should not have changed guid1"
            );
            Assert.That(
                GetInstanceIdFromMetadataFile(book3),
                Is.Not.EqualTo(_guid1),
                "book3 should have changed guid1"
            );
            Assert.That(
                GetInstanceIdFromMetadataFile(book1),
                Is.Not.EqualTo(GetInstanceIdFromMetadataFile(book3)),
                "the two changed IDs for guid1 should be different from each other"
            );
            Assert.That(
                GetInstanceIdFromMetadataFile(book4),
                Is.EqualTo(_guid2),
                "book4 should have guid2"
            );
            Assert.That(
                GetInstanceIdFromMetadataFile(book4),
                Is.Not.EqualTo(GetInstanceIdFromMetadataFile(book5)),
                "book4 should have different guid than book5"
            );
        }
    }
}
