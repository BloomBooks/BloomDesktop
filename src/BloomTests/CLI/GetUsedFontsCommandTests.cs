...
namespace BloomTests.CLI
{
    class GetUsedFontsCommandTests
    {
        ...
        public void GetFontsUsed_ExcludeFontForLanguageNotInBook(
            string[] languagesInHtml,
            string[] languagesInCss
        )
        {
            var htmlContents =
                $"<html><body><div class='bloom-page'>{string.Join("", languagesInHtml.Select(GetLangHtml))}</div></body></html>";
            ...
            using (var bookFolder = new TemporaryFolder(MethodBase.GetCurrentMethod().Name))
            {
                RobustFile.WriteAllText(
                    Path.Combine(bookFolder.FolderPath, "book.htm"),
                    htmlContents
                );
                RobustFile.WriteAllText(
                    Path.Combine(bookFolder.FolderPath, "defaultLangStyles.css"),
                    cssContents
                );
