...
namespace BloomTests.Publish
{
    public class AudioProcessorTests
    {
        ...
        public void AudioProcessorGetTrueForAllAudioElements_MixedSpansWithIds_OnlyAudioSpansAreProcessed()
        {
            ...
            bool result = SIL.Reflection.ReflectionHelper.GetBoolResult(
                typeof(AudioProcessor),
                "IsTrueForAllAudioSentenceElements",
                new object[] { "bookFolderPath", dom.RawDom, failOnInvalidInputPredicate }
            );

            Assert.IsTrue(
                result,
                "An invalid input was passed to the predicate. Make sure the unit under test removes all invalid inputs."
            );
        }
        ...
        private static bool ReturnTrueOnlyIfAudioSentence(string wavPath, string mp3Path)
        {
            ...
            return wavPath.Contains(audioSentenceFilenameWithoutExtension)
                && mp3Path.Contains(audioSentenceFilenameWithoutExtension);
        }
        ...
        public void AudioProcessorGetTrueForAllAudioElements_AudioSpansAndDivs_CheckAudioDivsAreProcessedToo()
        {
            ...
            bool result = SIL.Reflection.ReflectionHelper.GetBoolResult(
                typeof(AudioProcessor),
                "IsTrueForAllAudioSentenceElements",
                new object[] { "bookFolderPath", dom.RawDom, failIfSearchTargetFound }
            );

            Assert.IsFalse(
                result,
                "The method should've processed specialAudioRecordingGuid but it actually didn't"
            );
        }
