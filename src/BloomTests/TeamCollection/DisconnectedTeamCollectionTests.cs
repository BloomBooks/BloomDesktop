...
namespace BloomTests.TeamCollection
{
    public class DisconnectedTeamCollectionTests
    {
        ...
        public void GetStatus_NoLocalStatus_ReturnsNewBookStatus()
        {
            using (
                var collectionFolder = new TemporaryFolder(
                    "GetStatus_NoLocalStatus_ReturnsNewBookStatus"
                )
            )
            {
                ...
                var tc = new DisconnectedTeamCollection(
        ...
        public void GetStatus_LocalStatusWrongId_ReturnsNewBookStatus()
        {
            using (
                var collectionFolder = new TemporaryFolder(
                    "GetStatus_LocalStatusWrongId_ReturnsNewBookStatus"
                )
            )
            {
                var mockTcManager = new Mock<ITeamCollectionManager>();
                SyncAtStartupTests.MakeFakeBook(
                    collectionFolder.FolderPath,
                    "Copied book",
                    "This is only local but has status"
                );
                var tc = new DisconnectedTeamCollection(
                    mockTcManager.Object,
                    collectionFolder.FolderPath,
        ...
        public void GetStatus_LocalStatus_ReturnsLocalStatus_WithoutOldName()
        {
            using (
                var collectionFolder = new TemporaryFolder(
                    "GetStatus_LocalStatus_ReturnsLocalStatus"
                )
            )
            {
                var mockTcManager = new Mock<ITeamCollectionManager>();
                SyncAtStartupTests.MakeFakeBook(
                    collectionFolder.FolderPath,
                    "Repo book",
                    "This is simulating a book that's in repo"
                );
                var tc = new DisconnectedTeamCollection(
                    mockTcManager.Object,
                    collectionFolder.FolderPath,
                ...
                tc.WriteLocalStatus(
                    "Repo book",
                    new BookStatus()
                        .WithChecksum("a checksum")
                        .WithLockedBy("fred@somewhere.org")
                        .WithOldName("Renamed from")
                );
