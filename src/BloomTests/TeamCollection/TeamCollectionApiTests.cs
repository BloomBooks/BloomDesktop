	...
	public class TeamCollectionApiTests
	{
		...
		public void OneTimeSetUp()
		{
			...
			_api = new TeamCollectionApi(new CollectionSettings(collectionPath), new BookSelection(),
				new TeamCollectionManager(collectionPath, new BloomWebSocketServer(), new BookRenamedEvent()), null,  null);
		}
		...
		{
			using (var tempFolder = new TemporaryFolder("ProblemsWithLocation_ExistingTC_Fails"))
			{
		...
		{
			using (var tempFolder = new TemporaryFolder("ProblemsWithLocation_FolderIsATC_Fails"))
			{
		...
		{
			using (var tempFolder = new TemporaryFolder("ProblemsWithLocation_BloomCollection_Fails"))
			{
		...
		{
			using (var tempFolder = new TemporaryFolder("ProblemsWithLocation_DestFolderExists_Fails"))
			{
