...
namespace BloomTests.TeamCollection.Cloud
{
    ...
    public class CloudAuthTests
    {
        ...
        public void RealDpapiTokenStore_SessionSurvivesSimulatedRestart()
        {
            ...
            // up in the app. The local/fake provider stands in for Firebase, whose refresh follows the
        ...
        /// <summary>
        /// Live-verifies <see cref="LocalCloudAuthProvider"/> against a real local Supabase
        ...
        /// `dotnet test --filter FullyQualifiedName~LiveLocalProvider`. Exercises exactly the
        ...
        /// different local test users concurrently must each hold their own distinct, valid session.
        /// </summary>
        [Test]
        [Explicit("Requires the local Supabase stack (`supabase start`) to be running.")]
        public void LiveLocalProvider_TwoUsersSignInConcurrently_HoldDistinctSessions()
        {
            var env = CloudEnvironment.FromEnvironment();
            var aliceAuth = new CloudAuth(new LocalCloudAuthProvider(env));
            var bobAuth = new CloudAuth(new LocalCloudAuthProvider(env));
        ...
        public void GetLoginState_ReportsAuthModeAndCurrentIdentity()
        {
            ...
            var auth = new CloudAuth(provider, new FakeCloudTokenStore());
            var env = new CloudEnvironment(name =>
                name == "BLOOM_CLOUDTC_AUTH_MODE" ? "local" : null
            );

            var loggedOutState = auth.GetLoginState(env);
            Assert.That(loggedOutState.AuthMode, Is.EqualTo("local"));
        ...
        public void GetLoginState_CloudMode_ReportsCloudNotReal()
        {
            ...
            // ever declared "local" | "cloud", so the C# side must actually send "cloud".
