...
namespace BloomTests.TeamCollection.Cloud
{
    ...
    {
        [Test]
        public void NoEnvironmentVariablesSet_FallsBackToLocalStackDefaults()
        {
            var env = new CloudEnvironment(name => null);

            // These defaults must match server/dev/README.md's "Local value" column so a plain
            // checkout of Bloom talks to the local stack with zero configuration.
            Assert.That(env.SupabaseUrl, Is.EqualTo("http://127.0.0.1:54321"));
            Assert.That(env.S3Endpoint, Is.EqualTo("http://127.0.0.1:9000"));
            Assert.That(env.AuthMode, Is.EqualTo(CloudAuthMode.Local));
            Assert.That(env.AutoSignInEmail, Is.Null);
            Assert.That(env.AutoSignInPassword, Is.Null);
        ...
        public void EnvironmentVariables_OverrideCompiledDefaults()
        {
            ...
            Assert.That(env.AutoSignInEmail, Is.EqualTo("override@dev.local"));
            Assert.That(env.AutoSignInPassword, Is.EqualTo("pw"));
        ...
        [TestCase("local", CloudAuthMode.Local)]
        [TestCase("LOCAL", CloudAuthMode.Local)]
        [TestCase("cloud", CloudAuthMode.Cloud)]
        [TestCase("CLOUD", CloudAuthMode.Cloud)]
        [TestCase("", CloudAuthMode.Local)]
        [TestCase("garbage", CloudAuthMode.Local)]
        public void AuthMode_ParsesCaseInsensitivelyAndDefaultsToLocal(
            string raw,
