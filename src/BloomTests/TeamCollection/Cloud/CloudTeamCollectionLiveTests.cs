...
{
    /// <summary>
    /// [Explicit] live-infrastructure tests against the real local stack (Supabase Postgres/
    ...
    ///
    /// Uses a single local test account (alice) for both "sides" of the round trip -- two separate local
    ...
    public class CloudTeamCollectionLiveTests
    {
        [Test]
        [Explicit(
            "Requires the local Supabase stack running, INCLUDING `supabase functions serve --env-file server/dev/functions.env` (not started by `supabase start` alone)."
        )]
        public void LiveStack_SendThenReceive_RoundTripsBookContentAndLock()
        {
            var environment = CloudEnvironment.FromEnvironment();
            var senderAuth = new CloudAuth(new LocalCloudAuthProvider(environment));
            ...
            var receiverAuth = new CloudAuth(new LocalCloudAuthProvider(environment));
        ...
        {
            var environment = CloudEnvironment.FromEnvironment();
            var auth = new CloudAuth(new LocalCloudAuthProvider(environment));
