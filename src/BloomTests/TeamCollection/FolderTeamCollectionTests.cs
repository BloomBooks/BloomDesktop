...
namespace BloomTests.TeamCollection
{
    public class FolderTeamCollectionTests
    {
        ...
        public void OneTimeSetup()
        {
            ...
            RobustFile.WriteAllText(bookPath, XmlHtmlConverter.CreateDocumentWithBodyContent("This is just a dummy"));
            ...
            RobustFile.WriteAllText(anotherBookPath, XmlHtmlConverter.CreateDocumentWithBodyContent("This is just a dummy for another book"));
        ...
        {
            Assert.That(
                _collection.GetLocalStatus("My book").checksum,
                Is.EqualTo(_myBookStatus.checksum)
            );
            Assert.That(
                _collection.GetLocalStatus(kAnotherBook).checksum,
                Is.EqualTo(_anotherBookStatus.checksum)
            );
        }
        ...
        public void CopyAllBooksFromSharedToLocalFolder_RetrievesExpectedBooks_AndChecksums()
        {
            using (var destFolder = new TemporaryFolder("GetBooks_Retrieves"))
            {
                ...
                Assert.That(
                    ...
                    Is.EqualTo(XmlHtmlConverter.CreateDocumentWithBodyContent("This is just a dummy"))
                );
                var destCssPath = Path.Combine(destBookFolder, "BasicLayout.css");
                Assert.That(
                    RobustFile.ReadAllText(destCssPath),
                    Is.EqualTo("This is another dummy")
                );
                ...
                var anotherDestBookPath = Path.Combine(
                    anotherDestBookFolder,
                    kAnotherBook + ".htm"
                );
                Assert.That(
                    RobustFile.ReadAllText(anotherDestBookPath),
                    Is.EqualTo(XmlHtmlConverter.CreateDocumentWithBodyContent("This is just a dummy for another book"))
                );

                Assert.That(
                    Directory.EnumerateDirectories(destFolder.FolderPath).Count(),
                    Is.EqualTo(2)
                );
        ...
        public void PutBook_DoesNotRaiseNewBookEvent()
        {
            ...
            RobustFile.WriteAllText(
                bookPath,
                XmlHtmlConverter.CreateDocumentWithBodyContent("This is our newly put book")
            );
        ...
        {
            var bloomBookPath = Path.Combine(
                _repoFolder.FolderPath,
                "Books",
                "put book to modify.bloom"
            );
            ...
            RobustFile.WriteAllText(
                bloomBookPath,
                XmlHtmlConverter.CreateDocumentWithBodyContent(@"This is changed")
            ); // no, not a zip at all
        ...
        public void DeletedBook_RaisesDeleteRepoBookFileEvent()
        {
            var bloomBookPath = Path.Combine(
                _repoFolder.FolderPath,
                "Books",
                "put book to delete.bloom"
            );
            // Don't use PutBook here...changing the file immediately after putting it won't work,
            // because of the code that tries to prevent notifications of our own checkins.
            RobustFile.WriteAllText(
                bloomBookPath,
                XmlHtmlConverter.CreateDocumentWithBodyContent(@"This is original")
            ); // no, not a zip at all
        ...
        public void LockBook_WhoHasBookLocked_ReturnsLocker_UnlockClears()
        {
            ...
            Assert.That(
                _collection.WhatComputerHasBookLocked("My book"),
                Is.EqualTo(TeamCollectionManager.CurrentMachine)
            );
        ...
        public void WhoHasBookLocked_NotInRepoButInLocal_ReturnsThisUser()
        {
            ...
            Assert.That(
                _collection.WhoHasBookLocked("newly created book"),
        ...
        {
            var collectionFilePath = Bloom.TeamCollection.TeamCollection.CollectionPath(
                _collectionFolder.FolderPath
            );
            ...
            {
                File.WriteAllText(
                    Path.Combine(_collectionFolder.FolderPath, name),
                    @"contents of " + name
                );
            }
            ...
            using (
                var tempDest = new TemporaryFolder(
                    "CopySharedCollectionFilesToLocal_RetrievesFilePut"
                )
            )
            {
                var destCollectionFilePath = Path.Combine(
                    tempDest.FolderPath,
                    Path.GetFileName(collectionFilePath)
                );
                File.WriteAllText(destCollectionFilePath, "This should get overwritten");

                var removeMePath = Path.Combine(
                    tempDest.FolderPath,
                ...
                Assert.That(
                    File.ReadAllText(destCollectionFilePath),
                    Is.EqualTo("This is a fake collection")
                );
                var destStylesPath = Path.Combine(
                    tempDest.FolderPath,
                    "customCollectionStyles.css"
                );
                ...
                {
                    Assert.That(
                        File.ReadAllText(Path.Combine(tempDest.FolderPath, name)),
                        Is.EqualTo(@"contents of " + name)
                    );
                }

                Assert.That(
                    File.Exists(Path.Combine(tempDest.FolderPath, "ReaderToolsSettings-de.json")),
                    Is.False
                );

                Assert.That(
                    File.ReadAllText(allowedWords1DestPath),
                ...
                Assert.That(
                    File.Exists(allowedWords3DestPath),
                    Is.False,
                    "failed to delete allowed words not in repo"
                );
                Assert.That(
                    File.ReadAllText(
                        Path.Combine(tempDest.FolderPath, "Sample Texts", "texts1.txt")
                    ),
