// Constants describing the local stack (server/dev/README.md) that this harness drives
...
// Local test users seeded by server/dev/seed.sql. All share this password.
export const LOCAL_PASSWORD = "BloomDev123!";

export interface LocalStackUser {
    email: string;
    password: string;
}

export const ALICE: LocalStackUser = {
    email: "alice@dev.local",
    password: LOCAL_PASSWORD,
};
export const BOB: LocalStackUser = {
    email: "bob@dev.local",
    password: LOCAL_PASSWORD,
};
export const ADMIN: LocalStackUser = {
    email: "admin@dev.local",
    password: LOCAL_PASSWORD,
};

// Builds the BLOOM_CLOUDTC_* environment block for a Bloom.exe instance signed in as `user`.
export const cloudTcEnv = (user?: LocalStackUser): NodeJS.ProcessEnv => ({
