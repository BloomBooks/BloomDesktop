...
namespace BloomTests
{
    ...
    {
        [Test]
        public void CreateDocumentWithContent_CreatesDocStructure()
        {
            var bodyContent = "<div>foo</div>";
            var bodyContentWithNoWhitespace = Regex.Replace(bodyContent, @"\s+", "");
            string doc1 = XmlHtmlConverter.CreateDocumentWithContent(bodyContent);
            ...
            string doc2 = XmlHtmlConverter.CreateDocumentWithContent(bodyContent, headContent);
        ...
        {
            var doc = XmlHtmlConverter.CreateDocumentWithContent(
        ...
        public void SaveAsHTML_HasXHTMLSelfClosingDiv_ChangesToHTMLStandard()
        {
            ...
            dom.LoadXml(XmlHtmlConverter.CreateDocumentWithContent("<div data-book='test'/>"));
        ...
        public void SaveAsHTML_HasEmUpAgainstStrong_DoesNotInsertSpace()
        {
            ...
            dom.LoadXml(
                XmlHtmlConverter.CreateDocumentWithContent("<div data-book='test'/>" + original)
            );
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
        ...
        public void SaveAsHTM_HasEmptyParagraphsAndCites_RetainsThem()
        {
            ...
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(XmlHtmlConverter.CreateDocumentWithContent(pattern));
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
                var r = new Regex("<p");
        ...
        public void SaveAsHtml_VoidElements_ConvertsToSelfClosing(string tag)
        {
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(
                XmlHtmlConverter.CreateDocumentWithContent($"<{tag} data-foo='bar'></{tag}>")
            );
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
        ...
        public void SaveAsHtml_NonVoidElement_DoesNotConvert()
        {
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(
                XmlHtmlConverter.CreateDocumentWithContent($"<span data-foo='bar'></span>")
            );
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
        ...
        public void SaveAsHtml_HasSvgs_DoesNotChoke()
        {
            ...
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(XmlHtmlConverter.CreateDocumentWithContent($"{svg}<div>foobar</div>"));
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
                var text = File.ReadAllText(temp.Path);
        ...
        public void GetXmlDomFromHtml_HasSvgs_DoesNotChoke()
        {
            string svg =
                @"<svg xmlns=""http://www.w3.org/2000/svg""><path d=""M 0 0 L 10 10"" /></svg>";
            string html = XmlHtmlConverter.CreateDocumentWithContent($"{svg}<div>foobar</div>");
        ...
        {
            var html = XmlHtmlConverter.CreateDocumentWithContent("<div><br></br></div>");
            var dom = XmlHtmlConverter.GetXmlDomFromHtml(html, false);
            var found = 0;
        ...
        {
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(XmlHtmlConverter.CreateDocumentWithContent("<br />"));
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
                var text = File.ReadAllText(temp.Path);
        ...
        {
            var html = XmlHtmlConverter.CreateDocumentWithContent(
...
<b>six</b>seven <i>eight</i>nine</div>"
        ...
        public void GetXmlDomFromHtml_HasEmptyParagraphs_RetainsEmptyParagraphs()
        {
            ...
            var html = XmlHtmlConverter.CreateDocumentWithContent(pattern);
            var dom = XmlHtmlConverter.GetXmlDomFromHtml(html, false);
        ...
        {
            var html = XmlHtmlConverter.CreateDocumentWithContent("<div><u> </u></div>");
        ...
        public void GetXmlDomFromHtml_HasEmptyTagsWithAttributes_NoRemoveTags()
        {
            ...
            html = XmlHtmlConverter.CreateDocumentWithContent(
...
";
        ...
        {
            var html = XmlHtmlConverter.CreateDocumentWithContent("<div data-book='test'/>");
