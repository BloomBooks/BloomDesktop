...
namespace BloomTests
{
    ...
    {
        [Test]
        public void CreateValidHtmlString_CreatesDocStructure()
        {
            var bodyContent = "<div>foo</div>";
            var bodyContentWithNoWhitespace = Regex.Replace(bodyContent, @"\s+", "");
            string doc1 = XmlHtmlConverter.CreateValidHtmlString(bodyContent);
            ...
            string doc2 = XmlHtmlConverter.CreateValidHtmlString(bodyContent, headContent);
        ...
        {
            var doc = XmlHtmlConverter.CreateValidHtmlString(
        ...
        public void SaveAsHTML_HasXHTMLSelfClosingDiv_ChangesToHTMLStandard()
        {
            ...
            dom.LoadXml(XmlHtmlConverter.CreateValidHtmlString("<div data-book='test'/>"));
        ...
        public void SaveAsHTML_HasEmUpAgainstStrong_DoesNotInsertSpace()
        {
            ...
            dom.LoadXml(
                XmlHtmlConverter.CreateValidHtmlString("<div data-book='test'/>" + original)
            );
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
        ...
        public void SaveAsHTM_HasEmptyParagraphsAndCites_RetainsThem()
        {
            ...
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(XmlHtmlConverter.CreateValidHtmlString(pattern));
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
                var r = new Regex("<p");
        ...
        {
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(XmlHtmlConverter.CreateValidHtmlString($"<{tag} data-foo='bar'></{tag}>"));
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
                var text = File.ReadAllText(temp.Path);
        ...
        {
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(XmlHtmlConverter.CreateValidHtmlString($"<span data-foo='bar'></span>"));
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
                var text = File.ReadAllText(temp.Path);
        ...
        public void SaveAsHtml_HasSvgs_DoesNotChoke()
        {
            ...
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(XmlHtmlConverter.CreateValidHtmlString($"{svg}<div>foobar</div>"));
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
                var text = File.ReadAllText(temp.Path);
        ...
        public void GetXmlDomFromHtml_HasSvgs_DoesNotChoke()
        {
            string svg =
                @"<svg xmlns=""http://www.w3.org/2000/svg""><path d=""M 0 0 L 10 10"" /></svg>";
            string html = XmlHtmlConverter.CreateValidHtmlString($"{svg}<div>foobar</div>");
        ...
        {
            var html = XmlHtmlConverter.CreateValidHtmlString("<div><br></br></div>");
            var dom = XmlHtmlConverter.GetXmlDomFromHtml(html, false);
            var found = 0;
        ...
        {
            var dom = SafeXmlDocument.Create();
            dom.LoadXml(XmlHtmlConverter.CreateValidHtmlString("<br />"));
            using (var temp = new TempFile())
            {
                XmlHtmlConverter.SaveDOMAsHtml5(dom, temp.Path);
                var text = File.ReadAllText(temp.Path);
        ...
        {
            var html = XmlHtmlConverter.CreateValidHtmlString(
...
<b>six</b>seven <i>eight</i>nine</div>"
        ...
        public void GetXmlDomFromHtml_HasEmptyParagraphs_RetainsEmptyParagraphs()
        {
            ...
            var html = XmlHtmlConverter.CreateValidHtmlString(pattern);
            var dom = XmlHtmlConverter.GetXmlDomFromHtml(html, false);
        ...
        {
            var html = XmlHtmlConverter.CreateValidHtmlString("<div><u> </u></div>");
        ...
        {
            var html = XmlHtmlConverter.CreateValidHtmlString("<div><u style=\"test\"> </u></div>");
            var dom = XmlHtmlConverter.GetXmlDomFromHtml(html);
            var xml = dom.DocumentElement.GetElementsByTagName("body")[0].InnerXml;
            Assert.AreEqual("<div><u style=\"test\"> </u></div>", xml.Trim());

            html = XmlHtmlConverter.CreateValidHtmlString("<div><u><i style=\"test\" /></u></div>");
...
";
        ...
        {
            var html = XmlHtmlConverter.CreateValidHtmlString("<div data-book='test'/>");
