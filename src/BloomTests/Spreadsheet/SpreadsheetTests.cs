...
";
		...
		//TODO refactor to avoid repeat code, make a setup?
		[Test]
		public void RetainMarkupOptionRetainsMarkup()
		{
			using (var tempFile = TempFile.WithExtension("xslx"))
			{
				...
				using (var package = new ExcelPackage(info))
				{
					var worksheet = package.Workbook.Worksheets[0];
					int c = _sheetFromExport.StandardLeadingColumns.Length;
					bool markedUpTextFound = false;
					for (int r = 0; r < 4; r++)
					{
						ExcelRange currentCell = worksheet.Cells[r + 1, c + 1];
						//string markedUp = "elephant";
						...
						if (currentCell.Value != null && currentCell.Value.ToString().Contains(markedUp))
						{
							markedUpTextFound = true;
							break;
						}
					}
					Assert.That(markedUpTextFound);
				}
			}
		}
	}
