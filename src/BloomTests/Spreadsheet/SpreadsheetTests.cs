...
";
		...
		{
			SetupFor(source);
			var enCol = _sheet.GetColumnForLang("en");
			var frCol = _sheet.GetColumnForLang("fr");
			var deCol = _sheet.GetColumnForLang("de");
		...
		public void ColumnsNotLanguagesNotCounted()
		{
			...
			sheet.SetColumnForLang("en", "English");
			sheet.SetColumnForLang("es", "Español");
		...
		public void RetainMarkupOptionRetainsMarkup()
		{
			using (var tempFile = TempFile.WithExtension("xslx"))
			{
				...
				{
					var worksheet = package.Workbook.Worksheets[0];
					int c = _sheetFromExport.GetColumnForLang("en");
