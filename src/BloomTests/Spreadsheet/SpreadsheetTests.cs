...
";
		...
		// case to select one pair (_sheetXml, _rowsXml or _sheetFile, _rowsFile)
		// to set as _sheet and _rows.
		...
		public void OneTimeSetUp()
		{
			...
			_rowsFromExport = _sheetFromExport.GetContentRows().ToList();
			using (var tempFile = TempFile.WithExtension("xslx"))
			{
				...
				_rowsFromFile = _sheetFromFile.GetContentRows().ToList();
			}
		}
		...
		public void SavesTextFromBlocks(string source)
		{
			...
			Assert.That(_rows[0].Cell(offset).Text, Is.EqualTo("This elephant is running amok. Causing much damage."));
			Assert.That(_rows[0].Cell(offset + 1).Text, Is.EqualTo("This French elephant is running amok. Causing much damage."));
			Assert.That(_rows[1].Cell(offset).Text, Is.EqualTo("Elephants should be handled with much care."));
			// French is third in its group, but French is the second language encountered, so it should be in the second cell of its row.
			Assert.That(_rows[1].Cell(offset + 1).Text, Is.EqualTo("French elephants should be handled with special care."));
			Assert.That(_rows[1].Cell(offset + 2).Text, Is.EqualTo("German elephants are quite orderly."));

			Assert.That(_rows[2].Cell(offset).Text, Is.EqualTo("Riding on elephants can be risky."));
			Assert.That(_rows[2].Cell(offset + 1).Text, Is.EqualTo("Riding on French elephants can be more risky."));
			Assert.That(_rows[2].Cell(offset + 2).Text, Is.EqualTo("Riding on German elephants can be less risky."));
		...
		public void InsertsGroupNumbers(string source)
		{
			...
			Assert.That(_rows[0].Cell(groupIndex).Content, Is.EqualTo("1"));
			Assert.That(_rows[1].Cell(groupIndex).Content, Is.EqualTo("2"));
			Assert.That(_rows[2].Cell(groupIndex).Content, Is.EqualTo("1"));
		...
		public void AddsRowLabels(string source)
		{
			...
			Assert.That(_rows[0].Cell(pageNumIndex).Content, Is.EqualTo("1"));
			Assert.That(_rows[1].Cell(pageNumIndex).Content, Is.EqualTo("1"));
			Assert.That(_rows[2].Cell(pageNumIndex).Content, Is.EqualTo("2"));
			Assert.That(_rows[0].Cell(0).Content, Is.EqualTo(InternalSpreadsheet.TextGroupLabel));
			Assert.That(_rows[1].Cell(0).Content, Is.EqualTo(InternalSpreadsheet.TextGroupLabel));
			Assert.That(_rows[2].Cell(0).Content, Is.EqualTo(InternalSpreadsheet.TextGroupLabel));
		}
...
";
		...
		public void RetainMarkup_KeepsIt()
		{
			...
			var rows = sheet.GetContentRows().ToList();
			var offset = sheet.StandardLeadingColumns.Length;
			Assert.That(rows[0].Cell(offset).Content.Trim(), Is.EqualTo(
				"<p><span id=\"e4bc05e5-4d65-4016-9bf3-ab44a0df3ea2\" class=\"bloom-highlightSegment\" recordingmd5=\"undefined\">This elephant is running amok.</span> <span id=\"i2ba966b6-4212-4821-9268-04e820e95f50\" class=\"bloom-highlightSegment\" recordingmd5=\"undefined\">Causing much damage.</span></p>"));
		}
		...
		public void SortTranslationGroups_YieldsExpectedOrder()
		{
			...
			var output = SpreadsheetExporter.SortTranslationGroups(input);
