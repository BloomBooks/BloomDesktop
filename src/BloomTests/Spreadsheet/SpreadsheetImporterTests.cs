...
namespace BloomTests.Spreadsheet
{
	...
	public class SpreadsheetImporterChangeLanguageTests
	{
		...
		public void OneTimeSetUp()
		{
			...
			_sheet.GetContentRows().First().SetCell(_sheet.ColumnForLang("en"), "<p>This elephant is running amok.</p>");
			_sheet.GetContentRows().Skip(2).First().SetCell(_sheet.ColumnForLang("fr"), "<p>Riding on French elephants can be very risky.</p>");
		...
		// By default German, which is not in the sheet, should be kept. Subclass with RemoveOtherLanguages overrides.
		[Test]
		public virtual void GermanKeptOrRemoved()
		{
			var assertDom = AssertThatXmlIn.Dom(_dom.RawDom);
		...
		public void EnglishAndFrenchKeptOrModified()
		{
			...
			// Now, did it put things in the right places? "This elephant is running amok." which we put in the first row
			// should be placed into the SECOND translation grou
	...
	public class SpreadsheetImporterChangeLanguageCleanTests : SpreadsheetImporterChangeLanguageTests
	{
		...
		// With RemoveOtherLanguages set, German should be removed.
		[Test]
		public override void GermanKeptOrRemoved()
		{
			var assertDom = AssertThatXmlIn.Dom(_dom.RawDom);
