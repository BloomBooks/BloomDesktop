...
";
		...
		public void RoundtripSimpleFormatting(string source)
		{
			...
			Assert.That(!FormatNodeContainsText("//div[@id='simpleFormattingTest']//strong", "dog"));
			Assert.That(!FormatNodeContainsText("//div[@id='simpleFormattingTest']//em", "dog"));
			Assert.That(!FormatNodeContainsText("//div[@id='simpleFormattingTest']//u", "dog"));
			Assert.That(!FormatNodeContainsText("//div[@id='simpleFormattingTest']//sup", "dog"));
		}
		...
		private void RemoveOuterWhitespace(XmlNode node)
		{
			...
			foreach (XmlNode whitespacenode in nodeList)
			{
		...
		[TestCase("noRetainMarkup")]
		public void WhitespaceTestLineBreakToP(string source)
		{
			SetupFor(source);
			//This test assumes <span class=""bloom-linebreak""></span>﻿ newlines come back as new paragraphs
			var nodeList = _roundtrippedDom.SafeSelectNodes("//div[@id='whitespaceTest']");
			Assert.That(nodeList.Count, Is.EqualTo(1));
			var node = nodeList[0];
			RemoveOuterWhitespace(node);
			var children = node.ChildNodes;
			Assert.That(children.Count, Is.EqualTo(7));
			...
			Assert.That(children[4].InnerText, Is.EqualTo("This sentence follows two empty paragraphs. It will be followed by a new line."));
			Assert.That(children[5].InnerText, Is.EqualTo("This sentence is trailed by an empty paragraph."));
			Assert.That(children[6].InnerText, Is.EqualTo(""));
		}
		...
		{
			SetupFor(source);
			//This test assumes <span class=""bloom-linebreak""></span>﻿ newlines come back as is
			var nodeList = _roundtrippedDom.SafeSelectNodes("//div[@id='whitespaceTest']");
			Assert.That(nodeList.Count, Is.EqualTo(1));
			var node = nodeList[0];
			RemoveOuterWhitespace(node);
			...
			Assert.That(children[2].InnerText, Is.EqualTo(""));
			Assert.That(children[3].InnerText, Is.EqualTo(""));
			Assert.That(children[4].InnerXml, Is.EqualTo(@"This sentence follows two empty paragraphs. It will be followed by a new line.<span class=""bloom-linebreak""></span>﻿This sentence is trailed by an empty paragraph."));
