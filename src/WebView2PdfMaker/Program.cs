...
namespace WebView2PdfMaker
{
	...
	{
		[Option('T', "margin-top", Default = "10", HelpText = "Set the page top margin (in millimeters)")]
		public string TopMargin { get; set; }

		[Option('B', "margin-bottom", Default = "10", HelpText = "Set the page bottom margin (in millimeters)")]
		public string BottomMargin { get; set; }

		[Option('L', "margin-left", Default = "10", HelpText = "Set the page left margin (in millimeters)")]
		public string LeftMargin { get; set; }

		[Option('R', "margin-right", Default = "10", HelpText = "Set the page right margin (in millimeters)")]
		public string RightMargin { get; set; }

		[Option('O', "orientation", Default = "portrait", HelpText = "Set orientation to Landscape or Portrait")]
		public string Orientation { get; set; }

		[Option('s', "page-size", Default = "A4", HelpText = "Set paper size to: A4, Letter, etc. ")]
		public string PageSizeName { get; set; }

		[Option('h', "page-height", HelpText = "Page Height (in millimeters). Use this with along with page-width instead of page-size, if needed.")]
		...
		[Value(1, MetaName = "output", Required = true, HelpText = "The file name or path to the output pdf. Use quotation marks if it includes spaces.")]
		...
		public void ValidateOptions()
		{
			...
			if (IsFileMissing(InputHtmlUri))
				throw new ApplicationException($"Missing or invalid HTML input file path: \"{InputHtmlUri}\"");
			if (String.IsNullOrEmpty(OutputPdfPath))
				throw new ApplicationException("Missing output PDF file path");
			if (Orientation.ToLowerInvariant() != "portrait" && Orientation.ToLowerInvariant() != "landscape")
				throw new ApplicationException($"orientation must be either portrait or landscape: {Orientation}");
			...
			if (!String.IsNullOrEmpty(PageHeight) && !double.TryParse(PageHeight, out double height))
