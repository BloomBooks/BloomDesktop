...
`.claude/skills/bloom-automation/SKILL.md` (shared with Copilot agents; the
...
```bash
node .claude/skills/bloom-automation/launcherControl.mjs --status --json
...
node .claude/skills/bloom-automation/launcherControl.mjs --ensure-running --wait-ready --json
```
...
```bash
node .claude/skills/bloom-automation/launcherControl.mjs --restart --wait-ready --json
```
...
```bash
node .claude/skills/bloom-automation/switchWorkspaceTab.mjs --http-port <httpPort> --tab edit --json   # collection | edit | publish
...
```bash
node .claude/skills/bloom-automation/launcherControl.mjs --quit-bloom   # Bloom off (graceful), launcher parked for --start/--restart
node .claude/skills/bloom-automation/launcherControl.mjs --shutdown     # everything down: Bloom, dotnet watch, launcher, Vite
```
...
`node .claude/skills/bloom-automation/bloomProcessStatus.mjs --running-bloom --json`
...
`.claude/skills/bloom-automation/SKILL.md` before relying on it, and don't
