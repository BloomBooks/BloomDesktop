...
//
// Usage:
//   node .claude/skills/bloom-automation/dismissProblemDialog.mjs --http-port <port> [--wait] [--timeout-ms <ms>] [--json]
//
...
const parseArgs = () => {
    ...
    for (let index = 0; index < args.length; index++) {
        ...
        if (arg === "--help") {
            console.log(
                "Usage: node .claude/skills/bloom-automation/dismissProblemDialog.mjs --http-port <port> [--wait] [--timeout-ms <ms>] [--json]",
            );
            process.exit(0);
        }
    }
