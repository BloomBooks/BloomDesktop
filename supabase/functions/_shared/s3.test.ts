// Unit tests for _shared/s3.ts: the dev-mode MinIO AssumeRole credential seam, checksum
...
const { getScopedCredentials, hexToBase64, verifyUploadedObject, writeManifestBackup } = await import("./s3.ts");

Deno.test("hexToBase64 round-trips a known SHA-256 hex digest to its base64 form", () => {
    ...
    const rehexed = Array.from(decoded).map((c) => c.charCodeAt(0).toString(16).padStart(2, "0")).join("");
    assertEquals(rehexed, hex, "hexToBase64 must be a faithful hex->base64 re-encoding, not a no-op");
});

Deno.test("getScopedCredentials (dev mode) calls MinIO AssumeRole and returns the STS response shape", async () => {
    ...
    const result = await getScopedCredentials("tc/col1/books/book1/", ["s3:PutObject"]);
...
Deno.test("getScopedCredentials (dev mode) throws if MinIO AssumeRole returns no credentials", async () => {
    const stsMock = mockClient(STSClient);
    stsMock.on(AssumeRoleCommand).resolves({}); // no Credentials field

    let threw = false;
    try {
        await getScopedCredentials("tc/col1/books/book1/", ["s3:PutObject"]);
    } catch (err) {
        threw = true;
        assertEquals((err as Error).message, "MinIO AssumeRole did not return credentials");
    }
    assertEquals(threw, true, "must fail fast rather than return a half-formed credential object");

    stsMock.restore();
});

Deno.test("verifyUploadedObject returns the version-id + checksum when they match", async () => {
    ...
    const result = await verifyUploadedObject(client, "bucket", "tc/col1/books/book1/book.htm", expectedHex);
...
Deno.test("verifyUploadedObject returns null when the stored checksum does not match", async () => {
    const s3Mock = mockClient(S3Client);
    ...
    assertEquals(result, null, "a checksum mismatch must be reported as unverified, not thrown");

    s3Mock.restore();
});

Deno.test("verifyUploadedObject returns null (not throw) when the object is missing", async () => {
    const s3Mock = mockClient(S3Client);
    s3Mock.on(HeadObjectCommand).rejects(new Error("NotFound"));

    const client = new S3Client({ region: "us-east-1" });
    const result = await verifyUploadedObject(client, "bucket", "tc/col1/books/book1/missing.htm", "abc");

    assertEquals(result, null, "a missing object must surface as 'not verified', not an unhandled rejection");

    s3Mock.restore();
});

Deno.test("verifyUploadedObject returns null when VersionId is absent (unversioned bucket misconfig)", async () => {
    const s3Mock = mockClient(S3Client);
    const hex = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";
    s3Mock.on(HeadObjectCommand).resolves({ ChecksumSHA256: hexToBase64(hex) }); // no VersionId

    const client = new S3Client({ region: "us-east-1" });
    const result = await verifyUploadedObject(client, "bucket", "tc/col1/books/book1/book.htm", hex);
...
Deno.test("writeManifestBackup never throws even when the PUT fails (best-effort backup)", async () => {
    const s3Mock = mockClient(S3Client);
    ...
    await writeManifestBackup(client, "bucket", "tc/col1/books/book1/", { some: "manifest" });
