...
Deno.test(
    ...
    async () => {
        ...
        const txFunctions = await readText(
            "supabase/migrations/20260706000004_tc_checkin_txn_functions.sql",
        );

        const schemaHours = [
            ...schema.matchAll(
                /expires_at\s+timestamptz[^,]*INTERVAL '(\d+) hours'/g,
            ),
        ].map((m) => Number(m[1]));
        const resumeHours = [
        ...
        assert(
            schemaHours.length >= 1,
            "expected to find at least one expires_at DEFAULT INTERVAL in the schema",
        );
        assert(
...
Deno.test(
    "invariant: transaction lifetime (48h) is strictly less than the S3 noncurrent-version-expiry floor (7d, dev MinIO)",
    async () => {
        const schema = await readText(
            "supabase/migrations/20260706000001_tc_schema.sql",
        );
        ...
        const txHoursMatch = schema.match(
            /expires_at\s+timestamptz[^,]*INTERVAL '(\d+) hours'/,
        );
        assert(
            txHoursMatch,
            "could not find the checkin_transactions expires_at default in the schema migration",
        );
