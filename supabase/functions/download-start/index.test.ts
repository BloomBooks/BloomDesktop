...
Deno.test("download-start: happy path returns collection-scoped read-only creds", async () => {
    const stsMock = stubAssumeRole();
    const fetchStub = routedFetchStub([{ when: "rpc/download_start_check", status: 200, body: null }]);

    const res = await withMockFetch(
        fetchStub,
        () => callHandler(handler, mockRequest({ collectionId: "col-1" }), { collectionId: "col-1" }),
    );
...
Deno.test("download-start: not a member -> 403, no S3 credentials issued", async () => {
    const stsMock = stubAssumeRole();
    const fetchStub = routedFetchStub([
        { when: "rpc/download_start_check", status: 403, body: { message: JSON.stringify({ error: "not_a_member" }) } },
    ]);
    ...
    assertEquals(stsMock.commandCalls(AssumeRoleCommand).length, 0, "must not issue creds when membership check fails");
...
Deno.test("download-start: missing collectionId -> 400", async () => {
    ...
    const res = await withMockFetch(fetchStub, () => callHandler(handler, mockRequest({}), {}));
