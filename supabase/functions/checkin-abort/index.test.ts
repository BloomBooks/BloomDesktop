...
Deno.test("checkin-abort: happy path calls checkin_abort_tx with the transactionId and returns 200 {}", async () => {
...
Deno.test("checkin-abort: missing transactionId -> 400 before any RPC call", async () => {
...
Deno.test("checkin-abort: RPC 409 already_finished passes through", async () => {
    const fetchStub = routedFetchStub([
        { when: "rpc/checkin_abort_tx", status: 409, body: { message: JSON.stringify({ error: "already_finished" }) } },
    ]);
...
Deno.test("checkin-abort: RPC 404 transaction_not_found passes through", async () => {
    const fetchStub = routedFetchStub([
        { when: "rpc/checkin_abort_tx", status: 404, body: { message: JSON.stringify({ error: "transaction_not_found" }) } },
    ]);
