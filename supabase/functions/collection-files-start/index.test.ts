...
Deno.test("collection-files-start: happy path scopes creds under collectionFiles/{groupKey}/", async () => {
    const stsMock = stubAssumeRole();
    const fetchStub = routedFetchStub([
        { when: "rpc/collection_files_start_tx", status: 200, body: { transactionId: "tx-1", changedPaths: ["allowed.txt"] } },
    ]);
...
Deno.test("collection-files-start: invalid groupKey -> 400 before any RPC/S3 call", async () => {
    const stsMock = stubAssumeRole();
    const fetchStub = routedFetchStub([]);
    const badBody = { ...VALID_BODY, groupKey: "not-a-real-group" };

    const res = await withMockFetch(fetchStub, () => callHandler(handler, mockRequest(badBody), badBody));
...
Deno.test("collection-files-start: RPC 409 VersionConflict passes through with currentVersion", async () => {
    const stsMock = stubAssumeRole();
    const fetchStub = routedFetchStub([
        {
            when: "rpc/collection_files_start_tx",
            status: 409,
            body: { message: JSON.stringify({ error: "VersionConflict", currentVersion: 5 }) },
        },
    ]);
    ...
    assertEquals(stsMock.commandCalls(AssumeRoleCommand).length, 0, "must not issue creds on a version conflict");
